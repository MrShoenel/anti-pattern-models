...
temp <- generateModelOverview(results_sl, models_sl, validationData = valid_sl)
for (cn in colnames(temp)) {
  if (is.numeric(temp[[cn]])) {
    temp[[cn]] <- round(temp[[cn]], 4)
  }
}
...
```{r echo=FALSE, eval=!interactive()}
temp <- generateModelOverview(results_ms, models_ms, validationData = data_stack_valid_sl)
for (cn in colnames(temp)) {
  if (is.numeric(temp[[cn]])) {
    temp[[cn]] <- round(temp[[cn]], 4)
  }
...
```{r echo=FALSE, eval=!interactive()}
temp <- generateModelOverview(results_ms_all, models_ms_all, validationData = data_stack_valid_sl)
for (cn in colnames(temp)) {
  if (is.numeric(temp[[cn]])) {
    temp[[cn]] <- round(temp[[cn]], 4)
  }
