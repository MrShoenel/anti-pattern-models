...
All complementary data and results can be found at Zenodo [@honel_picha_2021]. This notebook was written in a way that it can be run without any additional efforts to reproduce the outputs (using the pre-computed results). This notebook has a canonical URL^[[Link]](https://github.com/MrShoenel/anti-pattern-models/blob/master/notebooks/comm-class-models.Rmd)^ and can be read online as a rendered markdown^[[Link]](https://github.com/MrShoenel/anti-pattern-models/blob/master/notebooks/comm-class-models.md)^ version. All code can be found in this repository, too.
...
The following will give us a correlation matrix of the models' predictions. The goal is to find models with high performance and unrelated predictions, so that they can be combined. The correlations are shown in tables \ref{tab:model-prediction-correlation1} and \ref{tab:model-prediction-correlation2}.
...
if (!interactive()) {
  knitr::kable(
    x = temp[, 1:7],
    booktabs = TRUE,
    caption = "Correlations of models' predictions (part 1).",
    label = "model-prediction-correlation1"
  )
}
```
...
if (!interactive()) {
  knitr::kable(
    x = temp[, 8:ncol(temp)],
    booktabs = TRUE,
    caption = "Correlations of models' predictions (part 2).",
    label = "model-prediction-correlation2"
  )
}
```
...
As for predicting on validation data, we will use the models that were fit to the entire training data (see table \ref{tab:overview1}).
  ...
  label = NULL, caption = NULL)
{
  ...
  for (modelName in modelNames) {
    ...
    overview <- rbind(overview, data.frame(
      model = modelName,
      trAcc = maxRow$Accuracy,
      trKap = maxRow$Kappa,
      predNA = hasNAs,
      valAcc_withNA = cmWithNAs$overall[["Accuracy"]],
      valKap_withNA = cmWithNAs$overall[["Kappa"]],
      valAcc = cm$overall[["Accuracy"]],
      valKap = cm$overall[["Kappa"]]
    ))
  }
  ...
  if (interactive()) {
    ...
    for (cn in colnames(overview)) {
      if (is.numeric(overview[[cn]])) {
        overview[[cn]] <- round(overview[[cn]], 4)
      }
    }
...
```{r warning=FALSE}
generateModelOverview(results_sl, models_sl, validationData = valid_sl, label = "overview1", caption = "Overview of trained models and their performance using validation data.")
```
...
Now show the overview (table \ref{tab:overview2}):

```{r}
generateModelOverview(results_ms, models_ms, validationData = data_stack_valid_sl, label = "overview2", caption = "Results of using a manual stacking of models.")
```


The overview for all models, using oversampled training data, was this (table \ref{tab:overview3}):
...
```{r}
generateModelOverview(results_ms_all, models_ms_all, validationData = data_stack_valid_sl, label = "overview3", caption = "Results of using all models and using oversampled data.")
```
