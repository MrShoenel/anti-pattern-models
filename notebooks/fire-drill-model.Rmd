...
mlm <- MultilevelModel$new(
  ...
  intervalNames = levels(fd_data_concat$interval),
...
# Set both limits to "01" to match shape; to "self" to match shape+magnitude!
# Shape+mag will result in lower, but usually more realistic scores.
stage1_A_Begin <- Stage1Rectifier$new(
  yLimitsRef = "self",
  yLimitsQuery = "self")
stage2_A_Begin <- Stage2Rectifier$new(
...
sm_A_Begin <- SubModel$new(varName = "A", intervalName = "Begin", weight = 1)
sm_A_Begin$setStage1(stage1 = stage1_A_Begin)
sm_A_Begin$setStage2(stage2 = stage2_A_Begin)
mlm$setSubModel(model = sm_A_Begin)



stage1_A_LongStretch <- Stage1NoModel$new(
  approxRefFun = TRUE, approxQueryFun = TRUE)$setYLimitsRef("self")$setYLimitsQuery("self")
stage2_A_LongStretch <- Stage2$new()
stage2_A_LongStretch$addScoreMethod(
  name = "Score_LM", scoreMethod = function(f1, f2) {
    stat_diff_2_functions_lm_score2(maxAngleBetween = 30, requireSign = FALSE)(f1, f2)^5 })
stage2_A_LongStretch$addScoreMethod(
  name = "Score_IntervalLength", scoreMethod = stat_diff_custom_score(callback = function(model, interval, useA) {
    mlm <- model # THIS IS UTMOST IMPORTANT
  ...
  }, model = mlm, interval = "LongStretch", useA = TRUE))

sm_A_LongStretch <- SubModel$new(varName = "A", intervalName = "LongStretch", weight = .5)
sm_A_LongStretch$setStage1(stage1 = stage1_A_LongStretch)
sm_A_LongStretch$setStage2(stage2 = stage2_A_LongStretch)
mlm$setSubModel(model = sm_A_LongStretch)
```
...
# distance >= 0.05 and <= 0.8
mlm$addDefaultBoundaryDistanceConstraints(boundaryDistance = .05, op = "geq")
mlm$addDefaultBoundaryDistanceConstraints(boundaryDistance = .8, op = "leq")

mlm$setBoundary(1, .1)$setBoundary(2, .6)$setBoundary(3, .8)
# For the moment, the last boundary has no influence on fitting, so we fixate it!
mlm$fixateBoundaryAt(indexOrName = 3, fixateAt = .9)

res3 <- doWithParallelCluster(expr = {
