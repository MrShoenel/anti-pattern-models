...
```{r}
res4 <- doWithParallelCluster(expr = {
  mlm$fit(verbose = TRUE, reltol = 1e-8)#, method = "SANN")
}, numCores = 16)
```
...
```{r}
plot_finalProject +
  geom_vline(xintercept = res4$optResult$par[1], color = "blue", size = .5) +
  geom_vline(xintercept = res4$optResult$par[2], color = "blue", size = .5) +
  geom_vline(xintercept = res4$optResult$par[3], color = "blue", size = .5)
```
...
The next test is to randomly initialize the boundaries and have the model attempt to fit them, not to the data but rather to the pattern itself. This is a dummy test to see if the model would converge to our suggested boundaries, given that the data would exactly look like we described it.

```{r}
mlmDummy <- mlm$clone(deep = TRUE)
# Set these to avoid potential errors.
mlmDummy$constrainBoundaryInterval(boundaryIndexOrName = 1, value = .01, op = "geq")
mlmDummy$constrainBoundaryInterval(boundaryIndexOrName = 3, value = .99, op = "leq")
```
...
dummyResults <- doWithParallelCluster(expr = {
  foreach::foreach(
    ...
    for (sf in mlmDummy$sourceFiles) {
      source(sf)
    }
    useMlm <- mlmDummy$clone(deep = TRUE)
    
    useMlm$refBoundaries <- testBoundaries[rIdx, ]
    fitResult <- doWithParallelCluster(expr = {
      useMlm$calibrate(requireAbsoluteConverge = FALSE, verbose = TRUE)
    })
...
# for (rIdx in 1:nrow(testBoundaries)) {
#   scoresBefore <- list()
#   scoresAfter <- list()
#   fitResults <- list()
#   
#   mlmDummy$refBoundaries <- testBoundaries[rIdx, ]
#   temp <- doWithParallelCluster(expr = {
#     mlmDummy$calibrate(requireAbsoluteConverge = FALSE, verbose = TRUE)
#   })
#   
#   if (R6::is.R6(mlmDummy$scoreRef)) {
#     scoresBefore[[paste0(rIdx)]] <- mlmDummy$scoreRef$clone(deep = TRUE)
#   }
#   if (R6::is.R6(mlmDummy$scoreCalibrated)) {
#     scoresAfter[[paste0(rIdx)]] <- mlmDummy$scoreCalibrated$clone(deep = TRUE)
#   }
#   
#   fitResults[[paste0(rIdx)]] <- temp
# }
```
...
```{r}
mlmDummy$refBoundaries <- c(1/4, 1/2, 3/4)
# Let's set some the reference or dummy boundaries:
mlmDummy$setAllBoundaries(fd_data_boundaries)
#mlmDummy$setAllBoundaries(c(1/4, 1/2, 3/4))

if (!is.environment(mlmDummy$calibratedScore)) {
  mlmDummy <- doWithParallelCluster(expr = {
    mlmDummy$calibrate(requireConverge = FALSE, verbose = TRUE)
  }, numCores = 16)
}

if (inherits(mlmDummy$calibratedScore, 'ScoreAggregator')) {
  # TODO: Make all this prettier...
  temp <- doWithParallelCluster({
    list(
      mlmDummy$setAllBoundaries(mlmDummy$refBoundaries)$compute(),
      mlmDummy$setAllBoundaries(mlmDummy$calibratedBoundaries[1, ])$compute())
  })
  
  saBefore <- temp[[1]]
  saAfter <- temp[[2]]
  
  cat("We found more optimal boundaries!\n")
  cat(paste0("Before: ", paste0(mlmDummy$refBoundaries, collapse = ", "), "\n"))
  cat(paste0("After: ", paste0(round(mlmDummy$calibratedBoundaries[1, ], 6), collapse = ", "), "\n\n"))
  
  print(paste0("Scores before: ", saBefore$aggregateUsing_Honel(), " (aggregated), each: ", paste0(sapply(saBefore$rawScores, function(rs) {
    paste0(rs$name, ": ", round(rs$value, 4))
  }), collapse = ", ")))
  print(paste0("Scores after: ", saAfter$aggregateUsing_Honel(), " (aggregated), each: ", paste0(sapply(saAfter$rawScores, function(rs) {
    paste0(rs$name, ": ", round(rs$value, 4))
  }), collapse = ", ")))
}
```
