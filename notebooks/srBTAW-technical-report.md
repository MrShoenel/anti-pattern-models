...
f_q3_prime <- function(x) {
  ...
  vartheta_q <- vt_q3[3]
  # These two is where we want the signal to be (translation target range) theta_q
  # <- bounds[3] - bounds[2]
...
f_q3_test <- function(x) {
  f_prime_proto(x, source_b = 0, source_e = 1, target_b = 0.5, target_e = 1)
}

f_q3_test2 <- function(x) {
  f_prime_proto(x, source_b = 0.55, source_e = 0.7, target_b = 0, target_e = 1)
}
...
is the lower bound for interval *q* and  \> 0). This could however lead
to the sum of all such lengths being greater than the extent of the
...
\\begin{aligned}
  ...
  \\phi_q\\equiv&\\;\\begin{cases}
    0,&\\text{if}\\;q=1,
    \\\\
    \\sum\_{i=1}^{q-1}\\,l_i^{(c)},&\\text{otherwise}
  \\end{cases}\\;,
  \\\\\[1ex\]
...
\\begin{aligned}
  ...
  \\\\\[0ex\]
  &\\;\\;\\;\\;\\;\\;+\\Big(x-t_b^{(q)}\\Big)\\times{\\delta_q^{(t)}}^{-1}\\times \\overbrace{\\max{\\Big(\\bm{\\lambda}\_q,\\mathcal{R}(2\\times l_q)-l_q\\Big)}\\times\\Bigg\[\\sum\_{i=1}^{\\max{(Q)}}\\,l'\_i\\Bigg\]^{-1}}^{\\text{recall}\\;l_q^{(c)}=\\frac{l'\_q}{\\psi}\\times(\\beta_u-\\beta_l)\\text{, here we got}\\;l'\_q\\times\\psi^{-1}}
  \\\\\[0ex\]
...
$$
\\begin{aligned}
  \\mathsf{T}^{(l)}\\Big(\\bm{\\tau}^{(b)}\\Big)=&\\;\\Big\\{\\;l_1=\\bm{\\tau}^{(b)}\_2-\\bm{\\tau}^{(b)}\_1,\\;\\dots,\\;l\_{\\left\\lvert\\bm{\\tau}\\right\\rvert-1}=\\bm{\\tau}^{(b)}\_{\\left\\lvert\\bm{\\tau}\\right\\rvert}-\\bm{\\tau}^{(b)}\_{\\left\\lvert\\bm{\\tau}\\right\\rvert-1}\\;\\Big\\}\\;\\text{,}
  \\\\\[0ex\]
  ...
  \\\\\[1ex\]
  q\\in Q;\\;Q=&\\;\\Big\\{1,\\;\\dots\\;,\\left\\lvert\\bm{\\theta}^{(b)}\\right\\rvert-1\\Big\\}\\;\\text{, where}\\;Q\\;\\text{is ordered low to high, i.e.,}\\;q_i\\prec q\_{i+1}\\text{,}
  \\\\\[0ex\]
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  &\\times\\bigg(x-\\overbrace{\\min{\\Big(\\gamma_e-\\gamma_d,\\max{\\big(\\gamma_b, \\min{(b,e)}\\big)}\\Big)}}^{\\beta_l}-\\overbrace{\\Big(\\max{(\\bm{\\lambda}\_1,l_1)}+\\;\\dots\\;+\\max{(\\bm{\\lambda}\_{q-1},l\_{q-1})}\\Big)}^{\\phi_q\\;\\text{(sum of all preceding intervals' lengths;}\\;0\\;\\text{if}\\;q=1\\text{)}}\\bigg)
  \\\\\[1ex\]
  ...
  \\\\\[1ex\]
  &\\times\\overbrace{\\max{\\Big(\\gamma_d,\\;\\max{(\\bm{\\lambda}\_1,l_1)}+\\;\\dots\\;+\\max{(\\bm{\\lambda}\_{\\max{(Q)}},l\_{\\max{(Q)}})}\\Big)}}^{\\phi^{(s)}\\;\\text{(scale of}\\;\\phi\\text{; sum of all intervals' minimum lengths)}}
  \\\\\[1ex\]
...
$\\phi=(\\beta_u-\\beta_l)\\times\\max{\\Bigg(\\gamma_d,\\Bigg\[\\sum\_{j=1}^{\\max{(Q)}}\\,\\max{(\\bm{\\lambda}\_j,l_j)}\\Bigg\]\\Bigg)}$.
...
(*β*<sub>*u*</sub>−*β*<sub>*l*</sub>), and we have already derived them.
The scale expression is a nested max  with a summation inside, that
affects ∀ *l*, regardless of what *q* equals.

$$
\\begin{aligned}
  \\phi^{(s)}=&\\;\\max{\\Bigg(\\gamma_d,\\Bigg\[\\sum\_{i=1}^{\\max{(Q)}}\\,\\max{(\\bm{\\lambda}\_i,l_i)}\\Bigg\]\\Bigg)}
  \\\\\[1ex\]
...
*l*<sub>*q*</sub> or ∀ *l*<sub>*r*</sub>, *r* \< *q*. However, since it
is the summation that will be affected, and since it does the same
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  =&\\;\\mathcal{H}\\Big(l_q-\\bm{\\lambda}\_q\\Big)\\times \\mathcal{H}\\Big(\\Big\[\\;\\overbrace{\\mathcal{R}(l_1-\\bm{\\lambda}\_1)+\\bm{\\lambda}\_1+\\;\\dots\\;+R(l_4-\\bm{\\lambda}\_4)+\\bm{\\lambda}\_4}^{\\sum\_{i=1}^{\\max{(Q)}}\\,\\max{(\\bm{\\lambda}\_i,l_i)}}\\;\\Big\]-\\gamma_d\\Big)\\;\\text{.}
\\end{aligned}
$$
...
$$
\\begin{aligned}
  m\_{q=4}^c=&\\;f\\bigg(s_b^{(q)}+\\frac{(x-\\beta_l-\\phi_q)\\times\\delta_q\\times\\phi}{\\max{(\\bm{\\lambda_q},l_q)}}\\bigg)
  \\\\\[1ex\]
  =&\\;f\\bigg(s_b^{(q)}+\\frac{(\\delta_qx-\\delta_q\\beta_l-\\delta_q\\phi_q)\\times\\phi}{\\max{(\\bm{\\lambda_q},l_q)}}\\bigg)
  \\\\\[1ex\]
  =&\\;f\\Big(s_b^{(q)}+\\Big\[\\;\\overbrace{\\delta_qx-\\delta_q\\beta_l-\\overbrace{\\delta_q\\max{(\\bm{\\lambda_1},l_1)}-\\delta_q\\max{(\\bm{\\lambda_2},l_2)}-\\delta_q\\max{(\\bm{\\lambda_3},l_3)}}^{\\phi\_{q=4}}}^{(a-b-c-d-e)}\\;\\times\\overbrace{\\phi\\times\\max{(\\bm{\\lambda_q},l_q)}^{-1}}^{(j-k)}\\;\\Big\]\\Big)
  \\\\
  \\vdots
  \\\\
...
with. It is worth noting that summands with *c* exist if *q* \> 1, those
with *d* exist if *q* \> 2 and so on. Those are essentially what
...
$$
\\begin{aligned}
  \\mathcal{R}^{(\\text{bb})}=&\\;\\sum\_{i=1}^{\\left\\lvert\\bm{\\theta}\\right\\rvert}\\;\\mathcal{H}\\Big(b_i^{(l)}-\\bm{\\theta}\_i\\Big)\*\\big(b_i^{(l)}-\\bm{\\theta}\_i\\big) + \\mathcal{H}\\Big(\\bm{\\theta}\_i-b_i^{(u)}\\Big)\*\\big(\\bm{\\theta}\_i-b_i^{(u)}\\big)\\;\\text{, where}
  \\\\\[1ex\]
...
more quickly, and it should speed up the process if *b* \> *e* (which is
handled by the model but still not OK).
...
increasing, only that *s*(0) ≺ *s*(*x*), ∀ *x* \> 0, so the function may
have saddle-points. Otherwise, its constrained version would potentially
divide by 0.
...
portion of the function in the interval \[0,1\], which is then scaled to
the current target-extent of \[0, *δ*<sub>*q*</sub><sup>(*t*)</sup>\]:
...
If we have the upper and lower bound for some loss ℒ(⋅), we can define a
**score-operator**, S<sup>⊕</sup>, as:
...
\[0,1\], where, depending on the definition, zero or one is the best (in
the previous definition 1 would be the best, hence we wrote
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  \\rho_q^{(\\text{WP})}=&\\;\\int\\displaylimits\_{\\mathbf{x}\_q^{(\\text{WP})}}\\,r(x)-y_q^{\\min}\\,dx\\;\\text{, note that}\\;\\forall\\,x\\to y_q^{\\min}\\leq r(x)\\leq y_q^{\\max}\\text{, or}
  \\\\\[1ex\]
  \\rho_q^{(\\text{WP}\_d)}=&\\;\\frac{1}{N}\\sum\_{i=1}^{N}\\,r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)\\;\\text{,}
  \\\\\[0ex\]
  ...
  \\\\\[1em\]
  \\mathcal{L}\_q^{(\\text{ABC}\_c)}=&\\;\\beta_u^{-1}\\times\\int\\displaylimits\_{\\mathbf{x}\_q^{(\\text{WP})}}\\,\\left\\lvert r(x_q)-m_q^{(c)}\\Big(x_q\\Big)\\right\\rvert\\;\\text{, the continous ABC-loss,}
  \\\\\[1ex\]
  \\mathcal{L}\_q^{(\\text{ABC}\_d)}=&\\;\\frac{1}{N\\times\\beta_u}\\sum\_{i=1}^{N}\\,\\left\\lvert r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)-m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)\\right\\rvert\\;\\text{, (discrete case), with gradient}
  \\\\\[1ex\]
  \\nabla\\,\\mathcal{L}\_q^{(\\text{ABC}\_d)}=&\\;\\frac{1}{N\\times\\beta_u}\\sum\_{i=1}^{N}\\,\\frac{\\nabla\\,m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)\\times\\bigg(m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)-r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)\\bigg)}{\\left\\lvert m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)-r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)\\right\\rvert}\\;\\text{.}
\\end{aligned}
$$
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  \\rho_q^{(\\text{WP})}=&\\;\\int\\displaylimits\_{\\mathbf{x}\_q^{(\\text{WP})}}\\,\\Big(r(x)-y_q^{\\min}\\Big)^2\\;\\text{, note that}\\;\\forall\\,x\\to y_q^{\\min}\\leq r(x)\\leq y_q^{\\max}\\text{, or}
  \\\\\[1ex\]
  \\rho_q^{(\\text{WP}\_d)}=&\\;\\frac{1}{N}\\sum\_{i=1}^{N}\\,\\bigg(r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)-y_q^{\\min}\\bigg)^2\\;\\text{,}
  \\\\\[0ex\]
  ...
  \\\\\[1em\]
  \\mathcal{L}\_q^{(\\text{RSS}\_c)}=&\\;\\beta_u^{-1}\\times\\int\\displaylimits\_{\\mathbf{x}\_q^{(\\text{WP})}}\\,\\bigg(r(x_q)-m_q^{(c)}\\Big(x_q\\Big)\\bigg)^2\\;\\text{, the continous RSS-loss,}
  \\\\\[1ex\]
  \\mathcal{L}\_q^{(\\text{RSS}\_d)}=&\\;\\frac{1}{N\\times\\beta_u}\\sum\_{i=1}^{N}\\,\\bigg(r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)-m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)\\bigg)^2\\;\\text{, (discrete case), with gradient}
  \\\\\[1ex\]
  \\nabla\\,\\mathcal{L}\_q^{(\\text{RSS}\_d)}=&\\;-\\frac{2}{N\\times\\beta_u}\\sum\_{i=1}^{N}\\,\\nabla\\,m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)\\times\\bigg(r\\Big(\\mathbf{x}\_{q,i}^{(\\text{WP})}\\Big)-m_q^{(c)}\\Big(\\mathbf{x}\_{q,i}^{(\\text{WC})}\\Big)\\bigg)\\;\\text{.}
\\end{aligned}
$$
...
$$
\\begin{aligned}
  {\\text{SSD/RSS}} &: (x, y) \\mapsto \\\|x-y\\\|\_2^2 = \\langle x-y, x-y\\rangle = \\sum\_{i=1}^{n} (x_i-y_i)^2
  \\\\\[0ex\]
  {\\text{MAE}} &: (x, y) \\mapsto \\frac{d\_{\\mathbf{SAD}}}{n} = \\frac{\\\|x-y\\\|\_1}{n} = \\frac{1}{n}\\sum\_{i=1}^{n} \\left\\lvert x_i-y_i\\right\\rvert
  \\\\\[0ex\]
  {\\text{MSE}} &: (x, y) \\mapsto \\frac{d\_{\\mathbf{SSD}}}{n} = \\frac{\\\|x-y\\\|\_2^2}{n} = \\frac{1}{n}\\sum\_{i=1}^{n} (x_i-y_i)^2
  \\\\\[0ex\]
  {\\text{Euclidean}} &: (x, y) \\mapsto \\\|x-y\\\|\_2 = \\sqrt{d\_{\\mathbf{SSD}}} = \\sqrt{\\sum\_{i=1}^{n} (x_i-y_i)^2}
  \\\\\[0ex\]
  {\\infty\\text{/Chebyshev}} &: (x, y) \\mapsto \\\|x-y\\\|\_\\infty = \\lim\_{p \\rightarrow \\infty}\\bigg(\\sum\_{i=1}^{n} \|x_i-y_i\|^p\\bigg)^\\frac{1}{p} = \\max\_{i} \\left\\lvert x_i-y_i\\right\\rvert
  \\\\\[0ex\]
  {\\text{p/Minkowski}} &: (x, y) \\mapsto \\\|x-y\\\|\_p = \\bigg(\\sum\_{i=1}^{n} \\left\\lvert x_i-y_i\\right\\rvert^p\\bigg)^\\frac{1}{p}
  \\\\\[0ex\]
  {\\text{Canberra}} &: (x, y) \\mapsto \\sum\_{i=1}^{n} \\frac{\\left\\lvert x_i-y_i\\right\\rvert}{\\left\\lvert x_i\\right\\rvert+\\left\\lvert y_i\\right\\rvert}
  \\\\\[0ex\]
  {\\text{cosine}} &: (x, y) \\mapsto 1-\\frac{\\langle x, y\\rangle}{\\\|x\\\|\_2\\\|y\\\|\_2} = 1-\\frac{\\sum\_{i=1}^{n} x_i y_i}{\\sqrt{\\sum\_{i=1}^{n} x_i^2}\\sqrt{\\sum\_{i=1}^{n} y_i^2}}
  \\\\\[0ex\]
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  \\operatorname{arclen}\_{(d)} f(x)=&\\;\\lim\_{N\\to\\infty}\\,\\sum\_{i=1}^N\\,\\left\\lvert f(\\delta_i)-f(\\delta\_{i-1})\\right\\rvert\\;\\text{, where}
  \\\\\[0ex\]
  ...
  \\\\\[1ex\]
  \\nabla\\,\\operatorname{arclen}\_{(d)}f(x)=&\\;\\lim\_{N\\to\\infty}\\,\\sum\_{i=1}^N\\,\\frac{\\Big(f(\\delta_i)-f(\\delta\_{i-1})\\Big)\\times\\Big(f'(\\delta_i)-f'(\\delta\_{i-1})\\Big)}{\\left\\lvert f(\\delta_i)-f(\\delta\_{i-1})\\right\\rvert}\\;\\text{.}
\\end{aligned}
$$
...
ℒ<sup>(WP)</sup>(⋅) \> ℒ<sup>(WC)</sup>(⋅) (and vice versa). It is now
straightforward to see that we can effortlessly plug in the arc-length
operator defined previously. We have defined the ratio-loss such that a
perfect ratio of $\\frac{1}{1}$ results in a loss of 0. As the
denominator approaches ∞, the loss approaches 1, i.e.,
...
| beta_1\_2-20       | \-  | \-  |  0.000 |  1.000 | 0.000 | 1.000 | 5.07e-05 | 2.53e-06 |   123 |   123 |
| beta_1\_2-40       | \-  | \-  |  0.000 |  1.000 | 0.000 | 1.000 | 1.35e-03 | 3.37e-05 |   123 |   123 |
...
of the sr-BAW parameters, i.e., *v* or any of the terminal translations
in **ϑ**<sup>(*y*)</sup>, then this will eliminate any
...
``` r
ggplot() + xlim(0, 1) + stat_function(fun = f, color = "red") + stat_function(fun = baw, 
...
0). Also, $\\frac{1}{2}\\big(\\beta_u-\\beta_l\\big)$ can be replaced
...
\\begin{aligned}
  ...
  \\\\\[1ex\]
  b\_{\\beta_l}(x),b\_{\\beta_u}(x)\\;\\dots&\\;\\text{functions over the lower- and upper boundaries,}
  \\\\\[1ex\]
  ...
  \\delta_q=&\\;\\begin{cases}
    \\text{if}\\;o_q\<\\beta_l,&\\begin{cases}
      \\text{if}\\;o_q+\\bm{\\vartheta}\_q^{(y)}>\\beta_l,&\\int_s^{\\max{(\\mathbf{x}\_q)}}\\,\\min{\\big(b\_{\\beta_u}(x),t_q(x)\\big)}-b\_{\\beta_l}(x)\\,dx,
      \\\\
      \\text{else}&\\min{\\Big(\\delta_q^{(\\max)},\\int_s^t\\,b\_{\\beta_l}(x)-t_q(x)\\Big)},
    \\end{cases}
    \\\\
    \\text{else if}\\;o_q\>\\beta_u,&\\begin{cases}
      \\text{if}\\;o_q+\\bm{\\vartheta}\_q^{(y)}\<\\beta_u,&\\int\_{\\min{(\\mathbf{x}\_q)}}^{t}\\,\\min{\\big(b\_{\\beta_u}(x),t_q(x)\\big)-b\_{\\beta_l}(x)}\\,dx,
      \\\\
      \\text{else}&0,
    \\end{cases}
    \\\\
    \\text{else}&\\begin{cases}
      \\text{if}\\;o_q+\\bm{\\vartheta}\_q^{(y)}>\\beta_u,&\\int_s^t\\,\\min{\\big(b\_{\\beta_u}(x),t_q(x)\\big)}-b\_{\\beta_l}(x)\\,dx,
      \\\\
      \\text{else}&\\int_s^t\\,t_q(x)-b\_{\\beta_l}(x)\\,dx,
    \\end{cases}
  \\end{cases}
  \\\\\[1ex\]
  \\mathcal{R}^{(\\text{trapez})}=&\\;{\\max{(Q)}}^{-1}\\times\\sum\_{i=1}^{\\max{(Q)}}\\,\\frac{\\delta_q}{\\delta_q^{(\\max)}}\\mapsto\[0+,1\]\\;\\text{, and the log-loss:}
  \\\\\[1ex\]
...
\\begin{aligned}
  ...
  \\\\
  \\lVert \\bm{\\omega}\\rVert_2=&\\;\\sqrt{\\sum\_{i=1}^{\\left\\lvert\\bm{\\omega}\\right\\rvert}\\,\\bm{\\omega}\_i^2}\\;\\text{, the}\\;L_2\\text{-norm, alternatively}
  \\\\\[1ex\]
  \\lVert \\bm{\\omega}\\rVert_2^2=&\\;\\sum\_{i=1}^{\\left\\lvert\\bm{\\omega}\\right\\rvert}\\,\\bm{\\omega}\_i^2\\;\\text{, the squared}\\;L_2\\text{-norm, with regularizer/loss}
  \\\\\[1ex\]
