...
for (pname in names(projects_sc)) {
  ...
  for (rn in rownames(grid)) {
    ...
    if (is_versus) {
      ...
      newrow_sc[1, feat_name] <- log(sqrt(.Machine$double.eps) + jsd_segment(p = pdf_p, q = pdf_q, ext = interval_ext))
    } else {
...
for (pname in names(projects_it)) {
  ...
  for (divergence in c("REQ_vs_DEV", "REQ_vs_DESC", "DEV_vs_DESC")) {
    ...
    for (idx in 1:10) {
      ...
      newrow_it[1, feat_name] <- log(sqrt(.Machine$double.eps) + jsd_segment(p = pdf_p, q = pdf_q, ext = interval_ext))
    }
  }
