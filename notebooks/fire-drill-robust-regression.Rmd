...
adaptive_training_caret <- function(
    ...
    model_type = c("avNNet", "glm", "M5", "rf", "svmPoly", "treebag", "xgbTree", "gbm", "rankModel"),
    num_train = 5,
    num_valid = 50,
    do_pca = TRUE
) {
  ...
  for (seed in seeds) {
    ...
    tryCatch({
      if (model_type == "rankModel") {
        ...
        temp1 <- caret::train(
          allowParallel = FALSE, nthreads = 1,
...
model_types <- c("avNNet", "gbm", "glm", "M5", "rf", "svmPoly", "treebag")
...
Let's show some results for the Random forest (figure \ref{fig:rob-reg-rf1}).
In figure \ref{fig:rob-reg-rf2} we show the distribution of validation errors depending on the number of training instances (same setup as in figure \ref{fig:rob-reg-rf1}).

```{r rob-reg-rf1, echo=FALSE, fig.cap="Robust regression using a Random forest, no PCA, and the oversampled source code dataset.", fig.align="top", fig.pos="ht!"}
temp <- res_grid[res_grid$model_type == "rf" & !res_grid$do_pca & res_grid$dataset == "dataset_sc_oversampled",]
...
plot(temp$Mean, type="l", xlab = "Number of training instances", ylab = "RMSE (validation data)", ylim = use_ylim)
grid()
lines(temp$Max, col="blue")
lines(temp$Min, col="red")
legend(x = 38, y = 4.25, legend = c("mean", "max", "min"), col = c("black", "blue", "red"), lwd = 2)
```

```{r rob-reg-rf2, echo=FALSE, message=FALSE, fig.height=8, fig.width=6, fig.cap="Ridge plot of distributions of RMSEs on validation data, dependent on the number of training samples.", fig.align="top", fig.pos="ht!"}
temp <- res_grid[res_grid$model_type == "rf" & res_grid$do_pca & res_grid$dataset == "dataset_sc_oversampled",]
