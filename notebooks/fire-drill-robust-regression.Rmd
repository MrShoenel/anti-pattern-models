...
for (v in rownames(mat)) {
  temp <- vi_np_sc$res[vi_np_sc$res$feature == v,] %>% group_by(segment) %>% summarise(total_imp = sum(vi_rel), .groups = "drop")
...
In figure \ref{fig:varimp-np-sc} it becomes clear that the divergence features are much less important than the amount features when __both__ types of features are used in a regression model.
...
Today, we have no _quantitative_ understanding of the Fire Drill and a decision tree gives us one such perspective.
