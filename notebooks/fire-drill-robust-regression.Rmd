...
```{r}
if (!endsWith(x = getwd(), suffix = "notebooks")) {
  setwd(paste0(getwd(), "/notebooks"))
}
```
...
gg2_style <- function(facets = FALSE) {
  sty <- theme_minimal(base_size = 10) + theme(
    ...
    axis.title.y.left = element_text(margin = ggplot2::margin(r = 8)))
  ...
  if (facets) {
    sty <- sty + theme(
      ...
      strip.text.x = element_text(color="black", margin = ggplot2::margin(t = 2, b = 2)),
      strip.text.y = element_text(color="black", margin = ggplot2::margin(l = 4, r = 2)))
  }
  sty
}
```
...
hist(ground_truth_all$consensus, breaks = -1:10)
```
        ...
        for (rn in rownames(use_grid)) {
          tryCatch({
            ...
            temp1 <- neuralnet::neuralnet(
              ...
              hidden = hidden, threshold = 5e-3, linear.output = TRUE, rep = 3, lifesign = "full", lifesign.step = 1000)
...
temp <- res_grid %>% group_by(do_pca) %>% summarise(
  RMSE_mean = mean(rmse_valid, na.rm = TRUE),
  RMSE_median = median(rmse_valid, na.rm = TRUE),
  RMSE_sd = sd(rmse_valid, na.rm = TRUE),
  RMSE_min = min(rmse_valid, na.rm = TRUE),
  RMSE_max = max(rmse_valid, na.rm = TRUE),
  .groups = "drop")
...
create_variable_importance <- function(dataset) {
  # Note that PLS is very sensitive to (un-)scaled data, so we
  # need to make sure the data is properly pre-processed.
  cn <- colnames(dataset)
  cn_x <- cn[cn != "gt"]
  ...
  model <- caret::train(
    x = dataset[, cn_x], y = dataset[,]$gt, method = "pls",
...
vi_sc <- create_variable_importance(
  dataset = cbind(dataset_sc, data.frame(gt = ground_truth_all$consensus)))
vi_sc_cdf <- create_variable_importance(
  dataset = cbind(dataset_sc_cdf, data.frame(gt = ground_truth_all$consensus)))
vi_it <- create_variable_importance(
  dataset = cbind(dataset_it, data.frame(gt = ground_truth_all$consensus)))
```
...
```{r echo=FALSE, eval=FALSE}
temp <- vi_sc$res %>% group_by(segment) %>% summarise(Mean_vi = sum(vi_rel), .groups = "drop")
...
Also, here we will be using the __non-mixture__ version of the `FREQ` activity. The mixture as used previously, simply is a linearly weighted convex combination of the `A`- and `CP`-densities.
In the following, when attempting to adaptively train a regression model and learning from the features, we expect that using a non-mixture version will increase the entropy.


```{r}
dataset_np_sc <- loadResultsOrCompute(file = "../data/rob-reg_dataset_np_sc.csv", computeExpr = {
  grid <- expand.grid(list(
    interval = 1:10,
    activity = c("A", "CP", "FREQ_nm", "A_vs_CP", "A_vs_FREQ_nm", "CP_vs_FREQ_nm") # Let's skip SCD and only use the activities for now
  ))
  grid$activity <- as.character(grid$activity)
...
```{r}
dataset_np_it <- loadResultsOrCompute(file = "../data/rob-reg_dataset_np_it.csv", computeExpr = {
  library(readxl)
...
```{r}
dataset_np_sc_oversampled <- loadResultsOrCompute(file = "../data/rob-reg_dataset_np_sc_oversampled.csv", computeExpr = {
  temp <- cbind(dataset_np_sc, data.frame(gt = ground_truth_all$consensus))
  balance_num_labels(dataset = temp, num = min_rows)
})

dataset_np_it_oversampled <- loadResultsOrCompute(file = "../data/rob-reg_dataset_np_it_oversampled.csv", computeExpr = {
...
par(mfrow = c(7,4), mar = c(4.5,4.5,2,0.25))
...
for (model_type in c("gbm", "glm", "nnet", "rf", "svmPoly", "treebag", "rankModel")) {
...
makefig <- function() {
  rlang::expr({
    temp <- res_grid_np_divergence %>% filter(!do_pca & num_train == 20 & dataset == "dataset_np_sc_oversampled")
    temp <- temp[!is.na(temp$rmse_valid),]
    
    # https://cran.r-project.org/web/packages/ggridges/vignettes/introduction.html
    ggplot(temp, aes(x = rmse_valid, y = model_type, fill = model_type)) +
      #stat_density_ridges(quantile_lines = TRUE) +
      geom_density_ridges2(alpha = .85) +
      #stat_density_ridges(quantile_lines = TRUE, show.legend = FALSE) +
      #scale_y_discrete() +
      scale_fill_manual(values = RColorBrewer::brewer.pal(6, "Set3")) +
      #geom_density_ridges_gradient(scale = 0.975, rel_min_height = 0.0025) +
      #scale_fill_viridis_c(option = "C") +
      scale_x_continuous(limits = c(0, 5), breaks = seq(-1, 6, by = .5), minor_breaks = seq(-1, 6, by = .25)) +
      #theme_ridges() +
      xlab("RMSE (validation data)") + ylab("") + labs(fill = "Model", title = "Distribution of validation errors for all six models, each trained on 20 instances.") +
      gg2_style() +
      theme(legend.title = element_text(size = 9, margin = ggplot2::margin(r = 10)),
            legend.text = element_text(size = 8, margin = ggplot2::margin(l = -1, r = 3)),
            legend.position = "right",
            legend.key = element_rect(fill = NA, colour = "white"),
            axis.title.x = element_text(size = 9, margin = ggplot2::margin(t = 1, unit = "line")),
            ...
            plot.margin = ggplot2::unit(x = c(.3, -.3, 0, -.3), units = "line"),
            plot.title = element_text(size = 9, face = "bold", hjust = 0.5, margin = ggplot2::margin(b = 1.5, unit = "line"))) +
      guides(fill = guide_legend(nrow = 6, byrow = TRUE, override.aes = list(size = 1)))
  })
}
eval(makefig())
```
...
```{r echo=FALSE, eval=FALSE}
tikzDevice::tikz(file = paste0("../figures/fig_", "rr_model-val-rmse.tex"), width = FIG_WIDTH, height = 3.1, verbose = FALSE)
eval(makefig())
dev.off()
```
...
Figure \ref{fig:np-overview} compares all of the seven trained models across both data types, source code and issue-tracking, with and without applied PCA, and using different feature sets (amount only, divergence only, both).
...
It appears that the expected average validation error is lower and has also lower variance when ___not___ using PCA.
Simplified, more features is better, but the difference is not large (but still significant).


```{r echo=FALSE, warning=FALSE}
temp <- res_grid_np_both %>% filter(model_type == "nnet") %>% group_by(dataset, do_pca) %>% arrange(dataset, do_pca) %>% summarise(
  RMSE_mean = mean(rmse_valid, na.rm = TRUE),
  RMSE_median = median(rmse_valid, na.rm = TRUE),
  RMSE_sd = sd(rmse_valid, na.rm = TRUE),
  RMSE_min = min(rmse_valid, na.rm = TRUE),
...
temp$dataset <- substr(x = temp$dataset, start = 9, stop = 99)
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Mean, median, standard deviation, min, and max of the RMSE validation error, grouped by the type of data (it=issue-tracking, sc=source code) conditionally applying PCA for feature set \"both\".",
...
Using all features, only $14$ instances are required to achieve a generalization error (RMSE) of $\leq1$ (with a standard deviation of $\approx0.31$).
...
```{r nnet-expected-gen-err, echo=FALSE, fig.cap="The expected validation error for the best-performing model and dataset (source code, no PCA, divergence-features)."}
temp <- res_grid_np_divergence[,]
temp <- temp[temp$model_type == "nnet" & !temp$do_pca & temp$dataset == "dataset_np_sc_oversampled",]
temp <- temp[stats::complete.cases(temp[, colnames(temp)[colnames(temp) != "num_comps"]]),]
temp <- temp %>% group_by(num_train) %>% summarize(Mean = mean(rmse_valid), Max = max(rmse_valid), Min = min(rmse_valid), .groups = "drop")
...
temp <- temp %>% group_by(num_train) %>% summarize(
  "Unimodal?" = stats::shapiro.test(x = rmse_valid)$p.value >= 0.05 || LaplacesDemon::is.unimodal(x = rmse_valid),
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    caption = "Normality test and p-value, mean, median, standard deviation, min, and max of the RMSE validation error, for the neural network model, using the source code data, amount- and divergence-features combined, and no PCA. The error reduces steadily with the number of training instances.",
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Confidence intervals of the best neural network model trained on $17$ instances. The table includes values according to the 68-95-99.7\\%-rule and Chebyshev's inequality (cut off at 0).",
...
if (interactive()) {
  ...
  knitr::kable(
    x = res,
    booktabs = TRUE,
    caption = paste0("Four different inequalities giving and their confidence for a given deviation (for the best neural network model trained on $17$ instances). The Three Sigma rule has the tightest bounds, while Chebyshev's inequality has the loosest. The mean of the data is $", round(mean(temp$rmse_valid), 3), "$ and the standard deviation is $", round(sd(temp$rmse_valid), 3), "$."),
...
makefig <- function(paper = FALSE) {
  rlang::expr({
    ...
    fields::image.plot(list(
      ...
      #col = c(grey.colors(n = 25, start = .2, end = .975, gamma = 1.5), "white"),
      col = c(
        "#005C4DFF",
        # 24 colors: 16,6,2 or 15,6,3:
        #colorRampPalette(colors = c("#006979FF", "#E0EFF3FF"))(18)[2:17],
        #colorRampPalette(colors = c("#0C46A0FF", "#E3F2FDFF"))(9)[3:7],
        #colorRampPalette(colors = c("#311A92FF", "#ECE6F6FF"))(10)[c(4,7,10)],
    ...
    #abline(h = c(.5, 1), lwd = 3, lty = c(2,5), col = "#fc7703")
    ...
    #grid(col = "#444444", lwd = .5, nx = 12, ny = 11)
    abline(v = seq(0, 55, by = 5), h = seq(0, 3, by = .5), lty = 3, lwd = .5, col = "#666666")
    use_leq <- function(n) {
      if (paper) paste0("$\\leq\\!", n, "$") else paste0("<= ", n)
    }
    legend(x = 29, y = 2.9, legend = c("Mean RMSE (validation)", paste0(use_leq(1), " standard deviation"), paste0(use_leq(2), " std. devs."), paste0(use_leq(3), " std. devs.")), col = c("#FD4103", "#000000", "#333333", "#666666"), lwd = 2, lty = c(1, 1, 2, 4), bg = "white", cex = cex, text.width = 15)
  })
}
...
saveTikzExpr(makefig(paper = TRUE), "rr_3sigma-robust", height = 3.7, width = FIG_WIDTH)
```
...
Here we will estimate the variable importance, similar to how it was done previously.
The main difference is that instead of only relying on partial least squares, we will rely on the averaged variable importance of a handful of models, in order to get more robust results.


```{r echo=FALSE}
# We will have to override the function for variable importance:
create_variable_importance <- function(dataset, method = c("cubist", "gamboost", "glmboost", "glmnet", "nnet", "pls", "plsRglm", "rpart2", "rf", "treebag", "xgbTree")) {
  method <- match.arg(method)
  # Note that PLS (and other methods) are very sensitive to (un-)scaled
  # data, so we need to make sure the data is properly pre-processed.
  cn <- colnames(dataset)
  cn_x <- cn[cn != "gt"]
  pre_proc_method <- c("nzv", "center", "scale")
  pre_proc <- caret::preProcess(x = dataset[, cn_x], method = pre_proc_method)
  ...
  model <- caret::train(
    x = dataset[, cn_x], y = dataset[,]$gt, method = method,
    trControl = caret::trainControl(method = "LOOCV", number = 100))
  ...
  for (rn in rownames(df)) {
    ...
    sp <- c(paste(temp[1:(length(temp) - 1)], collapse = "_"), temp[length(temp)])
...
vi_np_sc <- loadResultsOrCompute(file = "../results/rob-reg_vi_np_sc.rds", computeExpr = {
  doWithParallelCluster(expr = {
    ...
    for (method in c("gamboost", "nnet", "pls", "plsRglm", "treebag")) {
      templ[[method]] <- create_variable_importance(dataset = dataset_np_sc_oversampled, method = method)
    }
    templ
  })
})
...
vi_np_it <- loadResultsOrCompute(file = "../results/rob-reg_vi_np_it.rds", computeExpr = {
  doWithParallelCluster(expr = {
    templ <- list()
    for (method in c("gamboost", "nnet", "pls", "plsRglm", "treebag")) {
      templ[[method]] <- create_variable_importance(dataset = dataset_np_it_oversampled, method = method)
    }
    templ
  })
})
...
```{r echo=FALSE}
merge_varimps <- function(l) {
  n <- names(l)
  r <- NULL
  
  for (name in n) {
    vi <- l[[name]]$res %>% arrange(feature, segment)
    if (is.null(r)) {
      r <- data.frame(
        feature = vi$feature,
        segment = vi$segment,
      ...
      r$vi_rel <- r$vi_rel + vi$vi_rel
    }
  }
  
  r$vi_rel <- r$vi_rel / length(n)
  r
}
```


```{r varimp-np-sc, echo=FALSE, fig.width=8, fig.height=5, fig.cap="Variable importance for the no-pattern source code dataset estimated using 100 repeats and averaged over five models (Boosted Generalized Additive Model [@rpkg_gamboost], Neural network [@venables2002sstat], (Generalized Linear) Partial Least Squares [@rpkg_plsRglm], and Bagged CART [@rpkg_ipred])."}
par(mar = c(4.5,4.5,3,10))
use_vars_sc <- rev(c("A", "CP", "FREQ_nm", "A_vs_CP", "A_vs_FREQ_nm", "CP_vs_FREQ_nm")) # rev(unique(vi_np_sc$res$feature))
mat <- matrix(nrow = 6, ncol = 10, dimnames = list(use_vars_sc))
temp_vi <- merge_varimps(vi_np_sc)

for (v in rownames(mat)) {
  temp <- temp_vi[temp_vi$feature == v,] %>% group_by(segment) %>% summarise(total_imp = sum(vi_rel), .groups = "drop")
  mat[v,] <- if (nrow(temp) == 0) 0 else as.numeric(temp$total_imp)
}

mat_sums <- apply(X = mat, MARGIN = 2, FUN = sum)

pal_4 <- RColorBrewer::brewer.pal(6, "Set3")#[4:6]
bp <- barplot(height = mat, names.arg = 1:10, xlab = "Segment", ylab = "Total segment Importance", main = "No-pattern source code variable importance.", col = pal_4, ylim = c(0, max(mat_sums + .02)), angle = 30, density = 20 + 60 * mat_sums / max(mat_sums))
grid()
text(x = bp, y = .01 + mat_sums, labels = paste0(round(mat_sums, 4)), cex = .8)
legend(x = 12.5, y= .14, rev(rownames(mat)), fill = rev(pal_4), bg = "white", xpd = TRUE)
```

```{r echo=FALSE}
temp <- mat[rev(rownames(mat)),]

# Add two rows and two columns, holding the mean and sum per row/column
#temp <- cbind(temp, matrix(ncol = 1, data = as.vector(apply(X = temp[, 1:10], MARGIN = 1, FUN = mean))))
temp <- cbind(temp, matrix(ncol = 1, data = as.vector(apply(X = temp[, 1:10], MARGIN = 1, FUN = sum))))
colnames(temp) <- c(paste0("Seg", 1:10),  "sum")

temp <- rbind(temp, `rownames<-`(x = matrix(nrow = 1, data = as.vector(apply(X = temp[1:6,], MARGIN = 2, FUN = mean))), value = "mean"))
temp <- rbind(temp, `rownames<-`(x = matrix(nrow = 1, data = as.vector(apply(X = temp[1:6,], MARGIN = 2, FUN = sum))), value = "sum"))

temp[7:8, 11] <- NA_real_
temp[8, 11] <- sum(c(temp[8, 1:10], temp[1:6, 11]))
...
if (interactive()) {
  ...
  knitr::kable(
    x = round(100*temp, 2),
    booktabs = TRUE,
    caption = "Detailed variable importance (scaled to percent) for the no-pattern source code dataset, including means and sums across segments and features.",
    label = "varimp-np-sc-detailed"
  )
}
```

Table \ref{tab:varimp-np-sc-detailed} shows the variable importance from figure \ref{fig:varimp-np-sc}, including summaries for average and total importance across segments and features.


```{r varimp-np-it, echo=FALSE, fig.width=8, fig.height=5, fig.cap="Variable importance for the no-pattern issue-tracking dataset estimated using 100 repeats and averaged over the same five models as used for source code."}
par(mar = c(4.5,4.5,3,10))
use_vars_it <- rev(c("REQ", "DEV", "DESC", "REQ_vs_DEV", "REQ_vs_DESC", "DEV_vs_DESC"))
mat <- matrix(nrow = 6, ncol = 10, dimnames = list(use_vars_it))
temp_vi <- merge_varimps(vi_np_it)
...
pal_4 <- RColorBrewer::brewer.pal(6, "Set3")#[4:6]
bp <- barplot(height = mat, names.arg = 1:10, xlab = "Segment", ylab = "Total segment Importance", main = "No-pattern issue-tracking variable importance.", col = pal_4, ylim = c(0, max(mat_sums + .02)), angle = 30, density = 20 + 60 * mat_sums / max(mat_sums))
grid()
text(x = bp, y = .01 + mat_sums, labels = paste0(round(mat_sums, 4)), cex = .8)
legend(x = 12.5, y= .14, rev(rownames(mat)), fill = rev(pal_4), bg = "white", xpd = TRUE)
```
...
if (interactive()) {
  ...
  knitr::kable(
    x = round(100*temp, 2),
    booktabs = TRUE,
    caption = "Detailed variable importance (scaled to percent) for the no-pattern issue-tracking dataset, including means and sums across segments and features.",
    label = "varimp-np-it-detailed"
  )
}
```


Figure \ref{fig:varimp-np-it} and table \ref{tab:varimp-np-it-detailed} show the same thing, only for issue-tracking data.


```{r echo=FALSE}
temp_vi_sc <- merge_varimps(vi_np_sc)
temp_vi_it <- merge_varimps(vi_np_it)

temp_vi_sc$segment <- factor(x = temp_vi_sc$segment, levels = 1:10, ordered = TRUE)
temp_vi_it$segment <- factor(x = temp_vi_it$segment, levels = 1:10, ordered = TRUE)

temp_vi_sc$dtype <- "SC"
temp_vi_it$dtype <- "IT"

temp_vi <- rbind(temp_vi_sc, temp_vi_it)
temp_vi$feature <- factor(x = temp_vi$feature, levels = c(rev(use_vars_sc), rev(use_vars_it)))
temp_vi$dtype <- factor(x = temp_vi$dtype, levels = c("SC", "IT"), ordered = TRUE)
temp_vi$is_versus <- sapply(X = temp_vi$feature, FUN = function(f) grepl(pattern = "_vs_", x = f))
```

```{r echo=FALSE, eval=interactive(), fig.width=10, fig.height=4}
makefig <- function() {
  rlang::expr({
    ggplot(temp_vi, aes(x = dtype, y = vi_rel, fill = feature)) +
      ylim(0, .18) +
      geom_bar(stat = 'identity', position = 'stack') + facet_wrap(~ segment, ncol = 10, strip.position = "bottom") +
      geom_text(size = 0.36 * 8, vjust = .3, hjust = -.25, angle = 90, stat = "summary", fun = sum, mapping = aes(label = paste0("$", round(after_stat(y), 3), "$"), group = dtype)) +
      gg2_style(FALSE) +
      theme(axis.text.x = element_blank()) +
      xlab("Segment") + ylab("Relative Importance") + labs(fill = "Activity", title = "Variable importances for source code and issue-tracking, per segment.") +
      scale_x_discrete(position = "bottom") +
      scale_fill_manual(#"#FFFFB3", "#F589F5"
        values = c("#FF4A36", "#9BC94C", "#BC80BD", "#FCCDE5", "#BEBADA", "#DE657A", "#D58932", "#6F99B9", "#E6CF00", "#D9D9D9", "#8DD3C7", "#ACA585"),
        labels = sapply(X = levels(temp_vi$feature), FUN = function(l) {
          l <- gsub(pattern = "_vs_", replacement = " vs. ", x = l)
          #l <- gsub(pattern = "_", replacement = "\\\\_", x = l)
          search_for <- c("A", "CP", "FREQ_nm", "(?<!F)REQ", "DEV", "DESC")
          replace_with <- c("A", "CP", "FREQ_{nm}", "REQ", "DEV", "DESC")
          for (idx in 1:length(search_for)) {
            l <- gsub(pattern = search_for[idx], replacement = paste0("$\\\\mathsf{", replace_with[idx], "}$"), x = l, perl = TRUE)
          }
          l
        })) +
      theme(legend.title = element_text(size = 9, margin = ggplot2::margin(r = 10)),
            ...
            axis.line.x = element_line(colour = "#aaaaaa"),
            axis.text.y = element_text(size = 9),
            strip.background = element_blank(),
            strip.text = element_text(size = 9),
...
```{r echo=FALSE, eval=FALSE}
tikzDevice::tikz(file = paste0("../figures/fig_", "rr_var-imp-both", ".tex"), width = FIG_WIDTH, height = 3.6, verbose = FALSE)
eval(makefig())
dev.off()
```

```{r echo=FALSE, eval=FALSE}
temp_vi %>% group_by(is_versus, dtype) %>% summarize(Total = sum(vi_rel), .groups = "drop") %>% arrange(dtype)
```

```{r echo=FALSE, eval=FALSE}
temp_vi %>% group_by(feature) %>% summarize(Total = sum(vi_rel), .groups = "drop") %>% arrange(-Total)
```

```{r echo=FALSE, eval=FALSE}
temp_vi %>% group_by(dtype, is_versus, segment) %>% summarize(Total = sum(vi_rel), .groups = "drop") %>% arrange(-segment)
```



In figure \ref{fig:varimp-np-sc} it becomes evident that the divergence features are slightly more important than the amount features (with $\approx$ `r round(100 * sum(temp_vi[temp_vi$dtype == "SC" & temp_vi$is_versus,]$vi_rel), 2)`%), when __both__ types of features are used in a regression model.
The Fire Drill is a phenomenon that relies on a sensible balance between activities and disturbances to this balance can indicate it.
Also, the source code activities may not capture the true activities with high precision, so reliance on the divergence between activities is a feature with stronger predictive power on average.
Still, it is important _how much_ of each activity happened in each segment.

Table \ref{tab:varimp-np-sc-detailed} shows the total importance per feature.
The table shows that the divergence features are the most important, esp. `A_vs_CP`, which, in theory, should be the strongest indicator of a Fire Drill, as it can give indication to a disturbed balance between two main development activities (that should exhibit a certain balance otherwise).
However, the relative importance between features is perhaps not significantly different (i.e., there is no feature with extreme low or high relative importance).

In figure \ref{fig:varimp-np-sc} we can already see the total importance per segment.
We can see that the two most important segments are $\{3,4\}$.
The segments before and after are less important and there is almost a somewhat linearly declining trend, at least towards project end.
That could indicate that a Fire Drill may very well be detectable (predictable with high confidence) in earlier stages of a project.


## Decision Tree

Another kind of regression model is a decision tree. The goal here is to construct an approximate, yet useful tree.
Today, we have _no quantitative_ understanding of the Fire Drill and a decision tree can provide us with one such perspective.


In the following, we build a tree based on the "amount"-features only. The reason behind this is simple: these features are plainly understandable and straightforward to comprehend.
Also, we do not need to z-standardize them, which is almost required if we want to interpret the resulting tree.
...
#cn_x <- cn[cn != "gt"]
#pre_proc_method <- c("nzv", "center", "scale")
#pre_proc <- caret::preProcess(x = temp[, cn_x], method = pre_proc_method)
#temp <- stats::predict(pre_proc, newdata = temp)

#build the initial decision tree
tree <- rpart(gt ~ ., data=temp, method = "anova", control=rpart.control(minsplit = 3, cp = 1e-6))
...
The tree is shown in figure \ref{fig:dec-tree-amount-feats}. The values have been scaled to reflect percent.
The leaf nodes are the predicted Fire Drill severity and the tree can be traversed comfortably manually.
The root node has a severity of $5$, which is similar to the _ZeroR_ regression rule, in which we predict the average without further information.
If, for example, we observe that in the 3rd segment, more than $5.8$% of the total `CP`-activity have happened, then we proceed to the next node, which corresponds to a severity of $7.5$.
We can proceed with the drill down to obtain more and more accurate estimates.


```{r dec-tree-amount-feats, echo=FALSE, fig.height=7, fig.width=12, fig.cap="A decision tree using source code data and amount-type features only.", fig.align="top", fig.pos="ht!"}
#plot the pruned tree
prp(pruned_tree, type = 2, extra = 1, under = TRUE, fallen.leaves = TRUE, left = FALSE, digits = 2, tweak = 1.1, round = 0, yes.text = "Yes", no.text = "No")
```


## Weighted Mixtures from Data

We have previously estimated densities for each project and activity.
Using the ground truth, we can produce a mixture using the projects' weights.


The main purpose of this exercise is to obtain a density, per activity, that is indicative of the Fire Drill behavior for this activity.
It lets us better understand the Fire Drill in terms of these activities, since it is a weighted mixture of the observed instances.
It could also be then used as a pattern.
Like the decision tree, this is another kind of quantitative understanding of the Fire Drill.


Figure \ref{fig:weighted-mixt-sc} shows the mixtures for source code.
It appears that we have most of the adaptive activities in the 2nd half of the project, with a decline then towards the end.
Corrective and perfective activities [@swanson1976dimensions; @mockus2000change] steadily increase and peak towards the end.
My interpretation of this plot, after having seen the raters' notes, would be a late-stage rush with adding new features, followed by switch of dedication of time towards making them work as the project approaches the deadline.

In figure \ref{fig:weighted-mixt-sc} I had split `CP` into corrective and perfective again.
While the Fire Drill does not make a distinction between these two activities, we can perfectly do it again.
It appears that both activities exhibit very similar behavior: Up until $\approx75$ % of the time, they show a somewhat linear increase.
After that, they spike, and the decline only happens close to project end in both cases.
Now the spikes do not look exactly the same.
For corrective, it appears to have been carried out in three consecutive phases, each of which contained more corrective activity as the predecessing phase.
It could hint at some sort of "panic" that grew towards project end.
Perfective activities have basically only have a single bulk in that time. However, we can also discern another concentration right before it. Interestingly, that concentration ends (normalizes), and right after that, we get into highly active phases of both, corrective and perfective.
While the accumulation of perfective activities looks more homogeneous, we could perhaps still see this as what is perhaps three spikes.
The first one lasts a little longer than its corrective counter part, and the second and third seem to overlap with (perhaps accommodate) corrective activities.
...
```{r echo=FALSE, eval=FALSE, fig.width=15, fig.height=5}
par(mfrow = c(2, 3))
curve2(function(x) weighted_mixture_sc(x, "A"), 0, 1, main = "A")
grid()
curve2(function(x) weighted_mixture_sc(x, "C"), 0, 1, main = "C")
grid()
curve2(function(x) weighted_mixture_sc(x, "P"), 0, 1, main = "P")
grid()
curve2(function(x) weighted_mixture_sc(x, "CP"), 0, 1, main = "CP")
grid()
curve2(function(x) weighted_mixture_sc(x, "F"), 0, 1, main = "FREQ")
grid()
```

```{r weighted-mixt-sc, echo=FALSE, fig.height=6, fig.width=8, fig.cap="The weighted mixtures as of the source code activities. For FREQ, the non-mixture version is shown."}
curve2(function(x) weighted_mixture_sc(x, "A"), 0, 1, lwd = 1.5, col = "red", ylim = c(0,3.2), main = "Weighted mixtures for the source code activities.", xlab = "Relative project time", ylab = "Relative likelihood")
curve2(function(x) weighted_mixture_sc(x, "C"), 0, 1, lwd = 1.1, col = "blue", add = TRUE)
curve2(function(x) weighted_mixture_sc(x, "P"), 0, 1, lwd = 1.1, col = "orange", add = TRUE)
curve2(function(x) weighted_mixture_sc(x, "CP"), 0, 1, lwd = 1.75, col = "#00bb00", add = TRUE)
curve2(function(x) weighted_mixture_sc(x, "F"), 0, 1, lwd = 1.5, col = "magenta", add = TRUE)
grid()
legend(x = 0, y = 3.1, legend = c("A", "C", "P", "CP", "FREQ"), col = c("red", "blue", "orange", "#00bb00", "magenta"), bg = "white", lty = 1, lwd = 2)
```


For issue-tracking, we previously only used the CDF from oversampling directly.
Therefore, we have to redo this task and take the PDF then.
The result is shown in figure \ref{fig:weighted-mixt-it}.
Time spent on `REQ` is much more likely in the first half of a project and declines to almost zero towards project end.
`DEV` begins at almost zero, shows a slow start, and reaches a somewhat wide peak in the second half of a project. It meets its decline only close to the end of a project.
Descoping, `DESC`, almost linearly increases from project begin towards project end.
Therefore, the more time has passed, the more likely descoping is.
...
weighted_mixture_it <- (function() {
  ...
  for (pname in names(projects_it)) {
    ...
    for (activity in c("REQ", "DEV", "DESC")) {
      use_y <- as.numeric(temp[[tolower(activity)]])
      use_y[is.na(use_y)] <- 0
      templ[[activity]] <- rejection_sampling_issue_tracking(use_x = temp$`time%`, use_y = use_y)
    }
  ...
  function(x, activity = c("REQ", "DEV", "DESC")) {
    ...
    for (pidx in 1:15) {
      w <- ground_truth_all[ground_truth_all$project == paste0("project_", pidx),]$consensus / s
...
```{r echo=FALSE, eval=FALSE, fig.width=15, fig.height=4}
par(mfrow = c(1, 3))
curve2(function(x) weighted_mixture_it(x, "R"), 0, 1, main = "REQ")
grid()
curve2(function(x) weighted_mixture_it(x, "DEV"), 0, 1, main = "DEV")
grid()
curve2(function(x) weighted_mixture_it(x, "DESC"), 0, 1, main = "DESC")
grid()
```

```{r weighted-mixt-it, echo=FALSE, fig.height=5, fig.width=8, fig.cap="The weighted mixtures as of the issue-tracking activities."}
curve2(function(x) weighted_mixture_it(x, "R"), 0, 1, lwd = 1.5, col = "red", ylim = c(0,2.2), main = "Weighted mixtures for the issue-tracking activities.", xlab = "Relative project time", ylab = "Relative likelihood")
curve2(function(x) weighted_mixture_it(x, "DEV"), 0, 1, lwd = 1.5, col = "#0000bb", add = TRUE)
curve2(function(x) weighted_mixture_it(x, "DESC"), 0, 1, lwd = 1.5, col = "darkgreen", add = TRUE)
grid()
legend(x = 0, y = 2.1, legend = c("REQ", "DEV", "DESC"), col = c("red", "#0000bb", "darkgreen"), bg = "white", lty = 1, lwd = 2)
```


```{r weighted-mixtures, echo=FALSE, eval=FALSE}
# Let's do some tikz picz!
makefig <- function() {
  rlang::expr({
    par(mar = c(4.1, 3.6, 2.1, 6.3))
    # "#BEBADA", 
    use_pal <- c("#FF4A36", "#9BC94C", "#BC80BD", "#D58932", "#6F99B9", "#E6CF00")
    cex <- .9
    mline <- 2.5
    
    use_vars <- c("A", "CP", "FREQ", "REQ", "DEV", "DESC")
    
    plot(x = 0, y = 0, col = "#ffffffff", xlim = c(0, 1), ylim = c(0, 3.2), main = "Weighted mixtures for the source code and issue-tracking activities.", cex.main = cex, cex.lab = cex, cex.axis = cex, xlab = "", ylab = "", xaxt = "n", yaxt = "n")
    
    # Draw some polygons
    #use_x <- seq(0, 1, length.out = 1000)
    #for (idx in 1:6) {
    #  use_f <- if (idx > 3) weighted_mixture_it else weighted_mixture_sc
    #  polygon(x = c(use_x, rev(use_x)), y = c(rep(0, length(use_x)), use_f(x = rev(use_x), activity = use_vars[idx])), col = paste0(use_pal[idx], "88"))
    #}
    ...
    axis(2, at = seq(0, 3.5, by = .5), cex.axis = cex, las = 1)
    
    curve2(function(x) weighted_mixture_sc(x, "A"), 0, 1, lwd = 2.5, col = use_pal[1], add = TRUE)
    curve2(function(x) weighted_mixture_sc(x, "C"), 0, 1, lwd = 2.5, col = use_pal[2], add = TRUE)
    curve2(function(x) weighted_mixture_sc(x, "F"), 0, 1, lwd = 2.5, col = use_pal[3], add = TRUE)
    curve2(function(x) weighted_mixture_it(x, "R"), 0, 1, lwd = 2.5, col = use_pal[4], add = TRUE)
    curve2(function(x) weighted_mixture_it(x, "DEV"), 0, 1, lwd = 2.5, col = use_pal[5], add = TRUE)
    curve2(function(x) weighted_mixture_it(x, "DESC"), 0, 1, lwd = 2.5, col = use_pal[6], add = TRUE)
    
    
    # Draw some points
    for (idx in 1:6) {
      use_x <- seq(0.02, 0.92, by = .1) + (idx - 1) * .005
      use_f <- if (idx > 3) weighted_mixture_it else weighted_mixture_sc
      use_var <- use_vars[idx]
      points(x = use_x, y = use_f(use_x, use_var), pch = idx, col = use_pal[idx], cex = 1.1, lwd = 1.5)
    }
    
    grid()
    abline(v = seq(.1, .9, by = .2), lty = 3, lwd = .5, col = "#cccccc")
    
    legend("topright", inset = c(-.21, 0), xpd=TRUE, legend = paste0("$\\mathsf{", c("A", "CP", "FREQ_{nm}", "REQ", "DEV", "DESC"), "}$"), col = use_pal, bg = "white", lty = 1, lwd = 2.5, cex = cex, title = "Activity", title.adj = .25, bty = "n", pch = 1:6, pt.cex = 1.1)
    #legend(x = 0, y = 2.1, legend = c(), col = c("red", "#0000bb", "darkgreen"), bg = "white", lty = 1, lwd = 2)
  })
}
eval(makefig())
```
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(expr = makefig(), name = "rr_weighted-mixtures", height = 3.3)
```

```{r eval=FALSE, echo=FALSE}
get_smoothed_curve <- function(seed, npoints = 15, integral_eq_1 = TRUE) {
  if (!is.na(seed)) {
    set.seed(seed = seed)
  }
  ...
  # Make sure it's not negative, and actually slightly positive:
  temp$y <- temp$y - min(temp$y)
  temp$y <- temp$y + .1 * (max(temp$y) - min(temp$y))
  ...
  tempf <- Vectorize(appr)
  if (integral_eq_1) {
    the_int <- cubature::cubintegrate(f = tempf, lower = 0, upper = 1, maxEval = 1e4)$integral
...
jsd_segment2 <- function(p, q, ext = c(0, 0.1), xtol = 1e-20) {
  cubature::cubintegrate(f = function(x) {
    p_ <- p(x)
    q_ <- q(x)
    m_ <- .5 * (p_ + q_)
...
```{r eval=FALSE, echo=FALSE}
# Let's first do the data for the video:
wm <- function(x) weighted_mixture_sc(x, "A")
random_procs <- list()
metric_data <- c()

set.seed(1)
for (idx in 1:60) {
  #random_proc <- stats::approxfun(x = seq(0, 1, length.out = 10), y = runif(n = 10, 0, 5))
  random_proc <- get_smoothed_curve(seed = 1337 + idx)
  random_procs <- append(random_procs, random_proc)
  
  #metric_data <- c(metric_data, cubature::cubintegrate(f = function(x) {
  #  (wm(x) - random_proc(x))^2
  #}, lower = .2, upper = .4, maxEval = 1e3)$integral)
  
  #use_x <- seq(0.2, 0.4, length.out = 1e3)
  #metric_data <- c(metric_data, stats::cor(x = wm(use_x), y = random_proc(use_x)))
  metric_data <- c(metric_data, jsd_segment2(p = wm, q = random_proc, ext = c(.2, .4)))
}
```


```{r eval=FALSE, echo=FALSE}
# Here, we'll make the video
make_3way_plot <- function(l) {
  #par(mfrow = c(2, 2))
  ...
  curve2(wm, 0, 1, lwd = 2.5, col = use_pal[1], ylim = c(0,2.5), main = "Logarithmic Jensen-Shannon divergence on segment.", cex.main = 2.5, cex.axis = 2.5, cex.lab = 2.5, xlab = "", ylab = "")
  mtext(side = 1, text="Relative Project time", line = 4.5, cex = 2.5)
  mtext(side = 2, text="Relative Likelihood", line = 4.5, cex = 2.5)
  
  
  
  use_x <- c(seq(.2, .4, length.out = 20))
  use_x <- c(use_x, rev(use_x))
  # Let's first add the random processes to the first plot:
  for (idx in 1:l) {
  #for (idx in 1:length(random_procs)) {
    rp <- random_procs[[idx]]
    polygon(x = use_x, y = c(
    ...
    ), col = if (idx == l) "#add8e6f8" else "#eeeeeef8")
    curve2(func = rp, from = 0, to = 1, lwd = if (idx == l) 2.5 else 1.5, add = TRUE, col = if (idx == l) "#444444" else "#bbbbbb")
  }
  
  # Let's always over-draw the original proc:
  curve2(wm, 0, 1, lwd = 3, col = use_pal[1], ylim = c(0,3.2), add = TRUE)
  abline(v = c(.2, .4), lwd = 2)
  grid()
  
  
  use_data <- log(metric_data[1:l])
  
  # Let's draw the left plot:
  plot(density(use_data, bw = "SJ", cut = TRUE), main = "PDF", cex.main = 2.5, cex.axis = 2.5, cex.lab = 2.5, xlab = "", ylab = "")
  mtext(side = 1, text=paste0("log(JSD(p||q)); N=", l), line = 4.5, cex = 2.5)
  mtext(side = 2, text="Relative Likelihood", line = 4.5, cex = 2.5)
  grid()
  
  
  
  # Let's draw the below plot:
  temp_ecdf <- stats::ecdf(use_data)
  curve2(temp_ecdf, min(use_data), max(use_data), main = "ECDF", cex.main = 2.5, cex.axis = 2.5, cex.lab = 2.5, xlab = "", ylab = "")
  mtext(side = 1, text=paste0("log(JSD(p||q)); N=", l), line = 4.5, cex = 2.5)
  mtext(side = 2, text="F(x); cumulative probability", line = 4.5, cex = 2.5)
  grid()
}
```
...
```{r eval=FALSE, echo=FALSE}
for (l in 2:length(random_procs)) {
  png(filename = paste0("C:\\temp\\plot\\Rplot_", if (l < 10) paste0("0", l) else l, ".png"), width = 1600, height = 900)
  make_3way_plot(l)
  dev.off()
}

# ffmpeg -loglevel warning -stats -hide_banner -framerate 5 -pattern_type sequence -i "Rplot_%02d.png" -s:v 1600:900 -c:v libx264 -crf 10 -pix_fmt yuv420p -y out.mp4
```
...
Let us also try unsupervised learning: Clustering of similar projects.
The goal here is to see, whether similar projects (that is, with a similar ground truth) end up in the same clusters or are at least close to each other in a lower-dimensional space.


## 2D and 3D embeddings using t-distributed Stochastic Neighbor Embeddings

When we cluster, we must specify the number of clusters ahead of time.
t-SNE on the other hand, reduces the dimensionality of our observations [@vandermaaten08a].
If we choose to produce low-dimensional observations (i.e., 2D or 3D), then we can comfortably plot the result.
The expectation is that we can observe clusters of similar observations (here: projects with a similar ground truth).


Figure \ref{fig:tsne-first-test} shows a 2D-embedding of our $15$ projects.
It is problematic to have so few projects and so many expected clusters (ideally, we expect each severity to be a cluster, which means $11$ clusters).
Therefore, we could try to either use the oversampled data, or to group projects into severity ranges in order to reduce the amount of expected clusters.


```{r tsne-first-test, fig.cap="2-dimensional t-SNE embedding of all projects.", fig.align="top", fig.pos="ht!"}
set.seed(1)
temp <- Rtsne::Rtsne(X = dataset_np_sc[,], dims = 2, perplexity = 3, pca = TRUE, check_duplicates = FALSE, theta = 0)

plot(x = temp$Y[,1], y = temp$Y[,2], col = rgb(red = 0, green = 0, blue = 0, alpha = 0), xlab = "", ylab = "", xlim = c(min(temp$Y[,1]) - 10, max(temp$Y[,1]) + 10), ylim = c(min(temp$Y[,2]) - 15, max(temp$Y[,2]) + 5))
text(x = temp$Y[,1], y = temp$Y[,2], labels = paste0("Proj_", 1:15))
points(x = temp$Y[,1], y = temp$Y[,2] - 20, pch = 15, cex= 1.3, col = "darkred")
grid()
```
...
#pal <- viridis::viridis(11)
#pal <- RColorBrewer::brewer.pal(n = 11, name = "Set3")
#pal <- viridis::inferno(n = 11, begin = 0, end = .95)
#pal <- ggsci::pal_d3(palette = "category20")(n=11)
#pal <- ggsci::pal_igv()(n = 11)
#pal <- ggsci::pal_rickandmorty()(n = 11)
#pal <- ggsci::pal_simpsons()(n = 11)
#pal <- ggsci::pal_ucscgb()(n = 11)
pal <- c(
  ggsci::pal_material(palette = "green", n = 3)(n = 3)[2],
  ggsci::pal_material(palette = "blue", n = 7)(n = 7)[c(2, 4, 6)],
  ggsci::pal_material(palette = "purple", n = 9)(n = 9)[c(2, 4, 6, 8)],
  ggsci::pal_material(palette = "orange", n = 6)(n = 6)[c(2, 4, 6)])

tsne_2d <- Rtsne::Rtsne(
  X = temp[, "gt" != colnames(temp)], dims = 2, perplexity = 30, pca = TRUE, check_duplicates = FALSE, theta = 0.05, max_iter = 2e3)
tsne_3d <- Rtsne::Rtsne(
  X = temp[, "gt" != colnames(temp)], dims = 3, perplexity = 30, pca = TRUE, check_duplicates = FALSE, theta = 0.05, max_iter = 2e3)
```

```{r tsne-dual-embedding, echo=FALSE, fig.width=10, fig.height=5, fig.cap="A 2- and 3-dimensional embedding of the oversampled projects (using z-standardized source code features).", fig.align="top", fig.pos="ht!"}
par(mfrow = c(1,2))

plot3D::scatter2D(
  x = tsne_2d$Y[,1], y = tsne_2d$Y[,2], colvar = .5 + dataset$gt, col = pal, pch = 16, clab = "Severity",
  xlab = "", ylab = "", bty = "b2",
  colkey = FALSE) # colkey=list("dist" = 0.06, "length" = .66, "width" = .66, "cex.clab" = 0.8, at = 0:11, labels = paste0(-1:10))


par(mar = c(2,1,2,2))
plot3D::scatter3D(
  x = tsne_3d$Y[,1], y = tsne_3d$Y[,2], z = tsne_3d$Y[,3], colvar = .5 + dataset$gt, col = pal, phi = 22, theta = -42, r = 2,
  ticktype = "detailed", nticks = 5, pch = 16, cex.p = 1.3, clab = "Severity", xlab = "", ylab = "", zlab = "",
  colkey = list("dist" = 0.04, "length" = .66, "width" = .5, "cex.clab" = 0.8, at = 0:11, labels = paste0(-1:10)), bty = "b2")
```

Figure \ref{fig:tsne-dual-embedding} shows two embeddings. I have asked my colleague Angelos to interpret these, without telling him any details about the underlying dataset.

He kindly responded:

_Starting with the 2D projection, Observation 1 (O1) is about the distortion in the vertical space. I believe if there had been more space to project your points, I would have used a square (or as square as possible) grid. Following that, many points are overlapping in the 2D embedding and require interactivity in order to see them in comparison to 3D, which was clear in that case._

_O2 is related to groups of points forming a kind of circular motive. I think this happens when the data instances/records are quite similar or even identical._

_O3 is the most important observation because it seems that as you reduce the severity, there is a linear (or time-dependent) connection between your points. For example, the orange points are between dark red and light orange. The same is true for the purple points, but this is more clearly visible from the 3D projection. I am not sure why though; it could be that the 3D is "cheating here" due to the perspective from which you have taken the screenshot. I would definitely need to interact with both charts to understand more about what is going on._

__Comment Sebastian__: That linear trend appears to correspond to the linear Likert-scale used to assign the severity.

_The same goes for O4, where 2D is split into three areas while 3D shows two. Also, the overplotting issue is still evident here, especially for the 2D embedding. If I had no colors, I would argue that almost nothing is easy to observe from the plots except for the very distant and tight clusters. The remaining points in the middle would have been considered one cluster, in my opinion._

_Something deceiving in the 3D plot is marked as O2, with the green points appearing closer to the dark red compared to the 2D embedding. I guess it is because of the position of the "camera," so interaction is key here._


The second observation follows from the synthetic oversampling of the data, which will result in very similar data points.
I actually had to disable checking for duplicates when computing the t-SNE on the oversampled datasets for it to work.


So, what we could try, is to reduce the amount of expected clusters and use the original, non-oversampled data.
Figure \ref{fig:tsne-severity-groups} shows the result of this. There I have reduced the projects' cluster to two, similar to how the decision rule does it (ground truth less than $5$ means "no Fire Drill").
Still, this plot is not meaningful and we cannot observe any useful clusters.
The reason could quite well be that we do not have sufficient data. It could also very well be that the projects' underlying data does not reflect the ground truth all too well.
Further investigation and/or more data is needed.

```{r tsne-severity-groups, echo=FALSE, fig.cap="2D t-SNE embedding of the projects using source code data. The expected clusters have been reduced to two.", fig.align="top", fig.pos="ht!"}
set.seed(2)

dataset <- cbind(dataset_np_sc[,], data.frame(gt = ground_truth_all$consensus))
# Let's make groups of ground truth:
dataset$gt <- sapply(X = dataset$gt, FUN = function(gt) {
  #if (gt < 4) 1 else if (gt < 7) 2 else 3
  if (gt < 5) 0 else 1
})
...
pal <- c(
  #ggsci::pal_material(palette = "green", n = 3)(n = 3)[2],
  ggsci::pal_material(palette = "blue", n = 3)(n = 3)[2],
  ggsci::pal_material(palette = "orange", n = 3)(n = 3)[2])

tsne_grouped_2d <- Rtsne::Rtsne(
  X = temp[, "gt" != colnames(temp)], dims = 3, perplexity = 4, pca = FALSE, check_duplicates = TRUE, theta = 0.05, max_iter = 5e3)
plot3D::scatter2D(
  x = tsne_grouped_2d$Y[,1], y = tsne_grouped_2d$Y[,2], colvar = .5 + dataset$gt, col = pal, pch = 16, clab = "Severity",
  xlab = "", ylab = "", bty = "b2",
  colkey = list("dist" = 0.015, "length" = .66, "width" = .66, "cex.clab" = 0.8, at = c(.75, 1.25), labels = c("[0,5)", "[5,10]")))
```


## k-Means Clustering

Here we use k-Means clustering of the projects using three clusters (severity $<4$, severity $<7$, and severity $>7$),
In figure \ref{fig:kmeans-embedding}, the $x/y$-coordinates are obtained from the first two principal components to produce an embedding.
With a circle, the k-Means-assigned cluster for each project is shown. Next to each circle, the true cluster (using a triangle) is plotted.
Only four out of $15$ projects are assigned a correct cluster, three are in the wrong, directly neighboring cluster (e.g., assigned to severity cluster $2$ instead of $1$, or $1$ instead of $0$), and eight are in the most distant cluster (i.e., $0$ instead of $2$ or vice versa).


```{r kmeans-embedding, echo=FALSE, fig.cap="k-Means clustering of projects using 3 clusters and the first two principal components.", fig.align="top", fig.pos="ht!"}
set.seed(1)

dataset <- cbind(dataset_np_sc[,], data.frame(gt = ground_truth_all$consensus))
# Let's make groups of ground truth:
dataset$gt <- sapply(X = dataset$gt, FUN = function(gt) {
  if (gt < 4) 0 else if (gt < 7) 1 else 2
  #if (gt < 5) 0 else 1
})
...
temp.km <- stats::kmeans(x = temp[, "gt" != colnames(temp)], centers = 3, iter.max = 1000, nstart = 100)

temp.pca <- stats::prcomp(x = temp[, "gt" != colnames(temp)], center = FALSE, scale. = FALSE)
...
plot3D::scatter2D(
  x = temp.pca$x[,1], y = temp.pca$x[,2], colvar = temp.km$cluster - min(temp.km$cluster), col = pal, pch = 16, clab = "Severity", cex = 1.8,
  xlab = "", ylab = "", bty = "b2",
  colkey = list("dist" = 0.02, "length" = .66, "width" = .66, "cex.clab" = 0.8, at = c(1/3, 1, 5/3), labels = paste0(1:3)))
plot3D::points2D(
  x = temp.pca$x[,1] + .5, y = temp.pca$x[,2] + .2, pch = 17, cex = 1.2, colvar = temp$gt, col = pal, colkey = FALSE, add = TRUE)

legend(x = 4, y = 6.5, pch = 1:2, legend = c("k-Means cluster", "True cluster"), cex = 1.2, bg = "white")
```
