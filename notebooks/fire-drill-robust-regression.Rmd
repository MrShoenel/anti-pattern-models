...
* Compose a mixture density for `FREQ` as $f(x)=r_A f_A(x) + r_{CP} f_{CP}(x)$, where $r_a,r_{CP}$ correspond to the ratios of these activities.
  * We will also compose an ordinary density called `FREQ_nm` (no-mixture) that just captures all commit when they happened. This is going to be used later in the no-pattern approach.
...
adaptive_training_caret <- function(
  ...
  for (seed in seeds) {
    ...
    res <- rbind(res, data.frame(
      ...
      sd_valid = stats::sd(x = df_train$gt - pred_valid)
    ))
  }
...
Also, here we will be using the no-mixture version of the `FREQ` activity. The mixture as used previously, simply is a linearly weighted convex combination of the `A`- and `CP`-densities.
In the following, when attempting to adaptively train a regression model and learning from the features, we expect using a no-mixture version will increase the entropy.
...
```{r weighted-mixt-sc, echo=FALSE, fig.height=5, fig.width=8, fig.cap="The weighted mixtures as of the source code activities. For FREQ, the no-mixture version is shown."}
