...
grid <- expand.grid(list(
  ...
  num_train = 2:50,
  do_pca = c(TRUE, FALSE),
  model_type = c("avNNet", "glm", "M5", "rf", "svmPoly", "treebag", "xgbTree")
))
...
res_grid <- loadResultsOrCompute(file = "../results/rob-reg_res-grid.rds", computeExpr = {
  doWithParallelCluster(numCores = min(parallel::detectCores(), 123), expr = {
    res <- NULL
    for (model_type in c("avNNet", "glm", "M5", "rf", "svmPoly", "treebag", "xgbTree")) {
      temp <- foreach::foreach(
        ...
        tryCatch({
          ...
          adaptive_training_caret(
            org_dataset = dataset_sc_oversampled, seeds = row$seed, model_type = model_type,
            num_train = row$num_train, do_pca = row$do_pca)
        }, error = function(cond) {
          NULL # Return an empty result that will not disturb .combine
        })
      }
      saveRDS(object = temp, file = paste0("../results/rob-reg_res-grid_", model_type, ".rds"))
      res <- rbind(res, temp)
    }
...
Let's show some results for the Random forest:

```{r}
temp <- res_grid[res_grid$model_type == "rf",]
temp <- temp %>% group_by(num_train) %>% summarize(Mean = mean(rmse_valid), Max = max(rmse_valid), Min = min(min(rmse_valid)), .groups = "drop")
plot(temp$Mean, type="l", xlab = "Number of training instances", ylab = "RMSE (validation data)", ylim = c(0, max(c(temp$Mean, temp$Max, temp$Min))))
grid()
lines(temp$Max, col="blue")
lines(temp$Min, col="red")
legend(x = 40, y = 4.5, legend = c("mean", "max", "min"), col = c("black", "blue", "red"), lwd = 2)
```
