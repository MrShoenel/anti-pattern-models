...
grid <- expand.grid(list(
  interval = 1:10,
  activity = c("A", "CP", "FREQ")#, "SCD") # Let's skip SCD and only use the activities for now
))
grid$activity <- as.character(grid$activity)
...
for (pname in names(projects_sc)) {
  ...
  for (rn in rownames(grid)) {
    ...
    if (row$activity == "SCD") {
      # Average SCD over the interval's extension
      use_x <- seq(from = interval_ext[1], to = interval_ext[2], length.out = 1e3)
      newrow_sc[1, feat_name] <- mean(projects_sc[[pname]][[row$activity]](use_x))
    } else {
...
library(foreach)
res_grid_np <- loadResultsOrCompute(file = "../results/rob-reg-np_res-grid.rds", computeExpr = {
  res <- NULL
  ...
  for (use_dataset in use_datasets) {
    ...
    for (model_type in model_types) {
      ...
      the_file <- paste0("../results/rob-reg-np_res-grid_", model_type, "_", use_dataset, ".rds")
      temp <- as.data.frame(loadResultsOrCompute(file = the_file, computeExpr = {
        temp1 <- doWithParallelCluster(numCores = min(parallel::detectCores(), 123), expr = {
          foreach::foreach(
            rn = rownames(grid),
