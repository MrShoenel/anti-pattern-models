...
if (interactive()) {
  ...
  knitr::kable(
    x = res,
    booktabs = TRUE,
    caption = paste0("Four different inequalities giving and their confidence for a given deviation (for the best neural network model trained on $17$ instances). The Three Sigma rule has the tightest bounds, while Gauss' inequality has the loosest. The mean of the data is $", round(mean(temp$rmse_valid), 3), "$ and the standard deviation is $", round(sd(temp$rmse_valid), 3), "$."),
...
makefig <- function(paper = FALSE) {
  rlang::expr({
    ...
    lines(col = "#fc7703", lwd = 2, x = min(temp$num_train):max(temp$num_train), y =
    ...
    legend(x = 29, y = 2.9, legend = c("Mean RMSE (validation)", paste0(use_leq(1), " standard deviation"), paste0(use_leq(2), " std. devs."), paste0(use_leq(3), " std. devs.")), col = c("#fc7703", "#000000", "#333333", "#666666"), lwd = 2, lty = c(1, 1, 2, 4), bg = "white", cex = cex, text.width = 15)
  })
}
...
use_vars <- rev(c("A", "CP", "FREQ_nm", "A_vs_CP", "A_vs_FREQ_nm", "CP_vs_FREQ_nm")) # rev(unique(vi_np_sc$res$feature))
mat <- matrix(nrow = 6, ncol = 10, dimnames = list(use_vars))
...
temp_vi$feature <- factor(x = temp_vi$feature, levels = c(rev(use_vars), c("REQ", "DEV", "DESC", "REQ_vs_DEV", "REQ_vs_DESC", "DEV_vs_DESC")))
...
makefig <- function() {
  plotexpr <- rlang::expr({
    ggplot(temp_vi, aes(x = dtype, y = vi_rel, fill = feature)) +
      ...
      geom_text(size = 0.36 * 8, vjust = .3, hjust = -.25, angle = 90, stat = "summary", fun = sum, mapping = aes(label = round(after_stat(y), 3), group = dtype)) +
      gg2_style(FALSE) +
      ...
      scale_fill_manual(
        values = (function(n) {
          set.seed(3)
          sample(x = RColorBrewer::brewer.pal(n, "Set3"), size = n)
        })(12),
        labels = sapply(X = levels(temp_vi$feature), FUN = function(l) {
          ...
          for (vn in c("A", "CP", "FREQ_nm", "REQ", "DEV", "DESC")) {
            l <- gsub(pattern = vn, replacement = paste0("$\\\\mathsf{", vn, "}$"), x = l)
          }
          l
        })) +
      theme(legend.title = element_text(size = 9, margin = ggplot2::margin(r = 10)),
            ...
            plot.title = element_text(size = 9, face = "bold", hjust = 0.5, margin = ggplot2::margin(b = 2.5, unit = "line"))) +
...
```{r echo=FALSE, eval=FALSE}
tikzDevice::tikz(file = paste0("../figures/fig_", "rr_var-imp-both", ".tex"), width = FIG_WIDTH, height = 4, verbose = FALSE)
eval(makefig())
dev.off()
```
...
Table \ref{tab:varimp-np-sc-per-feature} shows the total importance per feature.
...
```{r echo=FALSE}
temp <- temp_vi[temp_vi$dtype == "SC",] %>% group_by(feature) %>% summarize(Sum = sum(vi_rel), .groups = "drop") %>% arrange(-Sum)
...
My interpretation of this plot, after having seen the raters' notes, is a late-stage rush with adding new features, followed by switch of dedication of time towards making them work as the project approaches the deadline.


```{r}
weighted_mixture_sc <- function(x, activity = c("A", "CP", "FREQ_nm")) {
...
grid()
curve2(function(x) weighted_mixture_sc(x, "C"), 0, 1, main = "CP")
grid()
curve2(function(x) weighted_mixture_sc(x, "F"), 0, 1, main = "FREQ")
grid()
```

```{r weighted-mixt-sc, echo=FALSE, fig.height=5, fig.width=8, fig.cap="The weighted mixtures as of the source code activities. For FREQ, the non-mixture version is shown."}
...
curve2(function(x) weighted_mixture_sc(x, "C"), 0, 1, lwd = 1.5, col = "#00bb00", add = TRUE)
curve2(function(x) weighted_mixture_sc(x, "F"), 0, 1, lwd = 1.5, col = "magenta", add = TRUE)
grid()
legend(x = 0, y = 3.1, legend = c("A", "CP", "FREQ"), col = c("red", "#00bb00", "magenta"), bg = "white", lty = 1, lwd = 2)
```
