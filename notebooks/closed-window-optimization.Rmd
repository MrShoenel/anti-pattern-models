...
Consider our Fire Drill example: The Warping Pattern has four variables and four intervals. If we want to fit a single project to it, we may want to define a single model and weight per variable and interval, which would result in already 16 objectives here. I wrote that SRBTW is implicitly capable of handling multiple variables and/or dataseries. Implicit because this complexity is reduced to having one SRBTW handle exactly two signals, the Warping Pattern and -Candidate, and then defining a loss per each. It is perfectly fine to reuse an SRBTW model across intervals of the same variable, but in practice the overhead of doing so may be reduced by having each objective use its own instance. We provide a __linear scalarizer__ to handle one or more objectives, each with their own weight.

$$
\begin{aligned}
  \mathcal{O}(\bm{\vartheta}^{(l)},b,e)=&\;w_1\mathcal{L}_1+\;\dots\;+w_n\mathcal{L}_n\;\text{, (linear scalarization)}
  \\[1ex]
  =&\;\mathbf{w}^\top\bm{\mathcal{L}}\;\text{, with gradient (for any parameter in}\;\mathcal{L}_i\text{)}
  \\[1ex]
  \nabla\,\mathcal{O}=&\;w_1\mathcal{L}'_1+\;\dots\;+w_n\mathcal{L}'_n\;\text{.}
\end{aligned}
$$
...
cow_og2_test1p_plots <- lapply(cow_og2_test1p, function(u) {
  ...
  s$setParams(vartheta_l = u$r$srbtw$getVarthetaL())
...
All of the above examples that were closed/closed resulted in perfect fits (within margin of error), using the RSS loss. The half-open (begin) was OK, although it cut off the ascend in the beginning. The half-open (end) cut off major parts of the signal. Note that in both cases, before the begin or after the end, the WP was mapped to $0$, resulting in (visual) padding. In both cases, the remainder of the WC was mapped well, however. The fully-open examples seem to have gotten stuck even more in local optima and are probably victims to the absent regularization. Like the half-open tests, the non-cut-off parts are matched very well, but there was no mechanism to tell the optimization to try and include more of the signal.

In all of the above cases, I used the discrete RSS data-loss, which itself uses a constant number of samples of $250$ per interval. So if we divide the loss by the number of intervals, we can actually make apples-to-apples comparisons.
...
ggplot() +
  xlim(0,1) +
  stat_function(fun = function(x) r(x) + 1) +
  stat_function(fun = f) +
  geom_path(data = data.frame(x = c(0, .5), y = c(.1, .7)), mapping = aes(x=x, y=y), color="blue", linetype = "dashed")
```
