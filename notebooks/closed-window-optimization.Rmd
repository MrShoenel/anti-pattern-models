...
vartheta_y_mlm <- runif(n = length(vartheta_l_mlm), min = sqrt(.Machine$double.eps), max = .05)
...
loss_mlm <- srBTAW_Loss_Rss$new(
  wpName = sig_wp_mlm$getName(), wcName = sig_wc_mlm$getName(),
  weight = 1, intervals = 1:4, continuous = TRUE)#, numSamples = rep(250, 4))

mlm$addLoss(loss = loss_mlm)
mlm$setObjective(obj = loss_mlm)
```
...
```{r}
mlm_fr <- mlm$fit(verbose = TRUE)
```

```{r}
plot(mlm_fr$getFitHist()[, "loss"], type="l")
mlm$.__enclos_env__$private$instances$`A_WP|A_WC`$plot_warp()
```
