...
\begin{aligned}
  ...
  \\[1ex]
  &\times\bigg(x-\beta_l-\overbrace{\Big(\max{(\bm{\lambda}_1,l_1)}+\;\dots\;+\max{(\bm{\lambda}_{q-1},l_{q-1})}\Big)}^{\phi_q\;\text{(sum of all preceding intervals' lengths;}\;0\;\text{if}\;q=1\text{)}}\bigg)
  \\[1ex]
  &\times\overbrace{(\beta_u-\beta_l)\times\max{\Big(\theta_d,\;\max{(\bm{\lambda}_1,l_1)}+\;\dots\;+\max{(\bm{\lambda}_{\max{(Q)}},l_{\max{(Q)}})}\Big)}}^{\phi\;\text{(sum of all intervals' minimum lengths)}}
  \\[1ex]
