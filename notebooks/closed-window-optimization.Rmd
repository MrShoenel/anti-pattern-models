...
$$
\begin{aligned}
  \frac{\partial\,\max{(\bm{\lambda_q},l_q)^{-1}}}{\partial\,l_q}=\frac{\mathcal{H}(l_q-\bm{\lambda}_q)}{(\mathcal{R}(l_q-\bm{\lambda}_q)+\bm{\lambda}_q)^2}
\end{aligned}
$$
...
\begin{aligned}
  ...
  \\[1ex]
  f\Big(s_b+\Big[\;\dots\;\times&\Bigg(\overbrace{\bigg(\beta_u\times\max{(\bm{\lambda_q},l_q)}^{-1}\times\max{(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)})}\bigg)}^{j}
  \\[0ex]
  &\;\;\;\;-\overbrace{\bigg(\beta_l\times\max{(\bm{\lambda_q},l_q)}^{-1}\times\max{(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)})}\bigg)}^{k}\Bigg)
\end{aligned}
$$
...
The final expression for our model is now much more convenient to work with.
