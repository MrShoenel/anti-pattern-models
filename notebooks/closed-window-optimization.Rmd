...
\begin{aligned}
  ...
  \\[1ex]
  &\;\text{where}\;\theta_d\geq 0\;\land\;\theta_b+\theta_d\leq\theta_e\;\text{,}
  \\[1ex]
  \beta_l=&\;\min{\Big(\theta_e-\theta_d,\max{\big(\theta_b, \min{(b,e)}\big)}\Big)}\;\text{, lower boundary as of}\;b,e\text{,}
  \\[1ex]
  \beta_u=&\;\max{\Big(\theta_b+\theta_d,\min{\big(\theta_e, \max{(b,e)}\big)}\Big)}\;\text{, upper boundary as of}\;b,e\text{,}
  \\[1em]
  l_q\dots&\;\text{the length of the }q\text{-th interval,}
  \\[1ex]
  t_b=\phi_q\equiv&\;\begin{cases}
    0,&\text{if}\;q=1,
    \\
...
$$
\begin{aligned}
  m_q^c(\dots)=f\Bigg(s_b\;+\;&\bigg(\delta_s
  \\[1ex]
  &\times\bigg(x-\overbrace{\min{\Big(\theta_e-\theta_d,\max{\big(\theta_b, \min{(b,e)}\big)}\Big)}}^{\beta_l}-\overbrace{\Big(\max{(\bm{\lambda}_1,l_1)}+\;\dots\;+\max{(\bm{\lambda}_{q-1},l_{q-1})}\Big)}^{\phi_q\;\text{(sum of all preceding intervals' lengths;}\;0\;\text{if}\;q=1\text{)}}\bigg)
  \\[1ex]
  &\times\overbrace{\Bigg(\overbrace{\max{\Big(\theta_b+\theta_d,\min{\big(\theta_e, \max{(b,e)}\big)}\Big)}}^{\beta_u}-\overbrace{\min{\Big(\theta_e-\theta_d,\max{\big(\theta_b, \min{(b,e)}\big)}\Big)}}^{\beta_l}\Bigg)}^{\phi^{(e)}\;\text{(extent of}\;\phi\text{)}}
  \\[1ex]
  &\times\overbrace{\max{\Big(\theta_d,\;\max{(\bm{\lambda}_1,l_1)}+\;\dots\;+\max{(\bm{\lambda}_{\max{(Q)}},l_{\max{(Q)}})}\Big)}}^{\phi^{(s)}\;\text{(scale of}\;\phi\text{; sum of all intervals' minimum lengths)}}
  \\[1ex]
...
$$
\begin{aligned}
  \beta_l=&\;\overbrace{\min{\Big(\theta_e-\theta_d,\overbrace{\max{\big(\theta_b, \overbrace{\min{(b,e)}}^{s_2=e-\mathcal{R}(e-b)}\big)}\Big)}^{s_1=\mathcal{R}(s_2-\theta_b)+\theta_b}}}^{\beta_l=s_1-\mathcal{R}(s_1-(\theta_e-\theta_d))}\;\text{, rearranged as}
  \\[1ex]
  =&\;\overbrace{\mathcal{R}(\overbrace{e-\mathcal{R}(e-b)}^{s_2}-\theta_b)+\theta_b}^{s_1}-\mathcal{R}(\overbrace{\mathcal{R}(\overbrace{e-\mathcal{R}(e-b)}^{s_2}-\theta_b)+\theta_b}^{s_1}-(\theta_e-\theta_d))\;\text{, with gradient}
  \\[1ex]
  \bigg[\frac{\partial\,\beta_l}{\partial\,b},\frac{\partial\,\beta_l}{\partial\,e}\bigg]=&\bigg[\;-\mathcal{H}(e-b)\times\mathcal{H}\big(-\mathcal{R}(e-b)-\theta_b+e\big)\times\Big(\mathcal{H}\big(\mathcal{R}(-\mathcal{R}(e-b)-\theta_b+e)+\theta_b-\theta_e-\theta_d\big)-1\Big),
  \\[0ex]
  &\;\;\;\;\Big(\mathcal{H}\big(-\mathcal{R}(e-b)-\theta_b+e\big)-1\Big)\times\Big(e\times\mathcal{H}\Big(\mathcal{R}\big(-\mathcal{R}(e-b)-\theta_b+e\big)+\theta_b-\theta_e+\theta_d\Big)-1\Big)\;\bigg]\;\text{.}
\end{aligned}
$$
...
$$
\begin{aligned}
  \beta_u=&\;\overbrace{\max{\Big(\theta_b+\theta_d,\overbrace{\min{\big(\theta_e,\overbrace{\max{(b,e)}\big)}^{s_2=\mathcal{R}(e-b)+b}}\Big)}^{s_1=s_2-\mathcal{R}(s_2-\theta_e)}}}^{\beta_u=\mathcal{R}(s_1-(\theta_b+\theta_d))+(\theta_b+\theta_d)}\;\text{, rearranged as}
  \\[1ex]
  =&\;\mathcal{R}(\overbrace{\overbrace{\mathcal{R}(e-b)+b}^{s_2}-\mathcal{R}(\overbrace{\mathcal{R}(e-b)+b}^{s_2}-\theta_e)}^{s_1}-(\theta_b+\theta_d))+(\theta_b+\theta_d)\;\text{, with gradient}
  \\[1ex]
  \bigg[\frac{\partial\,\beta_u}{\partial\,b},\frac{\partial\,\beta_u}{\partial\,e}\bigg]=&\bigg[\;\big(\mathcal{H}(e-b)-1\big)\times\Big(\mathcal{H}\big(\mathcal{R}(e-b)+b-\theta_e\big)-1\Big)
  \\[0ex]
  &\;\;\;\;\;\;\;\;\times\mathcal{H}\Big(-\mathcal{R}\big(\mathcal{R}(e-b)+b-\theta_e\big)+\mathcal{R}(e-b)+b-\theta_b-\theta_d\Big),
  \\[0ex]
  &\;\;\;\;e-\mathcal{H}(-\mathcal{R}(\mathcal{R}(e-b)+b-\theta_e)+\mathcal{R}(e-b)+b-\theta_b-\theta_d)\;\bigg]\;\text{.}
\end{aligned}
$$
...
Recall the definition of $\phi=(\beta_u-\beta_l)\times\max{\Bigg(\theta_d,\Bigg[\sum_{j=1}^{\max{(Q)}}\,\max{(\bm{\lambda}_j,l_j)}\Bigg]\Bigg)}$.
...
$$
\begin{aligned}
  f\Big(s_b+\Big[\;\dots\;\times&\overbrace{\phi\times\max{(\bm{\lambda_q},l_q)}^{-1}}^{\text{(will become}\;[j-k]\text{ after expansion)}}\;\Big]\Big)
  \\[1ex]
  f\Big(s_b+\Big[\;\dots\;\times&\Big(\beta_u\times\max{(\bm{\lambda_q},l_q)}^{-1}-\beta_l\times\max{(\bm{\lambda_q},l_q)}^{-1}\Big)\times\max{(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)})}
  \\[1ex]
  f\Big(s_b+\Big[\;\dots\;\times&\Bigg(\overbrace{\bigg(\beta_u\times\max{(\bm{\lambda_q},l_q)}^{-1}\times\overbrace{\max{(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)})}\bigg)}^{\phi^{(s)}}}^{j}
  \\[0ex]
  &\;\;\;\;-\overbrace{\bigg(\beta_l\times\max{(\bm{\lambda_q},l_q)}^{-1}\times\overbrace{\max{(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)})}\bigg)}^{\phi^{(s)}}}^{k}\Bigg)
\end{aligned}
$$
...
$$
\begin{aligned}
  \phi^{(s)}=&\;\max{\Bigg(\theta_d,\Bigg[\sum_{i=1}^{\max{(Q)}}\,\max{(\bm{\lambda}_i,l_i)}\Bigg]\Bigg)}
  \\[1ex]
  =&\;\max{\Big(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)}\Big)}\;\text{, (for}\;q=4\text{),}
  \\[1ex]
  =&\;\overbrace{\mathcal{R}\Big(\Big[\;\overbrace{\mathcal{R}(l_1-\bm{\lambda}_1)+\bm{\lambda}_1}^{\text{(first }\max\text{ of summation)}}+\;\dots\;+\overbrace{\mathcal{R}(l_4-\bm{\lambda}_4)+\bm{\lambda}_4}^{\text{(}n\text{-th }\max\text{ of summation)}}\;\Big]-\theta_d\Big)+\theta_d}^{\text{(outer }\max\text{; recall that }\max{(x,y)=\mathcal{R}(y-x)+x}\text{)}}\;\text{.}
\end{aligned}
$$

In the above expansion, $\theta_d$ is a constant defined at initialization time, and the same is true for $\bm{\lambda}$. However, $\phi^{(s)}$ will appear in each part of the model as we see later, and its gradient is sensitive $\forall\,l$, i.e., not just for, e.g., $l_q$ or $\forall\,l_r,r<q$. However, since it is the summation that will be affected, and since it does the same $\forall\,l$, the gradient, conveniently, will be the same for each.

$$
\begin{aligned}
  \nabla\,\phi^{(s)}=&\;\frac{\partial\,\phi^{(s)}}{\partial\,l_q,\forall\,q\in Q}\;\text{,}
  \\[1ex]
  =&\;\mathcal{H}\Big(l_q-\bm{\lambda}_q\Big)\times \mathcal{H}\Big(\Big[\;\overbrace{\mathcal{R}(l_1-\bm{\lambda}_1)+\bm{\lambda}_1+\;\dots\;+R(l_4-\bm{\lambda}_4)+\bm{\lambda}_4}^{\sum_{i=1}^{\max{(Q)}}\,\max{(\bm{\lambda}_i,l_i)}}\;\Big]-\theta_d\Big)\;\text{.}
\end{aligned}
$$
...
$$
\begin{aligned}
  m_{q=4}^c=&\;f\bigg(s_b+\frac{(x-\beta_l-\phi_q)\times\delta_s\times\phi}{\max{(\bm{\lambda_q},l_q)}}\bigg)
  \\[1ex]
  =&\;f\bigg(s_b+\frac{(\delta_sx-\delta_s\beta_l-\delta_s\phi_q)\times\phi}{\max{(\bm{\lambda_q},l_q)}}\bigg)
  \\[1ex]
  =&\;f\Big(s_b+\Big[\;\overbrace{\delta_sx-\delta_s\beta_l-\overbrace{\delta_s\max{(\bm{\lambda_1},l_1)}-\delta_s\max{(\bm{\lambda_2},l_2)}-\delta_s\max{(\bm{\lambda_3},l_3)}}^{\phi_{q=4}}}^{(a-b-c-d-e)}\;\times\overbrace{\phi\times\max{(\bm{\lambda_q},l_q)}^{-1}}^{(j-k)}\;\Big]\Big)
  \\
  \vdots
  \\
  =&\;f\Big(s_b+aj-ak-bj+bk-cj+ck-dj+dk-ej+ek\Big)\;\text{.}
\end{aligned}
$$
...
\begin{aligned}
  f\Bigg(&s_b
  \\[0ex]
  &+aj=\Big[\;\delta_s\times x\times \overbrace{\beta_u\times\max{(\bm{\lambda}_q,l_q)}^{-1}\times\max{\Big(\theta_d,\max{(\bm{\lambda}_1,l_1)}+\dots+\max{(\bm{\lambda}_4,l_4)}\Big)}}^{j}\;\Big]
  \\[1ex]
  &-ak=\Big[\;\delta_s\times x\times\overbrace{\beta_l\times\max{(\bm{\lambda}_q,l_q)}^{-1}\times\max{\Big(\theta_d,\;\dots\;\Big)}}^{k}\;\Big]
  \\[1ex]
  &-bj=\Big[\;\delta_s\times\beta_l\times \beta_u\times\max{(\dots)}^{-1}\times\max{(\dots)}\;\Big]
  \\[1ex]
  &+bk=\Big[\;\delta_s\times{\beta_l}^2\times{\dots}\times{\dots}\;\Big]
  \\[1ex]
  &-cj=\Big[\;\delta_s\times\max{(\bm{\lambda}_1,l_1)}\times\overbrace{\beta_u\times{\dots}\times{\dots}}^{j}\;\Big]
  \\[1ex]
  &+ck=\Big[\;\delta_s\times\max{(\bm{\lambda}_1,l_1)}\times\overbrace{\beta_l\times{\dots}\times{\dots}}^{k}\;\Big]
  \\[1em]
