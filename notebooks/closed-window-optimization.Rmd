...
And here is the test that should yield the exact same results as we got with `cow_og1_test1p`:
...
Now we have a very late cut-in, that basically ignores `5/6` of the signal. However, subjectively and visually, it is a fine match (slightly worse than the fully open, though). Let's do this once more, open begin, closed end:
...
\begin{aligned}
  ...
  \\[1ex]
  \frac{\partial\,\mathcal{R}^{\text{bb}}}{\partial\,\bm{\theta}_i}=&\;(\bm{\theta}_i-b_i^{(l)})\times\mathcal{D}(b_i^{(l)}-\bm{\theta}_i)+(\bm{\theta}_i-b_i^{(u)})\times\mathcal{D}(\bm{\theta}_i-b_i^{(u)})
  \\[0ex]
  ...
  \\[1ex]
  \mathcal{L}_{\mathcal{R}^{\text{bb}}}=&\;\log{\Big(1+\mathcal{R}^{\text{bb}}\Big)}\;\text{, with gradient}
  \\[1ex]
