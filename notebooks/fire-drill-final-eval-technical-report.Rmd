...
However first, we will implement a continuous relative 1D version to test our approach. Let's generate some sample data that should be highly correlated. See how we deliberately have different means in order to dislocate both variables.
...
curve(f_a, min(c(dens_a$x, dens_b$x)), max(c(dens_a$x, dens_b$x)))
...
Let's test using the previously generated data, density, and functions thereof:
...
Check, very similar. Next, we compute and plot the result of the numerator:
...
The continuous version of the numerator is in the previously computed result. If we integrate it, we get the __mean__ of the function (since the area under the curve over the interval $[0,1]$ is always a mean), not the sum.
...
Let's check some values as computed in the denominator for the two variables:
...
The continuous version of the denominators is the following:
...
In order to show the correlation in three dimensions, we'll compute a grid:
...
From this example we see there is clearly both, positive and negative correlation. Here is the same as a colorful contour plot:

```{r}
image(tempgrid, zlim = c(-.2,.2))
contour(tempgrid, zlim = c(-.2,.2), add = TRUE)
```
