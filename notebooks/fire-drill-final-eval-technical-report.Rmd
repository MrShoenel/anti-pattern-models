...
```{r}
plotexpr <- rlang::expr({
  par(mfrow=c(1,1), mar=c(3,3,1,0), family = "LM Roman 10", oma=rep(0,4))


  plot(x=0, y=0, xlim = c(0,1), ylim = c(0,1), col = "#00000000",
       xlab = "", ylab = "")
  title(ylab = "Relative cum. time spent", line = 2)
  title(xlab = "Relative project time", line = 2)
  grid()
  ...
  #text(x = .28, y = .86, paste0("$CI_{req}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"))
  #boxtext(x = .28, y = .86, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{CI}_{\\mathrm{req}}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"))
  text(x = .495, y = .86, paste0("$y_1=", round(req_ci_upper(t_1), 2), "$"))
  text(x = .305, y = .46, paste0("$y_2=", round(req_ci_lower(t_1), 2), "$"))
  ...
  # t_1(dev), t_2(dev):
  text(x = .295, y = .21, paste0("$y_3=", round(dev_ci_upper(t_1), 3), "$"))
  abline(h = dev_ci_upper(t_1), col = "#888888")
  #boxtext(x = .12, y = .2, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{dev}(t_1)=", round(dev(t_1), 3), "$"))
  abline(h = dev(t_2), col = "#888888")
  #text(x = .51, y = .46, paste0("$DEV(t_2)=", round(dev(t_2), 2), "$"))
  #boxtext(x = .51, y = .46, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{dev}(t_2)=", round(dev(t_2), 2), "$"))
  #text(x = .75, y = .45, paste0("$y_4=", round(dev(t_2), 2), "$"))
  
  # other thresholds:
  abline(h = dev_ci_upper(t_2), col = "#888888")
  text(x = .755, y = .76, paste0("$y_4=", round(dev_ci_upper(t_2), 2), "$"))
  #boxtext(x = .73, y = .77, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{CI}_{\\mathrm{dev}}^{+}(t_2)=", round(dev_ci_upper(t_2), 2), "$"))
  ...
  abline(v = t_1)
  text(x = t_1, y = 1.1, paste0("$t_1=", t_1, "$"), xpd = NA)
  abline(v = t_2)
  text(x = t_2, y = 1.1, paste0("$t_2=", t_2, "$"), xpd = NA)
  
  legend(.72, 0.42, legend = c("req", "$\\mathrm{CI}_{\\mathrm{req}}^{+}$", "$\\mathrm{CI}_{\\mathrm{req}}^{-}$"),
         col = "red", lty = c(1, 3, 2), lwd = c(2.25, 1.5, 1.5),
         bg = "white", cex = .85)
  
  legend(.01, 1.02, legend = c("dev", "$\\mathrm{CI}_{\\mathrm{dev}}^{+}$"),
         col = "blue", lty = c(1, 3), lwd = c(2.25, 1.5),
         bg = "white", cex = .85)
})
...
```{r}
saveTikzExpr(name = "model_t1_it_req-dev", height = 2.2, width = 3.45, expr = plotexpr)
```

```{r}
plotexpr <- rlang::expr({
  par(mfrow=c(1,1), mar=c(3,2.5,1,0), family = "LM Roman 10", oma=rep(0,4))
  ...
  #boxtext(x = .28, y = .86, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{CI}_{\\mathrm{req}}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"))
  text(x = .495, y = .86, paste0("$y_1=", round(req_ci_upper(t_1), 2), "$"))
  text(x = .495, y = .46, paste0("$y_2=", round(req_ci_lower(t_1), 2), "$"))
  ...
  polygon(
    x = c(x, rev(x)),
    y = c(dev_ci_upper(x), desc(rev(x))),
    col = "#1a701a44",
    border = NA)
  
  curve(dev_ci_upper, 0, 1, col = "forestgreen", lty = 3, add = TRUE)
  #curve(dev, 0, 1, col = "blue", lty = 1, lwd = 3, add = TRUE)
  curve(desc, 0, 1, col = "forestgreen", lty = 2, add = TRUE)
  ...
  # other thresholds:
  #abline(h = dev_ci_upper(t_2), col = "#888888")
  #text(x = .755, y = .75, paste0("$y_4=", round(dev_ci_upper(t_2), 2), "$"))
  #boxtext(x = .73, y = .77, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$\\mathrm{CI}_{\\mathrm{dev}}^{+}(t_2)=", round(dev_ci_upper(t_2), 2), "$"))
  ...
  abline(v = t_1)
  text(x = t_1, y = 1.1, paste0("$t_1=", t_1, "$"), xpd = NA)
  text(x = .895, y = .21, paste0("$y_5=", round(desc(1), 3), "$"))
  
  
  #legend(.72, 0.37, legend = c("req", "$\\mathrm{CI}_{\\mathrm{req}}^{+}$", "$\\mathrm{CI}_{\\mathrm{req}}^{-}$"),
  #       col = "red", lty = c(1, 3, 2), lwd = c(2, 1, 1),
  #       bg = "white", cex = .85)
  
  legend(.01, 1.02, legend = c("$\\mathrm{CI}_{\\mathrm{desc}}^{+}$", "$\\mathrm{CI}_{\\mathrm{desc}}^{-}$"),
         col = "forestgreen", lty = c(3, 2), lwd = 1.5,
         bg = "white", cex = .85)
})
...
```{r}
saveTikzExpr(name = "model_t1_it_req-desc", height = 2.2, width = 3.45, expr = plotexpr)
```
...
plotexpr <- rlang::expr({
  ...
  par(mar=c(3,3,1.2,1), family = "LM Roman 10", mfrow=c(1,2)) # mar=c(3,3,1.2,1)
  
  
  plot(x=0, y=0, xlim = c(0,1), ylim = c(0,1.02), col = "#00000000",
       xlab = "", ylab = "", cex.axis = 0.7)
  title(xlab = "Relative project time",
        ylab = "Amount of activity", line = 2, cex.lab = 0.85)
  grid()
  ...
  text(x = fd_data_boundaries[1], y = 1.12, paste0("$b_1=", fd_data_boundaries[1], "$"), cex = .8, xpd = NA)
  abline(v = fd_data_boundaries[2], col = "#888888")
  text(x = fd_data_boundaries[2], y = 1.12, paste0("$b_2=", fd_data_boundaries[2], "$"), cex = .8, xpd = NA)
  abline(v = fd_data_boundaries[3], col = "#888888")
  text(x = fd_data_boundaries[3], y = 1.12, paste0("$b_3=", fd_data_boundaries[3], "$"), cex = .8, xpd = NA)
  ...
  plot(x=0, y=0, xlim = c(0,1), ylim = c(0,1), col = "#00000000",
       xlab = "", ylab = "", cex.axis = 0.7)
  title(xlab = "Relative project time",
        ylab = "Relative cum. time spent", line = 2, cex.lab = 0.85)
  grid()
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(name = "model_t1_comb", height = 2.3, expr = plotexpr, width = 6.49)
```
...
plotexpr <- rlang::expr({
  ...
  # t_1:
  abline(v = t1_wavg)
  abline(v = t_1, col = "#999999")
  text(x = .3, y = .91, paste0("$t_1=", t_1, "$"), col = "#999999")
  boxtext(x = .43, y = .11, labels = paste0("$\\hat{t}_1=", round(t1_wavg, 2), "$"), col.bg = "white", padding = c(0.1, 1.5))
  
  legend(.7, .5, legend = c("$\\mathrm{req}$", "$\\mathrm{CI}_{\\mathrm{req}}^{+}$", "$\\mathrm{CI}_{\\mathrm{req}}^{-}$"),
         col = "red", lty = c(1, 3, 2), lwd = c(2, 1, 1),
         bg = "white", cex = .85)
  ...
  abline(v = t2_wavg)
  text(x = .47, y = .91, paste0("$\\hat{t}_2=", round(t2_wavg, 2), "$"))
  abline(v = t_2, col = "#999999")
  text(x = .74, y = .28, paste0("$t_2=", t_2, "$"), col = "#999999")
  
  legend(.02, 1.0, legend = c("$\\mathrm{CI}_{\\mathrm{dev}}^{+}$", "$\\mathrm{dev}$", "$\\mathrm{desc}$"),
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(name = "model_t2_it", height = 3.5, width = 3.33, expr = plotexpr)
...
plotexpr <- rlang::expr({
  par(mfrow=c(3,1), mar=c(2,4,1,.8), family = "LM Roman 10", cex.axis = 1.5, oma=rep(0,4))
  ...
  title(ylab = "Relative cum. time spent", line = 2.5, cex.lab = 1.2)
  grid()
  ...
  legend(0.49, .42, legend = c("$\\overline{\\mathit{req}\\vphantom{X}}$", "Weight $=0$", "Weight $>0$"), bg = "white",
  ...
  fields::image.plot(x = tempz$x, y = tempz$y, z = tempz$z,
    ...
    legend.shrink = .5, legend.lab = "Confidence", legend.line = 3)
  title(ylab = "Relative cum. time spent", line = 2.5, cex.lab = 1.2)
  grid()
  ...
  legend(0.01, .98, legend = c("$\\overline{\\mathit{dev}\\vphantom{X}}$"),
         col = "blue", lty = 1, lwd = 2.5, bg = "white", cex = 1.2)
  ...
  fields::image.plot(x = tempz$x, y = tempz$y, z = tempz$z,
    ...
    legend.shrink = .5, legend.lab = "Confidence", legend.line = 3, legend.cex = 1)
  title(xlab = "Relative project time", ylab = "Relative cum. time spent", line = 2.5, cex.lab = 1.2)
  grid()
  ...
  legend(0.01, 0.98 * desc_ci_upper_p3avg(1) * 1.1, legend = c("$\\overline{\\mathit{desc}\\vphantom{X}}$"),
         col = "forestgreen", lty = 1, lwd = 2.5, cex = 1.2, bg = "white")
})
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(name = "emp_ci_req_dev", height = 4.9, width = 3.33, expr = plotexpr)
```
...
plotexpr <- rlang::expr({
  ...
  legend(0.01, .98, legend = c("$\\overline{\\mathit{a}\\vphantom{X}}$", "Weight $=0$", "Weight $>0$"), bg = "white",
         col = c("red", rep("#00000033", 2)), lty = c(1, 3, 2), lwd = c(2.5, 2, 2), cex = 1.2)
  ...
  legend(0.01, .98, legend = c("$\\overline{\\mathit{cp}\\vphantom{X}}$"), bg = "white",
  ...
  legend(0.01, .98, legend = c("$\\overline{\\mathit{freq}\\vphantom{X}}$"), bg = "white",
  ...
  legend(0.325, .2, legend = c("$\\overline{\\mathit{scd}\\vphantom{X}}$"), bg = "white",
...
```{r}
saveTikzExpr(name = "emp_ci_req_dev_sc", height = 6.7, width = 3.33, expr = plotexpr)
```
...
plotexpr <- expression({
  temp <- t(rbind(
    `rownames<-`(x = pAll_it_corr, value = c("I", "II(a)", "III")),
    `rownames<-`(x = p4All_it_corr, value = c("IV(I)", "IV(II(a))", "IV(III)"))
  ))
  temp <- temp[, rev(colnames(temp))]
  
  # Remove 'mae' and leave in 'are':
  temp <- temp[!grepl(pattern = "_mae$", rownames(temp)),]
  ...
  ggcorrplot::ggcorrplot(corr = temp, ggtheme = fe_gg2_style() + theme(
      #plot.background = element_rect(colour = "red", fill=NA, size=1),
      axis.text.x.top = element_text(angle = 90, vjust = 1/3, hjust = 0, size = 8),
      axis.text.y.left = element_text(size = 8),
      legend.position = "bottom",
      legend.text = element_text(vjust = 0),
      legend.title = element_text(margin = ggplot2::margin(r = 10, b = 3)),
      legend.key.width = unit(20, 'pt'),
...
```{r}
# 806:323
tikzDevice::tikz(file = paste0("../figures/fig_fdfe_", "corr-all-it", ".tex"), width = 4.99, verbose = FALSE, height = 2.3)
eval(plotexpr)
dev.off()
#saveTikzExpr(name = "corr-all-it", height = 2.5, width = 6.3, expr = eval(plotexpr))
```
