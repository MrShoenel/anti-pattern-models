...
plotexpr <- rlang::expr({
  ...
  text(x = .82, y = .64, paste0("$REQ(t_1)=", round(req(t_1), 2), "$"))
  
  abline(h = req_ci_upper(t_1), col = "#888888")
  #text(x = .28, y = .86, paste0("$CI_{req}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"))
  boxtext(x = .28, y = .86, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$CI_{req}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"))
  ...
  #text(x = .2, y = .2, paste0("$DEV(t_1)=", round(dev(t_1), 3), "$"))
  boxtext(x = .12, y = .2, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$DEV(t_1)=", round(dev(t_1), 3), "$"))
  abline(h = dev(t_2), col = "#888888")
  #text(x = .51, y = .46, paste0("$DEV(t_2)=", round(dev(t_2), 2), "$"))
  boxtext(x = .51, y = .46, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$DEV(t_2)=", round(dev(t_2), 2), "$"))
  
  # other thresholds:
  abline(h = dev_ci_upper(t_2), col = "#888888")
  #text(x = .73, y = .77, paste0("$CI_{dev}^{+}(t_2)=", round(dev_ci_upper(t_2), 2), "$"))
  boxtext(x = .73, y = .77, col.bg = "white", padding = c(0.1, 1.5), labels = paste0("$CI_{dev}^{+}(t_2)=", round(dev_ci_upper(t_2), 2), "$"))
  abline(h = desc(1), col = "#888888")
  text(x = .87, y = .2, paste0("$DESC(1)=", round(desc(1), 2), "$"))
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(name = "model_t1_it", height = 4.2, expr = plotexpr)
...
plotexpr <- rlang::expr({
  ...
  par(mar=c(3,3,.25,0), family = "LM Roman 10")
  ...
  text(x = .165, y = .45, paste0("$b_1=", fd_data_boundaries[1], "$"), cex = .8)
  abline(v = fd_data_boundaries[2], col = "#888888")
  text(x = .54, y = .98, paste0("$b_2=", fd_data_boundaries[2], "$"), cex = .8)
  abline(v = fd_data_boundaries[3], col = "#888888")
  text(x = .795, y = .31, paste0("$b_3=", fd_data_boundaries[3], "$"), cex = .8)
  
  legend(.14, 1.05, legend = c("$\\mathsf{A}$", "$\\mathsf{CP}$"),
         col = c("red", "blue"), lty = 1, lwd = 2,
         bg = "white", cex = .7)
  legend(.29, 1.05, legend = c("$\\mathsf{FREQ}$", "$\\mathsf{SCD}$"),
         col = c("forestgreen", "purple"), lty = 1, lwd = 2,
         bg = "white", cex = .7)
  ...
  # other thresholds:
  abline(h = req(t_1), col = "#888888")
  text(x = .51, y = .64, paste0("$\\mathsf{REQ}(t_1)=", round(req(t_1), 2), "$"), cex = .8)
  
  abline(h = req_ci_upper(t_1), col = "#888888")
  text(x = .515, y = .86, labels = paste0("$\\mathsf{CI}_{\\mathsf{REQ}}^{+}(t_1)=", round(req_ci_upper(t_1), 2), "$"), cex = .8)
  ...
  # t_1:
  abline(v = t_1)
  text(x = .47, y = 1, paste0("$t_1=", t_1, "$"), cex = .8)
  ...
  # t_1(dev), t_2(dev):
  abline(h = dev(t_1), col = "#888888")
  text(x = .27, y = .11, labels = paste0("$\\mathsf{DEV}(t_1)=", round(dev(t_1), 3), "$"), cex = .8)
  abline(h = dev(t_2), col = "#888888")
  text(x = .745, y = .45, labels = paste0("$\\mathsf{DEV}(t_2)=", round(dev(t_2), 2), "$"), cex = .8)
  
  # other thresholds:
  abline(h = dev_ci_upper(t_2), col = "#888888")
  text(x = .74, y = .75, labels = paste0("$\\mathsf{CI}_{\\mathsf{DEV}}^{+}(t_2)=", round(dev_ci_upper(t_2), 2), "$"), cex = .8)
  abline(h = desc(1), col = "#888888")
  text(x = .89, y = .2, paste0("$\\mathsf{DESC}(1)=", round(desc(1), 2), "$"), cex = .8)
  ...
  abline(v = t_1)
  abline(v = t_2)
  text(x = .77, y = 1, paste0("$t_2=", t_2, "$"), cex = .8)
  
  legend(.01, 1.03, legend = c("$\\mathsf{REQ}$", "$\\mathsf{CI}_{\\mathsf{REQ}}^{+}$", "$\\mathsf{CI}_{\\mathsf{REQ}}^{-}$"),
         col = "red", lty = c(1, 3, 2), lwd = c(2, 1, 1),
         bg = "white", cex = .7)
  
  legend(.19, 1.03, legend = c("$\\mathsf{DESC}$", "$\\mathsf{DEV}$", "$\\mathsf{CI}_{\\mathsf{DEV}}^{+}$"),
         col = c("forestgreen", "blue", "blue"), lty = c(1, 1, 3),
         lwd = c(2, 2, 1), bg = "white", cex = .7)
})
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(name = "model_t1_comb", height = 4, expr = plotexpr)
```
