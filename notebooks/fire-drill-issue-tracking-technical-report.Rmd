...
The weights then assign an order to it. The bigger the differences between two raters' assessments, the bigger the difference in weight will be.


Here is the ground truth assessment as of the third rater:

```{r}
ground_truth_rater.c <- c(0, 0, 10, 8, 0, 2, 4, 2, 2, 1, 0, 0, 6, 0, 0)
```
...
c(
  ...
  stats::cor(ground_truth_all$rater.a, ground_truth_rater.c),
  stats::cor(ground_truth_all$rater.b, ground_truth_rater.c))
```
...
c(
  ...
  stats::cor.test(ground_truth_all$rater.a, ground_truth_rater.c)$p.value,
  stats::cor.test(ground_truth_all$rater.b, ground_truth_rater.c)$p.value)
```
...
(temp <- 1 - mean(c(
  ...
  Metrics::mae(ground_truth_all$rater.a, ground_truth_rater.c),
  Metrics::mae(ground_truth_all$rater.b, ground_truth_rater.c)
)) / 10)
```
...
```{r}
ratings <- cbind(ground_truth_all[, c("rater.a", "rater.b")], data.frame(rater.c = ground_truth_rater.c))
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Fleiss' Kappa, Gwet's AC1/AC2, Krippendorff's Alpha, and Conger's Kappa. The table shows the percent agreement (pa), percent chance agreement (pe), the coefficient's/Kappa's value, its standard error, the confidencen interval, and p-value.",
    label = "gt-kappas"
  )
}
```
...
\begin{align}
    ...
    \\[1ex]
    =&\;\int_a^b\,\frac{1}{\sqrt{\operatorname{Var}(\kappa)}\sqrt{2\pi}}e^{-\frac{1}{2}\left(\frac{x-\kappa}{\sqrt{\operatorname{Var}(\kappa)}}\right)^2}\,dx\label{eq:kappa-bench-param}.
\end{align}
...
curve2(tempf, 0, 1, xlim = c(.35, 1), ylim = c(0, 6.5), main = "Normal distribution for Gwet's AC1/AC2 coefficient.", xlab = "Kappa value", ylab = "Relative likelihood")
abline(v = irrCAC::landis.koch$ub.LK, col = "#aaaaaa", lty = 2)
pal <- viridis::viridis(100, begin = .2, end = .3)

text(x = .5, y = 6.25, "Moderate")
text(x = .7, y = 6.25, "Substantial")
text(x = .9, y = 6.25, "Almost Perfect")

for (lb in c(.4, .6, .8)) {
  ub <- lb + .2
  prob <- stats::integrate(f = tempf, lower = lb, upper = ub)$value
  intensity <- as.hexmode(round(15 + prob * 240))
  ...
  text(x = lb + .1, y = 1.5, labels = paste0(round(100 * prob, 2),"%"))
}
grid()
```
