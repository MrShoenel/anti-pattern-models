...
for (i in 1:length(projNames)) {
  for (j in 1:nrow(ac_weight_grid_it)) {
    ...
    temp[i, j] <- ac_grid_projects_results_uniform_it[
      ac_grid_projects_results_uniform_it$PM == "III" &
...
ac_it_weights_varimp <- loadResultsOrCompute(file = "../results/ac_it_weights_varimp.rds", computeExpr = {
  doWithParallelCluster(numCores = min(8, parallel::detectCores()), expr = {
    library(caret, quietly = TRUE)
...
```{r echo=FALSE}
if (interactive()) {
  print(ac_it_weights_varimp$fit$finalModel)
} else {
...
No. of variables tried at each split: 2
          ...
          Mean of squared residuals: 0.09069598
                    % Var explained: -2.75")
}
```
...
The MSE to beat was $\approx0.0096$, and the lowest MSE achieved by a Random forest was $\approx0.064$, which is an order of magnitude larger. So clearly, our variable importance approach is better.

```{r echo=FALSE}
ac_it_weights_varimp$fit$results$MSE <- ac_it_weights_varimp$fit$results$RMSE**2
if (interactive()) {
  ac_it_weights_varimp$fit$results
} else {
  knitr::kable(
    x = ac_it_weights_varimp$fit$results,
    booktabs = TRUE,
