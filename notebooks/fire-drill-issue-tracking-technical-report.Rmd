...
For demonstration purposes, we can compute the loss surface of a one-interval model using some loss:

```{r echo=FALSE}
temp_get_1interval_srbtaw <- function() {
  srbtaw <- srBTAW$new(
    theta_b = c(0, 1), gamma_bed = c(0, 1, .Machine$double.eps),
    lambda = .Machine$double.eps,
    begin = 0, end = 1, openBegin = TRUE, openEnd = TRUE)
  srbtaw$setParams(`names<-`(c(0, 1, 1), srbtaw$getParamNames()))
  ...
  loss_req <- srBTAW_Loss_Rss$new(
    wpName = "REQ_WP", wcName = "REQ", intervals = 1, returnRaw = FALSE)
  srbtaw$addLoss(loss = loss_req)
  loss_dev <- srBTAW_Loss_Rss$new(
    wpName = "DEV_WP", wcName = "DEV", intervals = 1, returnRaw = FALSE)
  srbtaw$addLoss(loss = loss_dev)
  loss_desc <- srBTAW_Loss_Rss$new(
    wpName = "DESC_WP", wcName = "DESC", intervals = 1, returnRaw = FALSE)
  srbtaw$addLoss(loss = loss_desc)
  ...
  doWithParallelClusterExplicit(cl = cl, expr = {
    foreach::foreach(
      ...
      while (TRUE) {
        ...
        srbtaw <- temp_get_1interval_srbtaw()
...
```{r proc-align-srbtaw-loss-surface, echo=FALSE, fig.cap="The loss surface for one-interval time warping using srBTAW of the parameters begin and end. The lowest loss is marked with a circle.", fig.align="center", fig.pos="ht!"}
z_temp <- list(
...
points(x = temp[, 1] / use_resol, y = temp[, 2] / use_resol)
title(ylab = "begin", xlab = "end")
```
