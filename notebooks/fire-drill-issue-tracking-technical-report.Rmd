...
As expected, the result for the RSS-based is very similar to the overall result as obtained by DTW (note that the mix-up of begin and end does not matter to srBTAW, is it regularizes this using its internal representation $\beta_l,\beta_u$). The correlation-based loss, surprisingly, manages to discover the true begin of $\approx0.25$. It is likely possible to find a combination of losses, weights, etc., that can find the offsets of some process better, but that is a bit out of scope here. In fig. \ref{fig:proc-align-srbtaw} we show the time warping as computed by the two objectives (RSS and correlation).
