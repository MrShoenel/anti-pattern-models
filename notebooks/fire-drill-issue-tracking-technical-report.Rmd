...
Figures \ref{fig:p4-no-plateaus-smooth} and \ref{fig:p4-no-plateaus-smooth-d1} show the non-plateau signal, its LOESS-smoothed version, and the derivative of the smoothed version.

```{r p4-no-plateaus-smooth, echo=FALSE, fig.height=3, fig.cap="The non-plateau signal on the left, and its LOESS-smoothed version (span=0.4) on the right.", fig.align="center", fig.pos="ht!"}
par(mfrow=c(1,2))
plot(x, y, type="l")

temp <- loess.smooth(x = x, y = y, span = .5, family = "g", evaluation = 1e4)
plot(temp$x,temp$y,type="l")

#tempf <- stats::approxfun(x = seq(0, max(x), length.out = length(temp$x)), y = temp$y)
tempf <- stats::approxfun(
  x = (temp$x - min(temp$x)) / (max(temp$x - min(temp$x))),
  y = temp$y / (max(temp$x - min(temp$x))))
temps <- Signal$new(func = tempf, name = "a", support = c(0, 1), isWp = TRUE)
```


```{r p4-no-plateaus-smooth-d1, echo=FALSE, fig.height=3, fig.cap="The LOESS-smoothed signal and its derivative.", fig.align="center", fig.pos="ht!"}
temps$plot(show1stDeriv = TRUE, showSignal = FALSE)# + ylim(c(-2,2))
```

From Figure \ref{fig:p4-no-plateaus-smooth-d1} we can observe that the gradient is probably acceptable for means of comparison, but the real problem lies in its values: The co-domain appears to go up close to $100$, which is a huge slope that is not found in the derived pattern. There the maximum observed slopes are $\approx5$. Spans $<0.4$ exacerbate the problem with the gradient, and higher values result in too strong smoothing. Attempting the same using just the cumulative data including the plateaus results in extreme-value gradients, too.
...
```{r p4-averaged-bins-d1, fig.height=2.5, fig.cap="The average-bin signal and its gradient.", fig.align="center", fig.pos="ht!"}
...
```{r echo=FALSE, fig.height=3.5, fig.align="center", fig.pos="ht!"}
temps <- Signal$new(func = tempf, name = "dummy", support = range(x), isWp = TRUE)
temps$plot(show1stDeriv = TRUE)
```

### Constrained B-splines non-parametric regression quantiles

.. or COBS, estimates (fits) a smooth function using B-splines through some "knots". In figure \ref{fig:p4-cobs-splines} we are attempting fitting with $[3,10]$ knots. The number of knots should probably not exceed $4$ or $5$ for the gradient to be useful, as otherwise it gets too many modes.
...
for (i in 1:8) {
  temp <- cobs::cobs(
    x = X,
    y = p3_signals$data$`cum req`,
    nknots = i+2,
    print.mesg = FALSE,
    print.warn = FALSE
  )
  
  tempf <- function(x) stats::predict(object = temp, z = x)[, "fit"]
  curve(tempf, min(X), max(X), main = paste0("knots=", i+2))
  tempf1 <- function(x) pracma::fderiv(f = tempf, x = x)
  curve(tempf1, min(X), max(X), main = paste0("deriv,knots=", i+2))
}
```

### Orthogonal polynomials

Using a varying degree, we fit polynomials to the data in order to obtain a truly smooth curve. This technique always results in smooth approximations both for the signal and its gradient. In figure \ref{fig:p4-polynomials-d1} we observe how the derivative captures more and more nuances of the signal, starting from a degree of approx. $4$. For our tests with this pattern later, I therefore do recommend using polynomials of degree $3$, because the true purpose of all this is to estimate the non-linear trend for the variable and its rate of change.
...
par(mfrow=c(4,4))
Y <- p3_signals$data$`cum req`
use_degs <- c(2, 3, 4, 5, 8, 12, 19, length(stats::coef(poly_autofit_max(x=X, y=Y))) - 1)

for (ud in use_degs) {
  temp <- stats::lm(Y ~ poly(x = X, degree = ud))
  tempf <- function(x) stats::predict(temp, newdata = data.frame(X=x))
  curve(tempf, min(X), max(X), main = paste0("degree=", ud))
  tempf1 <- function(x) pracma::fderiv(f = tempf, x = x)
  curve(tempf1, min(X), max(X), main = paste0("deriv,degree=", ud))
}
```
...
This pattern emerged only recently, but we have some options of scoring the projects against it. Again, we have (derived) confidence intervals for the variables `REQ` and `DEV`. Then we have derivatives for all variables. Scoring based on CIs are hence applicable, but only for the first two variables. Other than that, any method based on comparing two variables is applicable. In figure \ref{fig:p4-deriv-signals} we show all projects against the fourth pattern.

```{r p4-deriv-signals, echo=FALSE, fig.height=8, fig.cap="All projects' derivative variables plotted against the derivatives of the two variables req\\% and dev\\% of pattern type I. The polynomial degree used here was 3.", fig.align="center", fig.pos="ht!"}
par(mfrow=c(2,1))
polyDegree <- 3
...
for (i in 1:length(all_signals)) {
  ...
  Y_req <- cumsum(all_signals[[pId]]$data$req) / sum(all_signals[[pId]]$data$req)
  
  poly_req <- stats::lm(Y_req ~ stats::poly(x = X, degree = polyDegree))
  req_tempf <- function(x) stats::predict(poly_req, newdata = data.frame(X = x))
  req_tempf_d1 <- function(x) func_d1(f = req_tempf, x = x)
...
for (i in 1:length(all_signals)) {
  ...
  X <- 1:nrow(all_signals[[pId]]$data)
  X <- (X - min(X)) / (max(X) - min(X))
  Y_dev <- cumsum(all_signals[[pId]]$data$dev) / sum(all_signals[[pId]]$data$dev)
  
  poly_dev <- stats::lm(Y_dev ~ stats::poly(x = X, degree = polyDegree))
  dev_tempf <- function(x) stats::predict(poly_dev, newdata = data.frame(X = x))
  dev_tempf_d1 <- function(x) func_d1(f = dev_tempf, x = x)
...
p4_compute_areadist <- function(polyDegree = NA_real_, intFrom = 0, intTo = 1) {
  ...
  doWithParallelCluster(numCores = length(all_signals), expr = {
    ...
    foreach::foreach(
      ...
      .combine = rbind,
      .packages = c("cobs"),
      .export = c("all_signals", "poly_autofit", "L_areadist_p3_avg",
                  "func_d1", "req", "dev", "req_d1_p4", "dev_d1_p4",
                  "req_poly", "dev_poly", "req_ci_upper_d1_p4", "dev_ci_upper_d1_p4",
      ...
      dev_tempf <- NA
      if (is.na(polyDegree)) {
        poly_req <- poly_autofit(xData = X, yData = Y_req, maxDegree = floor(length(X) / 2))
        req_tempf <- function(x) stats::predict(poly_req, newdata = data.frame(xData = x))
        poly_dev <- poly_autofit(xData = X, yData = Y_req, maxDegree = floor(length(X) / 2))
        dev_tempf <- function(x) stats::predict(poly_dev, newdata = data.frame(xData = x))
      } else {
        poly_req <- tryCatch(expr = {
          stats::lm(Y_req ~ stats::poly(x = X, degree = polyDegree))
        }, error = function(cond) NA)
        req_tempf <- function(x) stats::predict(poly_req, newdata = data.frame(X = x))
        
        poly_dev <- tryCatch(expr = {
          stats::lm(Y_dev ~ stats::poly(x = X, degree = polyDegree))
        }, error = function(cond) NA)
        dev_tempf <- function(x) stats::predict(poly_dev, newdata = data.frame(X = x))
      }
      
      if (!is.na(poly_req) && !is.na(poly_dev)) {
...
p4_area_scores <- loadResultsOrCompute(file = "../results/p4_area_scores.rds", computeExpr =  {
  temp <- NULL
  for (pd in c(2, 3, 4, 5, 6, 8, 10, NA_real_)) {
    temp <- rbind(temp, p4_compute_areadist(polyDegree = pd))
    print(paste0("Finished computing for degree=", pd))
  }
  temp
})
```
...
for (pd in c(2, 3, 4, 5, 6, 8, 10, NA_real_)) {
  temp <- if (is.na(pd)) {
    p4_area_scores[p4_area_scores$REQ_deg > 10, ]
  } else {
    p4_area_scores[p4_area_scores$REQ_deg == pd, ]
  }
  temp <- temp[order(rownames(temp)), ]
  
  p4_area_scores_corr <- rbind(p4_area_scores_corr, data.frame(
    deg = if (is.na(pd)) paste0(range(temp$REQ_deg, temp$DEV_deg), collapse = "-") else pd,
...
In table \ref{tab:p4-area-scores-corr} we can clearly see how the degree of the polynomial influences the correlation with the ground truth. Note that the degrees $2$ and $>10$ are very likely to under- resp. over-fit the data, and those should not be used in practice.


```{r echo=FALSE}
p4_area_scores_partial <- loadResultsOrCompute(file = "../results/p4_area_scores_partial.rds", computeExpr =  {
  temp <- NULL
  for (pd in c(2, 3, 5, 8, NA_real_)) {
    for (end in c(1/3, 1/2, 2/3)) {
      temp <- rbind(temp, p4_compute_areadist(polyDegree = pd, intTo = end))
    }
    print(paste0("Finished computing for degree=", pd))
  }
  temp
...
for (pd in c(2, 3, 5, 8, NA_real_)) {
  for (end in c(1/3, 1/2, 2/3)) {
    temp <- if (is.na(pd)) {
      p4_area_scores_partial[p4_area_scores_partial$REQ_deg > 10 & p4_area_scores_partial$end == end, ]
    } else {
      p4_area_scores_partial[p4_area_scores_partial$REQ_deg == pd & p4_area_scores_partial$end == end, ]
    }
    temp <- temp[order(rownames(temp)), ]
...
for (pId in names(all_signals)) {
  ...
  Y_req <- cumsum(all_signals[[pId]]$data$req) / sum(all_signals[[pId]]$data$req)
  Y_dev <- cumsum(all_signals[[pId]]$data$dev) / sum(all_signals[[pId]]$data$dev)
  Y_desc <- cumsum(all_signals[[pId]]$data$desc) / max(cumsum(all_signals[[pId]]$data$dev))
  ...
  poly_desc <- stats::lm(Y_desc ~ stats::poly(x = X, degree = polyDegree))
  desc_tempf <- function(x) stats::predict(poly_desc, newdata = data.frame(X = x))
  desc_tempf_d1 <- function(x) func_d1(f = desc_tempf, x = x)
...
if (interactive()) {
  ...
  knitr::kable(
    x = p4_corr,
    booktabs = TRUE,
    caption = "Correlation of the derivatives with the derived project signals.",
    label = "p4-corr"
  )
}
```
...
The correlations in table \ref{tab:p4-corr} are varying, but their correlation with the ground truth is mediocre, and in the case of `REQ` and `DESC` even negative. That means that computing the correlation of the derived data with the derived project signals is not a suitable detector. Our previous attempt using the distance in areas was much better.
