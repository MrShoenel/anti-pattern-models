...
$$
\begin{aligned}
  \text{weighted mean}=&\;\Big[\sum\bm{\omega}\Big]^{-1}\times\sum_{i=1}^{N}\bm{\omega}_i\times\bm{\hat{t}_i}\;\text{, where}
  \\[1ex]
  \bm{\omega}\dots&\;\text{weight vector that corresponds to the consensus-score, and}
  \\[1ex]
  \bm{\hat{t}_1},\bm{\hat{t}_2}\dots&\;\text{vectors with optimal values for}\;t_1,t_2\;\text{(as learned earlier).}
\end{aligned}
$$

```{r}
omega <- ground_truth$consensus_score
t1_wavg <- sum(t1t2_opt[, 1] * omega) / sum(omega)
print(c(t_1, t1_wavg))
t2_wavg <- sum(t1t2_opt[, 2] * omega) / sum(omega)
print(c(t_2, t2_wavg))
```
...
p3_weighted_var <- function(name) {
  ...
  function(x) sapply(X = x, FUN = function(x_) {
    sum(omega * unlist(lapply(funcs, function(f) f(x_)))) / sum(omega)
  })
}
```
...
### Determining an empirical confidence interval
...
\begin{aligned}
  ...
  \\[1ex]
  \operatorname{CI}_{\text{upper}}(x)=&\;\sup{(\bm{f}_1(x),\;\dots,\;\bm{f}_n(x))}
  \\[1ex]
  \operatorname{CI}_{\text{lower}}(x)=&\;\inf{(\bm{f}_1(x),\;\dots,\;\bm{f}_n(x))}\text{.}
\end{aligned}
$$
...
Simply put, this loss is based on the confidence interval's hyperplane, and calculates an absolute average confidence based on it.

$$
\begin{aligned}
  \mathit{L}^{\text{avgweight}}(x_1,x_2,f)=&\;\Bigg[\int_{x_1}^{x_2}\,\operatorname{CI}(x, f(x))\,dx\Bigg]\times(x_2-x_1)^{-1}\;\text{,}
  \\
