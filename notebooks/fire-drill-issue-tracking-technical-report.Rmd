...
```{r echo=FALSE}
FIG_WIDTH <- round(6.49733, 4)

saveTikzExpr <- function(expr, name, width = FIG_WIDTH, height = 3.2) {
  tikzDevice::tikz(file = paste0("../figures/fig_", name, ".tex"), width = width, height = height, verbose = FALSE, )
  eval(expr)
  dev.off()
}
```
...
```{r echo=FALSE}
temp <- seq(from = 0, to = 1, length.out = 1e4)
pattern_it <- list(
  REQ = stats::approxfun(x = temp, y = req(temp), rule = 2),
  DEV = stats::approxfun(x = temp, y = dev(temp), rule = 2),
  DESC = stats::approxfun(x = temp, y = desc(temp), rule = 2))

invisible(loadResultsOrCompute(file = "../results/pattern_I_it.rds", computeExpr = {
  pattern_it
}))
```
...
## _f_-divergences as objectives

f-divergences describe the divergence between two continuous _random_ variables, not between two continuous non-random variables (processes).
As we will show, however, it is possible to mimic the most important properties of random variables, making f-divergences applicable to any continuous process.
The computation of any such divergence can be understood as an aggregation of all point-wise differences.
f-divergences are _convex_ functions that express by how much one quantity diverges from the other using a ratio. Therefore, a ratio of $1$ results in a divergence of $0$, and any other ratio in a divergence $>0$.
Three properties are required to mimic random variables. First, for a continuous process $f:\mathbb{R}\mapsto\mathbb{R}$, for all realizations $x\in\mathcal{X}$, $f(x)\geq0$. Second, the integral of $f=1$, i.e., $\int_{\mathcal{X}}\,f=1$. And third, _Absolute continuity_ between $f$ and a second continuous process $g$ is required, that is, the domains of $f$ and $g$ must coincide.
Absolute continuity, sharing the same support, is especially required in cases where the segments of the PM and the process are not the same, which might by a valid case for some objectives.

In many cases it might be feasible to fulfill all three properties.
The first one is perhaps just a translation of $f$ by defining $f'=f-\argmin{f}$. When designing a PM, this property can be conveniently considered.
The second property requires the first to hold. Then, $f$ can perhaps be scaled by defining $f''=f'\div\int\,f'$.
For fulfilling the third property, we suggest using a transform- and scale-operator, $\mathsf{T}$ \eqref{eq:transf-op}.
Consider the example of the Kullback--Leibler divergence between distributions $P,Q$, and their resp. densities $p,q$ \eqref{eq:kl-div}.
Plugging in the transform operator $\mathsf{T}$ allows us to define a ``relative'' variant of the divergence \eqref{eq:kl-div-rel}.
Another advantage is that integration intervals simplify to $[0,1]$ \eqref{eq:kl-div-rel-int}, which is beneficial to any measurement operator or objective.


\begin{align}
    \mathsf{T}(a,b,x)=&\;a+x(b-a)\label{eq:transf-op}\text{,}
    \\[1ex]
    D_{\text{KL}}\left(P\,\|\,Q\right)=&\;\int_{-\infty}^{\infty}\,p(x)\log{\left(\frac{p(x)}{q(x)}\right)}\,dx\label{eq:kl-div}\text{,}
    \\[1em]
    a_P,b_P,a_Q,b_Q\;\dots&\;\text{supports for}\;p,q\text{ (known a priori),}\nonumber
    \\[1ex]
    D_{\text{KL}}^{\text{rel}}\left(P\,\|\,Q\right)=&\;p\big(\mathsf{T}(a_P,b_P,x)\big)\log{\left(\frac{p\big(\mathsf{T}(a_P,b_P,x)\big)}{q\big(\mathsf{T}(a_Q,b_Q,x)\big)}\right)}\label{eq:kl-div-rel}\text{, where}
    \\[1ex]
    \operatorname{supp}\left(D_{\text{KL}}^{\text{rel}}\left(P\,\|\,Q\right)\right)=&\;[0,1]\nonumber\text{,}
    \\[1ex]
    %\operatorname{supp}\Big(D_{\text{KL}}^{\text{rel}}(P\,\|\,Q)\Big)=&\;[0,1]\;\text{, so the divergence}\;d_{PQ}\;\text{becomes}\nonumber
    %\\[1ex]
    d_{PQ}=&\;\int_0^1\,D_{\text{KL}}^{\text{rel}}\left(P\,\|\,Q\right)\,dx\label{eq:kl-div-rel-int}\text{.}
\end{align}


## $m$-dimensional relative continuous Pearson sample correlation coefficient\label{ssec:m-dim-pearson}
...
\begin{aligned}
  ...
  \\[0ex]
  &\;\;\times\prod_{i=1}^{m}\,(a_{i,1}-a_{i,2})^{-1}\,\text{, (analogously for}\;\overline{g}\;\text{using}\;\bm{\mathit{S}}_g\text{),}
  \\[1ex]
...
p4p1_it_signals <- list(
  "REQ"  = get_smoothed_signal_d1(name = "p4p1_it_REQ",  func = req,  use_span = use_span),
  "DEV"  = get_smoothed_signal_d1(name = "p4p1_it_DEV",  func = dev,  use_span = use_span),
  "DESC" = get_smoothed_signal_d1(name = "p4p1_it_DESC", func = desc, use_span = use_span))

p4p1_it_projects <- time_warp_wrapper(pattern = p4p1_it_signals, derive = TRUE)


p4p2a_it_signals <- list(
  "REQ"  = get_smoothed_signal_d1(name = "p4p2a_it_REQ",  func = req_p2a,  use_span = use_span),
  "DEV"  = get_smoothed_signal_d1(name = "p4p2a_it_DEV",  func = dev_p2a,  use_span = use_span),
  "DESC" = get_smoothed_signal_d1(name = "p4p2a_it_DESC", func = desc_p2a, use_span = use_span))

p4p2a_it_projects <- time_warp_wrapper(pattern = p4p2a_it_signals, derive = TRUE)


p4p3_it_signals <- list(
  "REQ"  = get_smoothed_signal_d1(name = "p4p3_it_REQ",  func = req_p3,  use_span = use_span),
  "DEV"  = get_smoothed_signal_d1(name = "p4p3_it_DEV",  func = dev_p3,  use_span = use_span),
  "DESC" = get_smoothed_signal_d1(name = "p4p3_it_DESC", func = desc_p3, use_span = use_span))
...
invisible(loadResultsOrCompute(file = "../results/project_signals_2nd_batch_it.rds", computeExpr = {
  all_signals_2nd_batch
}))
```
...
```{r ac-calib-dec-rule, echo=FALSE, fig.height=5, fig.cap="The automatically calibrated scores (losses) of the decision rule."}
par(mfrow = c(2,2))
...
We should also examine the scores more in detail for both configurations of weights, and find out what were the scores for the best/worst projects (table \ref{tab:ac-it-dr-scores-2-confs}).

```{r echo=FALSE}
as.density <- function(x) {
  temp <- stats::density(x)
  `attributes<-`(x = stats::approxfun(x = temp$x, y = temp$y, yleft = 0, yright = 0), value = list("ab" = range(temp$x)))
}
...
kl_div <- function(vals1, vals2) {
  ...
  x <- seq(from = r[1], to = r[2], length.out = 1e3)
  v1 <- f1(x)
  v2 <- f2(x)
  unname(suppressMessages({
    philentropy::distance(
      method = "kullback-leibler", unit = "log", x = rbind(v1 / sum(v1), v2 / sum(v2)))
  }))
}
```
...
for (confName in c("all_equal", "pref_reqt1")) {
  temp.weight <- if (confName == "all_equal") 1 else 2
  temp.pred <- data.frame(
    pred = sapply(X = names(temp), function(pName) ac_p1_dr(projName = pName, signals = temp, w_reqt1 = temp.weight)),
    ground_truth = temp.gt)
  
  temp.df <- rbind(temp.df, data.frame(
    Config = confName,
    ScForBest = ac_p1_dr(signals = temp, projName = paste0("Project", which.max(temp.gt))),
    ScForWorst = ac_p1_dr(signals = temp, projName = paste0("Project", which.min(temp.gt))),
    ScMax = max(temp.pred$pred),
    ScMin = min(temp.pred$pred),
    ScAvg = mean(temp.pred$pred),
    KLdiv = kl_div(temp.gt, temp.pred$pred),
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(temp.df[, 1], round(temp.df[, 2:10], 4)),
    booktabs = TRUE,
    caption = "Scores and scoring rules for two configurations as calculated by the decision rule for two different configurations.",
    label = "ac-it-dr-scores-2-confs"
  )
}
```
...
Let's use the weights (see table \ref{tab:ac-it-dr-opt-scores-with-weights}):
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Using the optimized weights for the decision rule to demonstrate the deviation from the ground truth.",
    label = "ac-it-dr-opt-scores-with-weights"
  )
}
```
...
```{r ac-it-lm-fit2, echo=FALSE, fig.cap="Residuals and QQ-plot of the automatically calibrated decision rule that was fitted using a linear model."}
par(mfrow=c(1,2))
plot(p3_avg_lm, ask = FALSE, which = 1:2)
```

Using the approximate coefficients of the linear model (figure \ref{fig:ac-it-lm-fit2}), we can define the detector as follows:
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    caption = "Impact of the smoothing-span on the correlation of the computed distance vs. the ground truth.",
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    caption = "Correlation of computed scores decline with more time considered.",
...
## Ground truth inter-rater agreement
...
We have previously reported the quadratic weighted Kappa, as well as the Pearson correlation.
We should note, however, that the proposed scale by @landis1977application was chosen rather arbitrarily, and that each Kappa _agreement coefficient_ is a point estimate associated with a probability distribution and a margin of error [@klein2018agreement].
It is therefore recommended to properly _benchmark_ raw Kappa coefficients.
Gwet suggests to compute the probability of a Kappa to fall into a certain range by integrating a standard normal distribution (where the Kappa coefficient is the mean and its associated error is the standard deviation) [@gwet2014].
We should probably also report Gwet's coefficient called "AC\textsubscript{$1$}", which outperforms other coefficients in terms of having reasonably small biases for estimating the true inter-rater reliability [@gwet2008ac1].
It is especially applicable in the presence of high agreement because of its comparatively low bias.


Also, so far, we had two raters. In the meantime, however, a third rater gave their assessment for all $15$ projects.
In order to assess inter-rater agreement of three or more raters, we will have to use a Kappa or coefficient that can handle at least three raters (Gwet's can, for example).

```{r}
# Let's put them together.
ground_truth_all <- rbind(ground_truth, ground_truth_2nd_batch)
# Add the 3rd rater to it:
ground_truth_all$rater.c <- c(0, 0, 10, 8, 0, 2, 4, 2, 2, 1, 0, 0, 6, 0, 0)
```

```{r echo=FALSE, eval=FALSE}
# This block and the next attempt to empirically find weights for the raters
# such that the weighted-consensus is closest to the original consensus.

tempf<-function(x) {
  g <- x[1] * ground_truth_all$rater.c + x[2] * ground_truth_all$rater.a + x[3] * ground_truth_all$rater.b
  Metrics::rmse(ground_truth_all$consensus, g)
}

tempf_ineq <- function(x) {
  # s.t. max(xTg) >= 9.5
  # = max(xTg) - 9.5 >= 0
  # = -max(xTg) + 9.5
  x <- x / sum(x)
  m <- max(x[1] * ground_truth_all$rater.c + x[2] * ground_truth_all$rater.a + x[3] * ground_truth_all$rater.b)
  -m + 9.5
}
...
(x <- nloptr::nloptr(
    ...
    eval_g_ineq = tempf_ineq,
    opts = list(algorithm = "NLOPT_GN_ISRES", xtol_rel=1e-6, maxeval=25000)
))

x <- x$solution
x <- x / sum(x)
x
round(round(x[1] * ground_truth_all$rater.c + x[2] * ground_truth_all$rater.a + x[3] * ground_truth_all$rater.b,1))
ground_truth_all$consensus
```
...
tempf_ineq_jac <- function(x) {
  pracma::jacobian(f = tempf_ineq, x0 = x)
}
...
(x <- nloptr::nloptr(
    ...
    eval_grad_f = function(x) pracma::grad(f = tempf, x0 = x),
    eval_g_ineq = tempf_ineq,
    eval_jac_g_ineq = tempf_ineq_jac,
    opts = list(algorithm = "NLOPT_LD_MMA", "check_derivatives"=TRUE, xtol_rel=1e-6, maxeval=100)
))
...
x
ground_truth_all$consensus
round(x[1] * ground_truth_all$rater.c + x[2] * ground_truth_all$rater.a + x[3] * ground_truth_all$rater.b, 0)
```
...
* Various other Kappas: Gwet's AC\textsubscript{$1$}, Fleiss' Kappa, Krippendorff's Alpha, and Conger's Kappa.

For the last point, we will also benchmark these Kappas according to Landis and Koch's scale, as well as Fleiss' scale (Altman's scale is almost identical with Landis + Koch).
Note that the various Kappas and coefficients (except for Krippendorff's) are designed to work with nominal data only.
All of the ones we are going to test support, however, weight matrices. That means, we can make them work with ordinal data (i.e., ordered nominal data).
We will therefore first convert our ground truth, which is in the range $[0,10]$ to a factor/string, and then use linear weights (our ground truth uses a linear Likert scale).
We can therefore treat our Likert-scaled data as ordinal. Since the items are spaced linearly, so will be the weights.
We should note, however, that there are are various other methods for assessing inter-rater agreement on Likert scales [@James1984; @ONeill2017] but here, however, we will treat it as an equally-spaced ordinal scale, which is equivalent but more straightforward.
The weights then assign an order and absolute distance between items to it. The bigger the differences between two raters' assessments, the bigger the difference in weight will be.
...
```{r}
c(
  stats::cor(ground_truth_all$rater.a, ground_truth_all$rater.b),
  stats::cor(ground_truth_all$rater.a, ground_truth_all$rater.c),
  stats::cor(ground_truth_all$rater.b, ground_truth_all$rater.c))
```
...
```{r}
c(
  stats::cor.test(ground_truth_all$rater.a, ground_truth_all$rater.b)$p.value,
  stats::cor.test(ground_truth_all$rater.a, ground_truth_all$rater.c)$p.value,
  stats::cor.test(ground_truth_all$rater.b, ground_truth_all$rater.c)$p.value)
```
...
```{r}
(temp <- 1 - mean(c(
  Metrics::mae(ground_truth_all$rater.a, ground_truth_all$rater.b),
  Metrics::mae(ground_truth_all$rater.a, ground_truth_all$rater.c),
  Metrics::mae(ground_truth_all$rater.b, ground_truth_all$rater.c)
)) / 10)
```

This means that we have a percentual agreement between all three raters of about `r round(100 * temp, 2)`%.
This quite good, but we have to proceed to the next method of benchmarking to get recognizable results.


### Benchmarking of Fleiss' Kappa, Gwet's AC1/AC2, Krippendorff's Alpha, and Conger's Kappa

Let's compute a few Kappas and agreement coefficients.
Table \ref{tab:gt-kappas} shows an overview. The percentage agreement is almost always the same and equal to what we computed previously manually.
The standard error seems to be quite similar, too.
But we see differences for the coefficient itself, with the largest being Gwet's AC\textsubscript{$1$}/AC\textsubscript{$2$} (the remaining others being pretty similar).
This is the one we should report, as Gwet has shown that it outperforms other Kappas/coefficients in the presence of high agreement, which is the case here [@gwet2008ac1].
...
temp <- rbind(
  irrCAC::fleiss.kappa.raw(ratings = ratings, weights = "linear", categ.labels = paste0(0:10))$est,
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Fleiss' Kappa, Gwet's AC1/AC2, Krippendorff's Alpha, and Conger's Kappa. The table shows the percent agreement (pa), percent chance agreement (pe), the coefficient's/Kappa's value, its standard error, the confidence interval, and p-value.",
    label = "gt-kappas"
  )
}
```
...
Let's compute some benchmarks.
They are shown in tables \ref{tab:bench-landis-koch} and \ref{tab:bench-fleiss}.
What these tables show is the amount of probability with with the computed agreement coefficient falls into the defined range.
For example, about $82.18$% of the computed coefficient falls into the _Substantial_-category.
Also, the Kappa is $>99.99$% associated with the category _Moderate_ ($\geq0.4$) or better.
These results mean that the majority of the Kappa is substantial, and we have a small (but not insignificant) portion being in the almost perfect category ($\approx5$%). Another $\approx12.9$% are in the moderate category.
In table \ref{tab:bench-fleiss}, about $1/6$ falls into the category _Excellent_, and the remaining $5/6$ into the next best category _Intermediate to Good_.
...
```{r echo=FALSE}
temp1$Prob <- diff(c(0, as.numeric(temp1$CumProb)))
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp1,
    booktabs = TRUE,
    caption = "Landis-Koch benchmark (cumulative probabilities) for Gwet's AC1/AC2 agreement coefficient for all three raters.",
    label = "bench-landis-koch"
  )
}
```

```{r echo=FALSE}
temp2$Prob <- diff(c(0, as.numeric(temp2$CumProb)))
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp2,
    booktabs = TRUE,
    caption = "Fleiss's benchmark (cumulative probabilities) for Gwet's AC1/AC2 agreement coefficient for all three raters.",
    label = "bench-fleiss"
  )
}
```
...
Tables \ref{tab:bench-landis-koch} and \ref{tab:bench-fleiss} show the benchmarks for @landis1977application and @Fleiss1981, respectively.
There are two more common benchmarks, the one by @cicchetti1981 and one by @Regier2013.
These are shown in tables \ref{tab:bench-cicc} and \ref{tab:bench-regier}, respectively.
Note that the benchmark by @Regier2013 uses the same levels as the one by @landis1977application, but only uses different labels.
We will compute these, too, to get a better idea of the goodness of the measured agreement.

```{r}
bench_cicc <- c("start" = 0, "Poor" = 0.4, "Fair" = 0.6, "Good" = 0.75, "Excellent" = 1)
bench_regier <- c("start" = 0, "Unacceptable" = 0.2, "Questionable" = 0.4, "Good" = 0.6, "Very Good" = 0.8, "Excellent" = 1)
...
compute_benchmark <- function(benchvec, name, cdf) {
  ...
  for (idx in 2:length(benchvec)) {
    ...
    df <- rbind(df, data.frame(
      X1 = names(benchvec)[idx],
      X2 = round(cdf(b) - cdf(a), 5),
...
```{r echo=FALSE}
temp_cdf <- function(q) pnorm(q = q, mean = ac1$coeff.val, sd = ac1$coeff.se)
temp3 <- compute_benchmark(benchvec = bench_cicc, name = "Cicchetti", cdf = temp_cdf)
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp3,
    booktabs = TRUE,
    caption = "Cicchetti et al.'s benchmark (cumulative probabilities) for Gwet's AC1/AC2 agreement coefficient for all three raters.",
    label = "bench-cicc"
  )
}
```
...
```{r echo=FALSE}
temp_cdf <- function(q) pnorm(q = q, mean = ac1$coeff.val, sd = ac1$coeff.se)
temp4 <- compute_benchmark(benchvec = bench_regier, name = "Regier", cdf = temp_cdf)
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp4,
    booktabs = TRUE,
    caption = "Regier et al.'s benchmark (cumulative probabilities) for Gwet's AC1/AC2 agreement coefficient for all three raters.",
    label = "bench-regier"
  )
}
```
...
We can also visualize the probability. It is computed using a standard normal distribution [@klein2018agreement].
Equation \ref{eq:kappa-bench} shows how to compute the probability for an agreement coefficient $\kappa$ to be in the range $[a,b]$.
This is also called _interval membership probability_ [@gwet2014].
Note that the square root of the variance ($\sqrt{\operatorname{Var}}$) is the same as the standard deviation.
Instead of this formula, we can also use a parameterized PDF (or CDF) directly (see equation \ref{eq:kappa-bench-param}).
For example:

```{r}
tempf <- function(q) pnorm(q = q, mean = ac1$coeff.val, sd = ac1$coeff.se)

# This is equivalent to the Landis-Koch benchmark of "Almost Perfect" (see previous table)
c(tempf(1) - tempf(.8))
```

\begin{align}
    \Phi\dots&\;\text{CDF of the standard normal distribution}\nonumber,
    \\[1ex]
    Pr(a\leq\kappa\leq b)=&\;\Phi\left(\frac{\kappa-a}{\sqrt{\operatorname{Var}(\kappa)}}\right)-\Phi\left(\frac{\kappa-b}{\sqrt{\operatorname{Var}(\kappa)}}\right)\label{eq:kappa-bench},
    \\[1ex]
    =&\;\int_{\begin{cases}
    a&\text{, if}\;a>0,
    \\
    ...
    \\
    \infty&\text{, otherwise}
    \end{cases}}\,\left[\frac{1}{\sqrt{\operatorname{Var}(\kappa)}\sqrt{2\pi}}e^{-\frac{1}{2}\left(\frac{x-\kappa}{\sqrt{\operatorname{Var}(\kappa)}}\right)^2}\right]\,dx\label{eq:kappa-bench-param}.
\end{align}


Figure \ref{fig:gwet-ac1ac2-pdf} shows the three top-most categories according to @landis1977application and their integral.

```{r gwet-ac1ac2-pdf, echo=FALSE, fig.align="top", fig.pos="ht!", fig.cap=paste0("Normal distribution for Gwet's AC1/AC2 coefficient. Shown are the integrated areas and the opacity indicates the probability for the coefficient to be in the given range. The mean is at approx. ", round(ac1$coeff.val, 3), " and the standard deviation is about ", round(ac1$coeff.se, 3), ".")}
tempf <- function(x) dnorm(x = x, mean = ac1$coeff.val, sd = ac1$coeff.se)

makefig <- function(paper = FALSE) {
  rlang::expr({
    paper <- !!paper
    cex <- if (paper) .9 else 1
    mline <- if (paper) 2.5 else 3
    if (paper) {
      # 5.1 4.1 4.1 2.1
      par(mar = c(4.1, 3.6, 2.1, .1))
    }
  
    curve2(tempf, 0, 1, xlim = c(.35, 1), ylim = c(0, 7), main = "Normal distribution for Gwet's AC1/AC2 coefficient.", cex.main = cex, cex.lab = cex, cex.axis = cex, xlab = "", ylab = "", xaxt = "n", yaxt = "n")
    mtext(text = "Kappa value", side = 1, line = mline, cex = cex)
    mtext(text = "Relative Likelihood", side = 2, line = mline, cex = cex)
    axis(1, at = seq(0, 1, by = .1), cex.axis = cex)
    axis(2, at = seq(0, 10, by = 1), cex.axis = cex)
    grid()
    abline(v = irrCAC::landis.koch$ub.LK, col = "#aaaaaa", lty = 2)
    pal <- viridis::viridis(100, begin = .2, end = .3)
    
    text(x = .5, y = 6.4, "Moderate", cex = cex)
    text(x = .7, y = 6.4, "Substantial", cex = cex)
    text(x = .9, y = 6.4, "Almost\nPerfect", cex = cex)
    
    for (lb in c(.4, .6, .8)) {
      ub <- lb + .2
      prob <- stats::integrate(f = tempf, lower = lb, upper = ub)$value
      intensity <- as.hexmode(round(35 + prob * 220))
      if (stringi::stri_length(intensity) == 1) {
        intensity <- paste0("0", intensity)
      }
      
      use_x <- seq(from = lb, to = ub, length.out = 100)
      polygon(x = c(ub, lb, use_x), y = c(0, 0, tempf(use_x)), col = paste0(substr(pal[round(prob * 100)], 1, 7), intensity))
      text(x = lb + .1, y = 1.8, labels = paste0(round(100 * prob, 2), (if (paper) "\\"), "%"), col = if (lb == .6) "white" else "black")
    }
  })
}
...
```{r echo=FALSE, eval=FALSE}
saveTikzExpr(makefig(paper = TRUE), "it_gwet-benchmark", height = 2.7, width = .75 * FIG_WIDTH)
```
...
* Furthermore, all processes start at $0,0$ (x/y). The variables `REQ` and `DEV` also always end at $1,1$.
* While $DESC$ does never end at $y=1$ (it cannot as of the normalization through whatever amount `DEV` accumulated), we can conservably limit it to $y=\frac{2}{3}$.
...
```{r echo=FALSE}
# This function works a bit different for issue-tracking data, as here we
# have exclusively cumulative quantities.
# Also, the DESC variable is special in that it can never reach 1, as it
# normalized by the amount of activities in the DEV variable. Them mean of
# DESC(1) is ~0.125 and the max is ~0.58. Therefore, we will assume a maximum
# de-scoping of 2/3.
# unlist(lapply(append(all_signals, all_signals_2nd_batch), function(p) p$DESC$get0Function()(1)))
get_smoothed_curve <- function(seed = NA, npoints = 15, include_deriv = FALSE, issue_tracking = FALSE, desc_var = FALSE) {
  if (!is.na(seed)) {
    set.seed(seed = seed)
  }
  
  x <- sort(c(0, 1, runif(n = npoints - 2)))
  y <- runif(n = length(x), min = 0, max = if (issue_tracking && desc_var) runif(n = 1, max = 2/3) else 1)
  if (issue_tracking) {
    # For issue-tracking, we exclusively have cumulative (monotonically increasing)
    # quantities that start at 0.
    y <- sort(y)
    y[1] <- 0
  }
  ...
  if (issue_tracking) {
    temp$y <- sort(temp$y) - min(temp$y)
  }
  appr <- stats::approxfun(
    x = ((temp$x - min(temp$x)) / (max(temp$x) - min(temp$x))),
    y = if (issue_tracking && !desc_var) {
      temp$y / (max(temp$y) - min(temp$y))
    } else {
      temp$y / (max(temp$x) - min(temp$x))
    },
    yleft = utils::head(temp$y, 1),
    yright = utils::tail(temp$y, 1))
...
ac_grid_results_it <- loadResultsOrCompute(file = "../results/ac_grid_results_it.rds", computeExpr = {
  ...
  doWithParallelClusterExplicit(cl = cl, stopCl = TRUE, expr = {
    ...
    foreach::foreach(
      ...
      smooth_curve <- get_smoothed_curve(seed = params$Seed, include_deriv = include_deriv, issue_tracking = TRUE, desc_var = params$Var == "DESC")
...
Let's show some randomly picked examples in figure \ref{fig:ac-grid-example-it}. Similarly to source code data, the ECCDFs are all clearly non-linear.

```{r ac-grid-example-it, echo=FALSE, fig.height=5, fig.cap="Four randomly picked ECCDFs as simulated using the random processes of the automatic calibration for issue-tracking data."}
...
for (i in 1:4) {
  ...
  curve2(function(x) 1 - tempf(x), min(temp), max(temp), xlab = "", main = paste0(
    "PM=", temp.pm,
    ", Var=", temp.var
  ), sub = paste0(
    "SegIdx=", temp.seg,
...
### Normality and uniformity tests of the objectives

In fact, let's check if any of the scores is normally or uniformly distributed.
We will do two tests, the Shapiro--Wilk and the Kolmogorov--Smirnov tests for normality.

```{r}
ac_distr_test_it <- loadResultsOrCompute(file = "../results/ac_distr_test_it.rds", computeExpr = {
  temp.grid <- expand.grid(list(
    PM = levels(ac_grid_it$PM),
    Var = unique(ac_grid_it$Var),
    Seg = 1:10,
    Metric = c("area", "corr", "jsd", "kl", "arclen", "sd", "var", "mae",
               "rmse", "RMS", "Kurtosis", "Peak", "ImpulseFactor")
  ))
  
  as.data.frame(doWithParallelCluster(numCores = min(parallel::detectCores(), 32), expr = {
    foreach::foreach(
      ...
      set.seed(1)
      row <- temp.grid[rn,]
      temp <- ac_extract_data(
        pmName = row$PM, varName = row$Var, segIdx = row$Seg, metricName = row$Metric,
        ac_grid = ac_grid_it, ac_grid_results = ac_grid_results_it)
      
      samp <- sample(x = temp, size = 5e3)
      st <- if (length(unique(samp)) == 1) list("statistic" = 0, "p.value" = 0) else shapiro.test(samp)
      ...
      `colnames<-`(x = matrix(data = c(
        st$statistic, st$p.value, ks_norm$statistic, ks_norm$p.value, ks_unif$statistic, ks_unif$p.value
      ), nrow = 1), value = c("shap.W", "shap.pval", "ks_norm.D", "ks_norm.pval", "ks_unif.D", "ks_unif.pval"))
    }
  }))
})
```

Out of `r nrow(ac_distr_test_it)` ECDFs, __`r nrow(ac_distr_test_it[ac_distr_test_it$shap.pval > 0.05 | ac_distr_test_it$ks_norm.pval > 0.05,])`__ are normally distributed, and __`r nrow(ac_distr_test_it[ac_distr_test_it$ks_unif.pval > 0.05,])`__ are uniformly distributed.
The highest p-values for either normally test are `r max(ac_distr_test_it$shap.pval)` and `r max(ac_distr_test_it$ks_norm.pval)`. The highest W-statistic was `r max(ac_distr_test_it$shap.W)`, and the lowest D-statistic was `r min(ac_distr_test_it$ks_norm.D)`.
The highest p-value for the standard uniform test was `r max(ac_distr_test_it$ks_unif.pval)`, and the lowest D-statistic was `r min(ac_distr_test_it$ks_unif.D)`.
It is fair to say that nothing here is normally or uniformly distributed.
We now have reasonable evidence that justifies that conversion to linear behaving scores is required.
  ...
  doWithParallelClusterExplicit(cl = cl, expr = {
    ...
    foreach::foreach(
      ...
      p_var <- if (use_deriv) {
        ...
        # The recommended span is >= 0.2, and we'll use the value 0.35 as this was also the
        # span used for smoothing the projects' variables.
...
```{r ac-grid-projects-results-uniform-it}
  ...
  doWithParallelCluster(numCores = min(32, parallel::detectCores()), expr = {
    ...
    foreach::foreach(
      ...
      for (i in 1:length(project_signals_all)) {
        res[1, i] <- 1 - tempf(ac_grid_projects_results_it[
...
Let's have a look at how the calibrated models behave, without further fitting of weights.
This will give us insights into which (family of) models is best suitable for predicting the ground truth (given the configuration).
...
for (pmName in c("I", "II(a)", "III", "IV(I)", "IV(II(a))", "IV(III)")) {
  ...
  temp.df <- rbind(temp.df, data.frame(
    ...
    ScAvg = mean(temp.pred$pred),
    KLdiv = kl_div(temp.gt, temp.pred$pred),
    Corr = stats::cor(temp.gt, temp.pred$pred),
    RMSE = sqrt(Metrics::mse(actual = temp.gt, predicted = temp.pred$pred)),
    MSE = Metrics::mse(actual = temp.gt, predicted = temp.pred$pred),
    Log = mean(scoring::logscore(object = ground_truth ~ pred, data = temp.pred))
  ))
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(data.frame(PM = temp.df$PM), round(temp.df[, 2:ncol(temp.df)], 4)),
    booktabs = TRUE,
...
Table \ref{tab:ac-compare-pms-it} shows the results of the calibrated models. It is worth noting that we are looking at two families of models: the ordinary PMs, and the derivative PMs are in on family each.
What we can see directly is that the derivative models all suffer from the inconsistency that the score for the best project is lower than the score for the worst project. The ordinary models do not have that problem. Also the MSE is much larger for derivative models.
Another observation is that the score for best/worst is not necessarily the lowest/highest score predicted.
The mean score in each family tells us which the best model is. Here, it is type III, and that is no big surprise, as it was derived from data, so it should resemble the observed projects best.
However, looking at the spread of best/worst scores, it seems that there is room for improvement in terms of selecting the right objectives (weights).


### Weighted by optimization\label{sssec:weighted-by-opt}
...
ac_it_weights_optim <- function(pmName, useLog = FALSE) {
  loadResultsOrCompute(file = paste0("../results/ac_it_weights_pm_", pmName, (if (useLog) "_Log"), ".rds"), computeExpr = {
    cl <- parallel::makePSOCKcluster(min(123, parallel::detectCores()))
      ...
      optimParallel::optimParallel(
        ...
        fn = function(x) {
          if (useLog) {
            temp.pred <- data.frame(
              ground_truth = temp.gt,
              pred = sapply(X = temp.names, FUN = function(projName) {
                ac_pmp_score_weighted(
                  pmName = pmName, projName = projName, weightGrid = ac_weight_grid_it,
            ...
            return(Metrics::mse(actual = temp.gt, predicted = sapply(X = temp.names, FUN = function(projName) {
              ac_pmp_score_weighted(
                pmName = pmName, projName = projName, weightGrid = ac_weight_grid_it,
                results_uniform = ac_grid_projects_results_uniform_it, weightVector = x)
            })))
...
An overview of the optimization process on a per-project basis is shown in table \ref{tab:ac-it-weights-optim-overview}.
...
temp <- data.frame(
  PM = c("I", "II(a)", "III", "IV(I)", "IV(II(a))", "IV(III)"),
  Objective = rep("MSE", 6),
  Solution = round(unlist(lapply(templ, function(res) res$value)), 5),
  RMSE = round(unlist(lapply(templ, function(res) sqrt(res$value))), 5),
  Num_w_gt0 = unlist(lapply(templ, function(res) sum(res$par > 0))),
  Perc_w_pruned = unlist(lapply(templ, function(res) paste0(round(100 * (1 - sum(res$par > 0) / length(res$par)), 2), "%"))),
  Iter = unlist(lapply(templ, function(res) res$counts["function"])),
  Iter_grad = unlist(lapply(templ, function(res) res$counts["gradient"])))


templ <- list(
  ac_it_weights_pm_I_Log,
  ac_it_weights_pm_IIa_Log,
  ac_it_weights_pm_III_Log,
  ac_it_weights_pm_IV_I_Log,
  ac_it_weights_pm_IV_IIa_Log,
  ac_it_weights_pm_IV_III_Log)
temp <- rbind(temp, data.frame(
  PM = c(
    "I", "II(a)", "III",
    "IV(I)", "IV(II(a))", "IV(III)"),
  Objective = rep("Log-Score", 1),
  Solution = round(unlist(lapply(templ, function(res) res$value)), 5),
  RMSE = rep(NA_real_, 6),
  Num_w_gt0 = unlist(lapply(templ, function(res) sum(res$par > 0))),
  Perc_w_pruned = unlist(lapply(templ, function(res) paste0(round(100 * (1 - sum(res$par > 0) / length(res$par)), 2), "%"))),
...
if (interactive()) {
  ...
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Overview of the results of optimizing the weights using the normalizing linear scalarizer objective, on a per-project basis, for issue-tracking process models. Shown are results once minimized using the MSE, and once using the Log-score.",
    label = "ac-it-weights-optim-overview"
  )
}
```

Table \ref{tab:ac-it-weights-optim-overview} gives on overview of the optimization results, which are quite diverse.
All but model type IV(II(a)) could be pruned considerably (by $\approx97$% or more), which means that about $6-12$ objectives is enough to predict with a small deviation.
Again, we observe a substantial difference between the families of ordinary and derived models, with the former performing considerably better.
Running the minimization with the Log-score results in significantly smaller models. However, this was just a test, and we should not derive any conclusions from the Log-score results. This is because technically, the Log-score assumes mutually-exclusive, discrete classes (which we do not have).
So the test was whether or not some optimization can be reached using another objective.
...
for (useLog in c(FALSE, TRUE)) {
  for (pmName in c("I", "II(a)", "III", "IV(I)", "IV(II(a))", "IV(III)")) {
    temp.weights <- switch (
      pmName,
      "I" = if (useLog) ac_it_weights_pm_I_Log$par else ac_it_weights_pm_I$par,
      "II(a)" = if (useLog) ac_it_weights_pm_IIa_Log$par else ac_it_weights_pm_IIa$par,
      "III" = if (useLog) ac_it_weights_pm_III_Log$par else ac_it_weights_pm_III$par,
      "IV(I)" = if (useLog) ac_it_weights_pm_IV_I_Log$par else ac_it_weights_pm_IV_I$par,
      "IV(II(a))" = if (useLog) ac_it_weights_pm_IV_IIa_Log$par else ac_it_weights_pm_IV_IIa$par,
      "IV(III)" = if (useLog) ac_it_weights_pm_IV_III_Log$par else ac_it_weights_pm_IV_III$par,
      {
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(
      temp.df[, colnames(temp.df) %in% c("PM", "Obj")],
      round(temp.df[, !(colnames(temp.df) %in% c("PM", "Obj"))], 4)),
    booktabs = TRUE,
...
The results of table \ref{tab:ac-compare-pms-it-optim} demonstrate that we can drastically improve the models by optimizing the variable importance.
Going by lowest MSE and widest spread, the clear winner is perhaps again type III.
Again, the family of derivative models has much higher MSEs, but we were able to correct the inconsistency for best/worst projects.

In general we can say that the family of derivative models performs worse than their ordinary counter part. However, the performance of the derivative models is perhaps still acceptable in some scenarios, and from these results we can say that they work and are operationalizable, even if not the best.
An MSE $<\approx0.1$ means that the models is perhaps already usable in practice.


We also want to check what results are achievable using an ordinary Random forest.
For that, we are going to use the champion model (type III).


```{r ac-it-weights-varimp-traditional}
ac_it_weights_varimp <- loadResultsOrCompute(file = "../results/ac_it_weights_varimp.rds", computeExpr = {
  temp.gt <- c(ground_truth$consensus_score, ground_truth_2nd_batch$consensus_score)
  ...
  for (pmName in levels(ac_grid_it$PM)) {
    ...
    for (i in 1:length(projNames)) {
      for (j in 1:nrow(ac_weight_grid_it)) {
        ...
        temp[i, j] <- ac_grid_projects_results_uniform_it[
          ac_grid_projects_results_uniform_it$PM == pmName &
          ac_grid_projects_results_uniform_it$Metric == params$Metric &
          ac_grid_projects_results_uniform_it$Var == params$Var &
          ac_grid_projects_results_uniform_it$SegIdx == params$SegIdx, projNames[i]]
      }
    }
    ...
    templ[[pmName]] <- doWithParallelCluster(numCores = min(8, parallel::detectCores()), expr = {
      library(caret, quietly = TRUE)
...
```{r echo=FALSE, eval=FALSE}
randomForest:::print.randomForest(ac_it_weights_varimp$I$fit$finalModel)
```

Let's take a look at the fitted model's performance (table \ref{tab:ac-it-weights-varimp-mse}).
The results are only ever so slightly better than those obtained using the source code PMs (table \ref{tab:ac-sc-weights-varimp-mse}).
Four out of six explained variances are still negative, with the other only being barely positive, meaning that the Random forest overfits every time.
Therefore, it is hard to assess the validity of the obtained results for the variable importance.
The RMSEs are slightly better, but similar.
The MSE to beat was $\approx0.0096$, and the lowest MSE achieved by a Random forest was $\approx0.058$, which is an order of magnitude larger. So, clearly, our variable importance approach is better.

```{r echo=FALSE}
temp.df <- NULL

for (pmName in names(ac_it_weights_varimp)) {
  temp <- as.data.frame(ac_it_weights_varimp[[pmName]]$fit$results)
  temp$MSE <- temp$RMSE**2
  temp <- head(temp[order(temp$RMSE), ], 1)
  temp$VarExpl <- round(100 * tail(ac_it_weights_varimp[[pmName]]$fit$finalModel$rsq, 1), 3)
  
  temp <- cbind(data.frame(PM = pmName), temp)
  temp.df <- rbind(temp.df, temp)
}
colnames(temp.df) <- gsub(pattern = "Rsquared", replacement = "Rsq", x = colnames(temp.df))
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(data.frame(PM = temp.df$PM, mtry = temp.df$mtry), round(temp.df[, 3:ncol(temp.df)], 4)),
    booktabs = TRUE,
    caption = "Best models of fitting of a Random forest to the calibrated data in order to assess the variable importance.",
    label = "ac-it-weights-varimp-mse"
  )
}
```


#### Detailed results for model type I

If we inspect the metrics used in model (I/MSE), we can make some very interesting observations (table \ref{tab:ac-it-details-pm-i}).
Using only twelve objectives in total (5/3/4 usages for `REQ`/`DEV`/`DESC`), we see that the by far most important objective is the `ImpulseFactor` on the `DESC` variable in segment $9$. This seems plausible, as the de-scoping is an important indicator for a project going awry, but only towards project end.
...
The next two most (and equally) important objectives are the standard deviation and the variance in segment $4$.
Apart from the `arclen` in segment one and the `jsd` and `kl` in segment $10$, the rest of the objectives are signal metrics.

Also, segments $\{2,3,6\}$ were not used and deemed unimportant. In other words, using only $7$/$10$ segments, the model is still able to detect the presence (or absence) of the Fire Drill with great confidence.


```{r echo=FALSE}
temp <- ac_weight_grid_it[ac_it_weights_pm_I$par > 0, ]
temp$weight <- ac_it_weights_pm_I$par[ac_it_weights_pm_I$par > 0]
temp$weight <- temp$weight / max(temp$weight)
temp$weight_rel <- temp$weight / sum(temp$weight)
temp$weight_cum <- cumsum(temp$weight_rel) # keep order by segIdx
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    row.names = FALSE,
    caption = "The metrics used in PM I. All but one metric are metrics from signal theory.",
    label = "ac-it-details-pm-i"
  )
}
```

#### Detailed results ordinary vs. derivative models

Let's compare the performance of all ordinary PMs vs their derivative counterparts.

```{r echo=FALSE}
temp <- cbind(ac_weight_grid_it, data.frame(
  weight =
    1 / 6 * (
    ac_it_weights_pm_I$par +
    ac_it_weights_pm_IIa$par +
    ac_it_weights_pm_III$par +
    ac_it_weights_pm_I_Log$par +
    ac_it_weights_pm_IIa_Log$par +
    ac_it_weights_pm_III_Log$par)
))

temp.deriv <- cbind(ac_weight_grid_it, data.frame(
  weight =
    1 / 6 *(
    ac_it_weights_pm_IV_I$par +
    ac_it_weights_pm_IV_IIa$par +
    ac_it_weights_pm_IV_III$par +
    ac_it_weights_pm_IV_I_Log$par +
    ac_it_weights_pm_IV_IIa_Log$par +
    ac_it_weights_pm_IV_III_Log$par)
))


temp <- temp[temp$weight > 0,]
temp <- temp[order(-temp$weight),]
temp.deriv <- temp.deriv[temp.deriv$weight > 0,]
temp.deriv <- temp.deriv[order(-temp.deriv$weight),]
```


```{r ac-it-weights-detailed-plot, echo=FALSE, fig.height=8, fig.cap="Average and total importances of scores per segment, variable, and metric, for the families of ordinary and derivative process models (optimized using MSE)."}
layout(mat = matrix(data = c(1,3,2,4,5,5,6,6), ncol = 2, byrow = TRUE), widths = c(2.6, 1))
par(oma = rep(0, 4))

set.seed(1)
pal_6 <- sample(RColorBrewer::brewer.pal(7, "Set3"))
pal_13 <- c(pal_6[7], pal_6[1], rep(pal_6[2], 2), pal_6[3], rep(pal_6[4], 2), rep(pal_6[5], 2), rep(pal_6[6], 4))

pal_10 <- viridis::viridis(10)
pal_rdd <- c("red", "blue", "forestgreen")
...
barplot(temp.seg, main = "Average importance per segment", col = pal_10, angle = 30, density = 15+50 * temp.seg / max(temp.seg))
grid()
temp.seg <- `names<-`(sapply(X = 1:10, FUN = function(si) {
  weights <- temp.deriv[temp.deriv$SegIdx == si,]$weight
  if (length(weights) == 0) 0 else mean(weights)
}), paste0(1:10))
barplot(temp.seg, main = "Average importance per segment (derivative)", col = pal_10, angle = 30, density = 15+50 * temp.seg / max(temp.seg))
grid()
...
barplot(temp.var, main = "Average importance per variable", col = pal_rdd, angle = 30, density = 15+50 * temp.var / max(temp.var))
grid()
temp.var <- `names<-`(sapply(X = sort(unique(temp$Var)), function(var) {
  weights <- temp.deriv[temp.deriv$Var == var,]$weight
  if (length(weights) == 0) 0 else mean(weights)
}), sort(unique(temp$Var)))
barplot(temp.var, main = "Average importance per variable (derivative)", col = pal_rdd, angle = 30, density = 15+50 * temp.var / max(temp.var))
grid()
...
barplot(temp.metric, main = "Average importance per metric", col = pal_13, angle = 30, density = 15+50 * temp.metric / max(temp.metric))
grid()

temp.metric <- `names<-`(sapply(X = sort(unique(temp$Metric)), function(m) {
  weights <- temp.deriv[temp.deriv$Metric == m,]$weight
  if (length(weights) == 0) 0 else mean(weights)
}), sort(unique(temp$Metric))) #%>% sort %>% rev
# We need to shorten some metrics' names:
names(temp.metric) <- sapply(names(temp.metric), function(m) {
...
barplot(temp.metric, main = "Average importance per metric (derivative)", col = pal_13, angle = 30, density = 15+50 * temp.metric / max(temp.metric))
grid()
```

In figure \ref{fig:ac-it-weights-detailed-plot} we compare importances between the two families of ordinary and derivative process models.
It appears that the latter type of models make a more even use across all segments, while ordinary models focus their attention more on some segments than on others.
In both cases, the `DESC` variable is the most important, followed by `REQ`.
Correlation and signal metrics are important for both families, with varying importance for other kinds of metrics.
This is interesting, as these metrics are comparatively cheap to compute.

For the ordinary models it does not seem important by how much PM/P actually differ in absolute terms. In other words, it appears that it is more important to quantify deviation and resemblance by metrics other than `area`/`mae` or `rmse`. Instead, it makes apparently more sense to compare PM/P as signals (ratios), as well as by how their _shape_ diverges (`corr`, `sd`/`var` are sensitive to these).


## Comparing process models

Figure \ref{fig:compare-pms} summarizes the workflow for comparing two or more process models.
This may be done with or with (labeled) observations.
Without observations but some compatibility, one of the PMs' configurations can be calibrated, in order to quantify how and by how much the other PM deviates from it.
Even without a ground truth, we can learn something about the PMs if we were to calibrate them, and that is which of the two better resembles the observations, given the chosen configuration.
This however is only a valid procedure under the assumptions that a) all observations exhibit the sought-after phenomenon, and b) they all do so to a similar degree.
If both PMs are in the same family, then we will obtain a perfect goodness score that allows to choose a champion PM within the family.
The champion PM is the one that incurs the lowest loss (or highest scores) across all unlabeled observations, which means that the champion resembles the observations best, given the same configuration as used within the family.


Another interesting case arises from having access to a somewhat richer corpus of ground truth, as we can then learn the variable importances (here: how important each calibrated score is) of a PM (see section \ref{sssec:weighted-by-opt}).
These allow us to inspect in detail, which the scores of importance are. For example, one might learn that a certain score or segment is irrelevant for capturing the phenomenon the PM describes.
Variable importances can also be further facilitated in a regression model for future observations.


```{r compare-pms, echo=FALSE, out.width="95%", fig.cap="Workflow for examining differences between two PMs. Automatic calibration allows to gain detailed insights into each PM and the used configuration.", fig.align="center", fig.pos="ht!"}
knitr::include_graphics(
  path = "../figures/compare-pms.png",
  )
```



# Qualitative Evaluation of the Rater's Notes

Although this is not specific to issue tracking (or source code for that matter), we add a section about a qualitative evaluation of the three raters' findings for each project.
The analysis is already done and is based on commit `7a4d8`[^1].
What we will do here, is to load the data into a table so that we can produce nice figures for the upcoming paper [@honel2023embedded].
It is the proper successor to our previous pilot study [@picha2022Firedrill].

[^1]: https://github.com/MrShoenel/Software-process-antipatterns-catalogue/blob/7a4d8/catalogue/Fire_Drill.md

## List of Symptoms and Consequences

The first data structure we'll employ is a list with associations Symptoms & Consequence => Empirical Instance(s).
The empirical instances will be put into a dataframe next.
...
qe_sc <- list(
  ...
  SC5 = paste0("E", 8),
  SC6 = paste0("E", 9:11),
  SC7 = c(), # No concrete instances for this one!
  
  # ESC1, ESC2, and ESC3 are new, empirical symptoms and consequences!
  ESC1 = paste0("E", 12:16),
  ESC2 = paste0("E", 17:27),
  ESC3 = paste0("E", 28:31))
```
...
## Aggregations for Empirical Instances

```{r}
make_row <- function(idx, obs, obs_agg) {
  `rownames<-`(data.frame(
    E = idx,
    
    obs = paste0(obs, collapse = ","),
    num = length(obs),
    avg = round(mean(obs), 2),
    sum = sum(obs),
    
    agg = paste0(obs_agg, collapse = ","),
    numAgg = length(obs_agg),
    meanAgg = round(mean(obs_agg), 2),
    sumAgg = sum(obs_agg),
...
split_obs <- function(obs) as.integer(strsplit(x = obs, split = ",")[[1]])
```

```{r}
qe_ei <- dplyr::bind_rows(list(
  # [1,A,1], [3,B,2], [5,A,1], [7,B,1], [9,B,2], [13,A,1], [14,A,1], [15,A,2]
  make_row( 1, c(1,2,1,1,2,1,1,2), c(1,2,1,1,2,1,1,2)),
  # [1,A,0], [15,A,0]
  make_row( 2, c(0,0), c(0,0)),
  # [3,C,1], ([13,A,3], [13,B,2], [13,C,4]) -> [13,ABC,5], [15,C,1]
  make_row( 3, c(1,3,2,4,1), c(1,5,1)),
  # [3,C,1], [6,A,1], [14,A,3]
  make_row( 4, c(1,1,3), c(1,1,3)),
  # [13,A,3]
  make_row( 5, c(3), c(3)),
  # [3,C,2]
  make_row( 6, c(2), c(2)),
  ...
  make_row( 8, c(1,2,2,2,4,2), c(3,2,2,5)),
  # [1,A,1], [3,C,4], ([5,A,1], [5,B,1]) -> [5,AB,2], [9,A,1], [10,B,2], [12,A,3],
  # ([13,A,4], [13,B,4]) -> [13,AB,5]
  make_row( 9, c(1,4,1,1,1,2,3,4,4), c(1,4,2,1,2,3,5)),
  # [3,C,4], [7,C,3], [9,C,3], [10,C,1], [14,A,1], [15,B,1]
  make_row(10, c(4,3,3,1,1,1), c(4,3,3,1,1,1)),
  # [3,C,4], [4,C,2], [7,C,1]
  make_row(11, c(4,2,1), c(4,2,1)),
  ...
  make_row(12, c(4,4,3,4,3,1,3,3,3,3,3,3,2,1,1,1,1,1,1), c(5,5,1,4,4,5,1,1,1,1,2)),
  # [3,C,3], ([10,A,3], [10,B,3]) -> [10,AB,4], ([13,A,3], [13,C,4]) -> [13,AC,5]
  make_row(13, c(3,3,3,3,4), c(3,4,5)),
  # [3,A,2]
  make_row(14, c(2), c(2)),
  # [3,A,3], [4,C,1]
  make_row(15, c(3,1), c(3,1)),
  # [9,B,2]
  make_row(16, c(2), c(2)),
  # ([3,C,3], [3,A,3]) -> [3,AC,4], [4,C,3], [6,A,2], [12,A,1]
  make_row(17, c(3,3,3,2,1), c(4,3,2,1)),
  # [3,C,2]
  make_row(18, c(2), c(2)),
  # [3,C,1], [4,A,2]
  make_row(19, c(1,2), c(1,2)),
  # ([6,A,2], [6,B,2]) -> [6,AB,3], [12,A,1], [15,A,1]
  make_row(20, c(2,2,1,1), c(3,1,1)),
  # ([6,B,2], [6,C,2]) -> [6,BC,3], ([7,A,1], [7,B,1]) -> [7,AB,2], [9,A,1], [12,C,1],
  # ([13,A,3], [13,B,2]) -> [13,AB,4]
  make_row(21, c(2,2,1,1,1,1,3,2), c(3,2,1,1,4)),
  # [6,C,1], [9,A,1], [13,C,2]
  make_row(22, c(1,1,2), c(1,1,2)),
  # [7,C,4], [13,A,4]
  make_row(23, c(4,4), c(4,4)),
  # [9,C,1], [11,A,1], [13,A,1]
  make_row(24, c(1,1,1), c(1,1,1)),
  # [10,C,1]
  make_row(25, c(1), c(1)),
  # ([12,A,3], [12,B,1]) -> [12,AB,3], ([15,A,1], [15,C,1]) -> [15,AC,2]
  make_row(26, c(3,1,1,1), c(3,2)),
  # [13,B,4], [15,A,2]
  make_row(27, c(4,2), c(4,2)),
  # [3,C,1]
  make_row(28, c(1), c(1)),
  # [4,C,1]
  make_row(29, c(1), c(1)),
  # [3,C,1], [7,C,4], [10,A,1], [15,A,1]
  make_row(30, c(1,4,1,1), c(1,4,1,1)),
  # [4,B,1]
  make_row(31, c(1), c(1))
))
```

Table \ref{tab:empirical-fd-instances} shows, for each empirical instance of a symptom/consequence, how often it has been observed and how severe it was.
The columns ending in `_agg` represent the count, sum (total severity), and average severity of the aggregated instances.
Aggregation happens if two or more raters observed the same instance.
...
if (interactive()) {
  ...
  temp <- qe_ei[,]
  temp$obs <- sapply(X = temp$obs, FUN = function(s) {
    if (nchar(s) > 14) paste0(substr(x = s, start = 1, stop = 14), " ...") else s
  })
  temp$agg <- sapply(X = temp$agg, FUN = function(s) {
    if (nchar(s) > 14) paste0(substr(x = s, start = 1, stop = 14), " ...") else s
  })
  knitr::kable(
    x = temp,
    booktabs = TRUE,
    caption = "Empirical instances of Fire Drill symptoms and consequences as found in the 15 projects.",
    label = "empirical-fd-instances"
  )
}
```


## Connect Symptoms, Consequences, and Empirical Instances
...
aggregate_e <- function(rns) {
  ...
  temp <- if (length(rns) == 0) NULL else emp_obs_df(rns = rns) %>% arrange(-mean)
  
  data.frame(
    numObs = length(all_obs),
    numAgg = length(all_obs_agg),
    min = if (length(all_obs_agg) == 0) NA_real_ else min(all_obs_agg),
    mean = if (length(all_obs_agg) == 0) NA_real_ else round(mean(all_obs_agg), 2),
    #median = if (length(all_obs_agg) == 0) NA_real_ else round(median(all_obs_agg), 2),
    max = if (length(all_obs_agg) == 0) NA_real_ else max(all_obs_agg),
    total = sum(all_obs_agg),
...
Table \ref{tab:sc-ei-overview} shows, for each of the Fire Drill's symptoms/consequences, how many types of empirical instances were observed, how often they occurred, as well as the average severity.
The last column orders the associated empirical instances from highest to lowest total severity (only shown interactively).
...
if (interactive()) {
  ...
  knitr::kable(
    x = sc_agg[, colnames(sc_agg) != "severity"],
    booktabs = TRUE,
    caption = "Association between (empirical) symptoms and consequences, and empirical instances of concrete observations.",
    escape = FALSE,
    label = "sc-ei-overview"
  )
}
```
...
```{r echo=FALSE, eval=FALSE}
pie(x = sc_agg$total, labels = sc_agg$SC)
```

```{r echo=FALSE, eval=FALSE}
temp <- sc_agg[sc_agg$total > 0,]
lp <- c(0.4870491, 0.8538340, 0.9814114, 1.0770944, 1.3163020, 2.1136605, 3.5170115, 5.2393060, 6.3237136)
lp[1] <- lp[1] - .05
lp[2] <- lp[2] - .07
lp[3] <- lp[3] - .175
lp[5] <- lp[5] + .2
lp[7] <- lp[7] + .5
lp[9] <- lp[9] + .1
plotrix::pie3D(x = temp$total, labels = temp$SC, explode = .1, theta = 1, start = .2, edges = 300, radius = 2, shade = .9)#, labelpos = lp)
```


Figures \ref{fig:fd-sc-mean} and \ref{fig:fd-sc-total} show the average (out of maximal $5$) and total severity per symptom / consequence.


```{r fd-sc-mean, echo=FALSE, warning=FALSE, fig.width=8, fig.height=5, fig.cap="Average severity for each (empirical) Symptom / Consequence."}
makefig <- function(paper = FALSE) {
  rlang::expr({
    paper <- !!paper
    temp <- rbind(data.frame(SC = sc_agg$SC, data = sc_agg$mean, num = sc_agg$numAgg, grp = "mean"))
    temp$grp <- factor(x = temp$grp, levels = c("max", "mean"), ordered = TRUE)
    
    data_labels <- as.character(temp$data)
    if (paper) {
      data_labels[!is.na(data_labels)] <- paste0("$\\,=", data_labels[!is.na(data_labels)], "$")
    }
    data_labels[is.na(data_labels)] <- "n/a"
    
    g <- ggplot(temp, aes(x = rev(grp), y = data, fill = SC)) + # y = round(100 * total / sum(total))
      ylim(0, 5) +
      facet_wrap(~ SC, scales = "fixed", nrow = 2, labeller = labeller(
        SC = `names<-`(x = paste0((if (paper) "\\texttt{"), temp$SC, (if (paper) "}" else " = "), data_labels, " (", (if (paper) "$"), temp$num, (if (paper) "$"), ")"), value = temp$SC)
      )) +
      geom_bar(width = 1, stat = "identity") +
      coord_polar("y", start = 0) +
      scale_fill_manual(values = c("#FF4A36", "#9BC94C", "#BC80BD", "#FCCDE5", "#D58932", "#DE657A",
                                   "#BEBADA", "#6F99B9", "#E6CF00", "#8DD3C7", "#ACA585"),
                        drop = FALSE,
                        labels = sapply(X = levels(temp$SC), FUN = function(l) {
                          if (paper) paste0("\\texttt{", l, "}") else l
                        })) +
      xlab("") + ylab("") + labs(fill = "Symptom / Consequence", title = "Average severity per symptom and consequence.") +
    ...
    if (paper) {
      g <- g + gg2_style(FALSE) +
        theme(
          ...
          legend.title = element_text(size = 9, margin = ggplot2::margin(r = 10)),
          legend.text = element_text(size = 8, margin = ggplot2::margin(l = -4, r = 3)),
          legend.position = "bottom",
          legend.margin = ggplot2::margin(t = -1, unit = "line"),
          strip.background = element_blank(),
          strip.text = element_text(size = 9),
          panel.spacing = unit(1.2, "lines"),
          panel.background = element_rect(linetype = "blank", fill = "white"),
          panel.grid.major = element_line(linetype = "dotted", color = "#aaaaaa", size = .6),
          panel.grid.minor = element_line(linetype = "dotted", color = "#cccccc", size = .5),
          plot.margin = ggplot2::unit(x = c(-.5, 0, -1, -1.3), units = "line"),
          plot.title = element_text(size = 9, face = "bold", hjust = 0.5, margin = ggplot2::margin(b = 1.5, unit = "line"))) +
      guides(fill = guide_legend(nrow = 2, byrow = TRUE, override.aes = list(size = 1)))
    } else {
      g <- g + theme(axis.text.y = element_blank(),
                     legend.position = "bottom",
                     panel.background = element_rect(fill = "#fbfbfb", linetype = "blank"),
                     panel.grid.major = element_line(linetype = "dotted", color = "#aaaaaa"),
                     panel.grid.minor = element_line(linetype = "dotted", color = "#cccccc"))
    }
...
```{r echo=FALSE, eval=FALSE}
tikzDevice::tikz(file = paste0("../figures/fig_", "it_fd-sc-mean.tex"), width = FIG_WIDTH, height = 3.9, verbose = FALSE)
eval(makefig(paper = TRUE))
dev.off()
```



```{r fd-sc-total, echo=FALSE, warning=FALSE, fig.width=8, fig.height=5, fig.cap="Total severity and number of (aggregated) observations for each (empirical) Symptom / Consequence."}
makefig <- function(paper = FALSE) {
  rlang::expr({
    paper <- !!paper
    temp <- rbind(data.frame(SC = sc_agg$SC, data = sc_agg$total, num = sc_agg$numAgg, grp = "mean"))
    temp$grp <- factor(x = temp$grp, levels = c("max", "mean"), ordered = TRUE)
    
    g <- ggplot(temp, aes(x = SC, y = data, fill = SC)) +
      geom_bar(width = 0.8, stat = "identity") +
      # https://stackoverflow.com/questions/17311917/ggplot2-the-unit-of-size
      geom_text(aes(label = paste0((if (paper) "$"), data, (if (paper) "$"), " (", (if (paper) "$"), num, (if (paper) "$"), ")")), stat = "identity", vjust = -1, size = 9*0.352777778) +
      scale_fill_manual(values = c("#FF4A36", "#9BC94C", "#BC80BD", "#FCCDE5", "#D58932",
                                   "#DE657A", "#BEBADA", "#6F99B9", "#E6CF00", "#8DD3C7", "#ACA585"),
                        labels = sapply(X = levels(temp$SC), FUN = function(l) {
                          if (paper) paste0("\\texttt{", l, "}") else l
                        })) +
      xlab("") + ylab("") + labs(fill = "Symptom / Consequence", title = "Total severity per symptom and consequence.") +
      theme(legend.position = "bottom") +
      guides(fill = guide_legend(nrow = 2, byrow = TRUE))
    
    if (paper) {
      g <- g + gg2_style(FALSE) +
        scale_y_continuous(limits = c(0, 63), breaks = seq(0, 100, by = 10), labels = seq(0, 100, by = 10)) +
        scale_x_discrete(labels = sapply(X = levels(temp$SC), FUN = function(l) {
                          if (paper) paste0("\\texttt{", l, "}") else l
                        })) +
        theme(
          legend.title = element_text(size = 9, margin = ggplot2::margin(r = 10)),
          ...
          panel.spacing = unit(1.2, "lines"),
          panel.background = element_rect(linetype = "blank", fill = "white"),#element_rect(linetype = "blank", fill = "#fbfbfb"),
          panel.grid.major = element_line(linetype = "dotted", color = "#aaaaaa", size = .6),
          panel.grid.minor = element_line(linetype = "dotted", color = "#cccccc", size = .5),
          plot.margin = ggplot2::unit(x = c(0, 0, 0, -1), units = "line"),
          plot.title = element_text(size = 9, face = "bold", hjust = 0.5, margin = ggplot2::margin(b = 1.5, unit = "line"))) +
      guides(fill = guide_legend(nrow = 2, byrow = TRUE, override.aes = list(size = 1)))
    } else {
      g <- g + ylim(0, 62)
...
```{r echo=FALSE, eval=FALSE}
tikzDevice::tikz(file = paste0("../figures/fig_", "it_fd-sc-total.tex"), width = FIG_WIDTH, height = 3.1, verbose = FALSE)
eval(makefig(paper = TRUE))
dev.off()
```
...
```{r echo=FALSE, eval=FALSE}
temp <- rbind(
  cbind(sc_agg, data.frame(grp = 1, data = sc_agg$mean)),
  cbind(sc_agg, data.frame(grp = 2, data = sc_agg$total)))


ggplot(temp, aes(x = grp, y = data, fill = SC)) + # y = round(100 * total / sum(total))
  facet_wrap(~ SC, ncol = 10, strip.position = "bottom") +
  geom_bar(width = 1, stat = "identity") +
  #scale_fill_manual(values = ggsci::pal_igv()(n = 10)) +
  scale_fill_manual(values = c("#FF4A36", "#9BC94C", "#BC80BD", "#FCCDE5", "#D58932", "#DE657A", "#BEBADA", "#6F99B9", "#E6CF00", "#8DD3C7", "#ACA585")) +
  gg2_style() +
  xlab("") + ylab("") + labs(fill = "Symptom / Consequence")
```

```{r echo=FALSE, eval=FALSE}
temp <- NULL
for (n in names(qe_sc)) {
  data <- data.frame(
    data = do.call("c", lapply(X = qe_sc[[n]], function(rn) {
      split_obs(qe_ei[rn,]$obs_agg)
    }))
  )
  if (nrow(data) > 0) {
    data$SC <- n
...
temp$SC <- factor(x = temp$SC, levels = levels(sc_agg$SC), ordered = TRUE)
```


```{r echo=FALSE, eval=FALSE}
ggplot(temp, aes(x = data, y = SC, fill = rev(SC))) +
...
```{r echo=FALSE, eval=FALSE}
set.seed(1)
ggplot(temp, aes(x = SC, y = data, fill = SC)) +
  geom_violin() +
  geom_jitter(position = position_jitter(.1), show.legend = FALSE) +
