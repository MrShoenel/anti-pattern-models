...
TODO: MOVE THIS SECTION p.g.a. `p3_it_scores`
...
```{r var-imp-all-features, echo=FALSE, fig.height=4.5, fig.cap="Variable-importance for all features (activities) for predicting the ground truth.", fig.align="center", fig.pos="ht!"}
plot(imp_it_all)
```
...
modelFit_it_req <- temp$req
modelFit_it_dev <- temp$dev
modelFit_it_desc <- temp$desc

imp_it_req <- caret::varImp(object = modelFit_it_req)
imp_it_dev <- caret::varImp(object = modelFit_it_dev)
imp_it_desc <- caret::varImp(object = modelFit_it_desc)
```


When comparing figures \ref{fig:var-imp-all-features} and \ref{fig:var-imp-all-features-sep}, we observe that


```{r var-imp-all-features-sep, echo=FALSE, fig.height=4.5, fig.cap="Variable-importance for all features (activities) separately for predicting the ground truth on a per-activity basis.", fig.align="center", fig.pos="ht!"}
temp <- imp_it_req
temp$importance <- rbind(temp$importance, imp_it_dev$importance, imp_it_desc$importance)
plot(temp)
```
