...
ac_itp1_desc1_ecdf <- function(x) x # Since all have same likelihood
```
...
```{r}
ac_p1_dr <- function(
  projName, signals = all_signals, expectations = c(
...
ac_it_dr_scores <- `colnames<-`(`rownames<-`(matrix(ncol = 3, nrow = length(temp), byrow = FALSE, data = c(
...
)), names(temp)), c("all_equal", "pref_reqt1", "ground_truth"))
```
...
The scores from the automatically calibrated decision rule are shown in table \ref{tab:ac_it_dr_scores}.
...
```{r}
temp <- cbind(ac_it_dr_scores, data.frame(ground_truth = c(ground_truth$consensus_score, ground_truth_2nd_batch$consensus_score)))
  
`names<-`(c(
  Metrics::mse(temp$all_equal, temp$ground_truth),
  Metrics::mse(temp$pref_reqt1, temp$ground_truth)
...
eval_f <- function(x) {
  res <- sapply(X = names(temp), function(pName) {
    ac_p1_dr(projName = pName, signals = temp, w_reqt1 = x[1], w_devt1 = x[2], w_devt2 = x[3], w_desc1 = x[4])
  })
  # Use this instead to maximize the correlation:
  # 1 - cor(temp.gt, res)
  Metrics::mse(actual = temp.gt, predicted = res)
}
...
The weights found by the optimization are:

```{r}
`names<-`(res$solution, c("reqt1", "devt1", "devt2", "desc1"))
```

It appears we should not put any weight on the indicator for the `DESC`-variable at $t=1$.
...
These results are a little worse than for the first batch ($\approx0.133$ and $\approx0.4$). Let's compute the scoring rules for all projects together, the results should be somewhere in between:
...
ac_grid_results_it <- loadResultsOrCompute(file = "../results/ac_grid_results_it.rds", computeExpr = {
  ...
  cl <- parallel::makePSOCKcluster(124)
  ...
  doWithParallelClusterExplicit(cl = cl, stopCl = TRUE, expr = {
    ...
    foreach::foreach(
      ...
      pm_var <- switch (as.character(params$PM),
        "I" = p1_it_signals,
        "II(a)" = p2_it_signals,
        "III" = p3_it_signals,
