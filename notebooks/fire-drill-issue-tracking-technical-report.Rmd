...
Unlike the optimized models for issue-tracking data, we have a clear champion model here, type IV(II(a)) (time-derivative model of type II(a)).
It has the highest score for the best project, the lowest score for the worst project (and hence also the largest spread), as well as the lowest MSE- and Log-scores (cf. table \ref{tab:ac-compare-pms-it-optim}).
With an RMSE of $\approx0.16$ we should be close to or have a usable (in practice) model.

A general observation is that the derivative version of each PM is always better in any regard (best/worst, spread, (R)MSE- and Log-score) when compared to its non-derivative ancestral model.
...
If we inspect the metrics used in the champion model, it unveils the following (table \ref{tab:ac-it-details-pm-iv-iia}):
Except for one usage of `corr`, all other metrics are signal theory metrics, with the `ImpulseFactor` being used $5$ out of $8$ times!
...
Derivative PMs and observations can result in signals with extreme amplitudes, and the excessive use of signal theory metrics could indicate that such extremes are suitable and more appropriate in derivative scenarios.
