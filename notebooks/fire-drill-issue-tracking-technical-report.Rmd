...
The results of table \ref{tab:ac-compare-pms-it} are quite OK on the surface, as all PMs consistently produce a higher score for the best project than they do for the worst.
However, all the scores are quite bad, so there is room for improvement in terms of selecting the right objectives (weights).
...
temp.objLog <- function(x) {
  ...
  mean(scoring::logscore(object = ground_truth ~ pred, data = temp.pred))
}

temp.objMSE <- function(x) {
  ...
  loadResultsOrCompute(file = paste0("../results/ac_it_weights_pm_", pmName, (if (useLog) "_Log"), ".rds"), computeExpr = {
    ...
    doWithParallelClusterExplicit(cl = cl, expr = {
      optimParallel::optimParallel(
        ...
        fn = function(x) {
          if (useLog) temp.objLog(x) else temp.objMSE(x)
        },
        parallel = list(cl = cl, forward = FALSE, loginfo = TRUE)
      )
    })
...
Similarly to the source code PMs, it appears that the expert-designed models of type I are also the worst after optimization, requiring many weights $>0$ to achieve a low (R)MSE.
Running the minimization with the Log-score in significantly smaller models for the non-derivative PMs. However, this was just a test, and we should not derive any conclusions from the Log-score results. This is because technically, the Log-score assumes mutually-exclusive, discrete classes (which we do not have). So the test was wether or not some optimization can be reached using another objective.
...
The results of table \ref{tab:ac-compare-pms-it-optim} indicate that there is no clear winner, although there are some remarkable results.
First of all, the MSE of the derivative models that were also optimized using the MSE is consistently lower compared to their non-derivative counterparts.
This means that we can make the general statement that using a derivative version of any PM is better for indicating the degree to which the Fire Drill is present in a project. It is also likely that the derivative models resemble the derivative observations better, or that the evaluated metrics possess more explanatory power in a derivative scenario.
Secondly, using the Log-score as objective during optimization, it is possible to actually achieve comparatively low Log-scores, sometimes even in combination with a low MSE.
The optimization increased the margin between the scores for worst/best project for all PMs significantly.
One of the perhaps best results is PM type IV(II(a)) using MSE, as it has the largest margin, lowest Log-score, and is along the PMs with lowest MSE.
An RMSE $<\approx0.2$ means that the models is perhaps already usable in practice.


#### Detailed results derivative models

If we inspect the metrics used in the picked champion model (IV(II(a))/MSE), we can make some very interesting observations (table \ref{tab:ac-it-details-pm-iv-iia}).
Using only six objectives in total, only one of them is the `ImpulseFactor`, while the other five are correlation.
Remember that the derivative depicts the rate of change, which means that absolute differences such as `area`, `mae`, or `rmse` become useless the more similar two curves are, since all curves now have an intercept of $0$.
Derivative PMs and observations can result in signals with extreme amplitudes.
So apparently, comparing rate-of-change models is done most effectively using the correlation.
If we translate this to our concrete use case, it means that if a project's activities' rate of change (i.e., in-/decrease) correlates well with how the PM models the rate of change, then the Fire Drill is likely present (quite logical). It could however also indicate that we did a better job designing how the variables change, while we did a worse job designing the actual magnitudes.

Furthermore, this model does not cover the segments $\{1,3,6,9\}$, but is still able to detect the presence (or absence) of the Fire Drill with great confidence.
...
```{r echo=FALSE}
temp <- ac_weight_grid_it[ac_it_weights_pm_IV_IIa$par > 0, ]
temp$weight <- round(ac_it_weights_pm_IV_IIa$par[ac_it_weights_pm_IV_IIa$par > 0], 3)
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    row.names = FALSE,
    caption = "The metrics used in PM IV(II(a)). All but one metric are metrics from signal theory.",
    label = "ac-it-details-pm-iv-iia"
  )
}
```
...
```{r ac-it-weights-detailed-plot, echo=FALSE, fig.height=8, fig.cap="Average and total importances of scores per segment, variable, and metric, for derivative process models (optimized using MSE)."}
...
Interestingly, the derivative models only use the correlation and the impulse factor, with correlation being much more frequently used (see figure \ref{fig:ac-it-weights-detailed-plot}).
This is interesting, because those two metrics are cheap to compute and straightforward to understand.
