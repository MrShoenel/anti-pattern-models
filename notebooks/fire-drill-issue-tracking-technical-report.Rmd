...
for (confName in c("all_equal", "pref_reqt1")) {
  ...
  temp.df <- rbind(temp.df, data.frame(
    Config = confName,
    Score_for_best = ac_p1_dr(signals = temp, projName = paste0("Project", which.max(temp.gt))),
    Score_for_worst = ac_p1_dr(signals = temp, projName = paste0("Project", which.min(temp.gt))),
    Score_mean = mean(temp.pred$pred),
...
for (pmName in c("I", "II(a)", "III", "IV(I)", "IV(II(a))", "IV(III)")) {
  ...
  temp.df <- rbind(temp.df, data.frame(
    ...
    Score_low = min(temp.pred$pred),
    Score_high = max(temp.pred$pred),
    Score_mean = mean(temp.pred$pred),
    RMSE = sqrt(Metrics::mse(actual = temp.gt, predicted = temp.pred$pred)),
    MSE = Metrics::mse(actual = temp.gt, predicted = temp.pred$pred),
    Log = mean(scoring::logscore(object = ground_truth ~ pred, data = temp.pred))
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(temp.df$PM, round(temp.df[, 2:ncol(temp.df)], 4)),
    booktabs = TRUE,
...
if (interactive()) {
  ...
  knitr::kable(
    x = cbind(data.frame(
      PM = temp.df$PM, Obj = temp.df$Obj), round(temp.df[, c(2:3, 5:6)], 4)),
    booktabs = TRUE,
