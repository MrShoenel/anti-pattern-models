...
curve(desc_p1a, 0, 1, add = TRUE, col = "#00cc00", lty = 1, lwd = 3)
...
This is the same approach we undertook for pattern type III (average) for the Fire Drill in source code. However, we will also learn an __empirical confidence interval__, which is later used for an additional detection method.

```{r}
p3_weighted_var <- function(name) {
  funcs <- list()
  for (pId in names(all_signals)) {
...
```{r}
req_p3 <- p3_weighted_var(name = "REQ")
dev_p3 <- p3_weighted_var(name = "DEV")
```
...
legend(0, 1, legend = c("cum. emp. req%", "cum. emp. dev%"), col = c("blue", "red"), lty = 1, lwd = 2)
```
...
legend(0.61, .31, legend = c("Pt. 3 (avg) upper CI", "Pt. III (avg) lower CI", "Pt. III (avg) req%"),
       col = "red", lty = c(3, 2, 1), lwd = c(1, 1, 2))
...
CI_p3avg <- function(x, y, funclist, f_ci_upper, f_ci_lower, gbar) {
...
CI_req_p3avg <- function(x, y) CI_p3avg(x = x, y = y, funclist = funclist_REQ, f_ci_upper = req_ci_upper_p3avg, f_ci_lower = req_ci_lower_p3avg, gbar = req_p3)
CI_dev_p3avg <- function(x, y) CI_p3avg(x = x, y = y, funclist = funclist_DEV, f_ci_upper = dev_ci_upper_p3avg, f_ci_lower = dev_ci_lower_p3avg, gbar = dev_p3)
```
...
compute_z_p3avg <- function(varname) {
  ...
  f <- if (varname == "REQ") CI_req_p3avg else CI_dev_p3avg
...
Finally, we show the empirical confidence intervals in figure \ref{fig:p3-emp-cis}. Note that the minimum non-zero confidence is `r round(min(z_req[z_req>0]),6)`, while the maximum is $1$. Therefore, while we gradate the colors from $0$ to $1$, we slightly scale and transform the grid's non-zero values using the expression $0.9\times z+0.1$, to improve the visibility.
...
For the variable `REQ` we get a weaker, yet moderate correlation of $\approx-0.50$, and there is no significant evidence for the null hypothesis either, so must reject it.
...
The correlation for the variable `DEV` is poor again. Also, it is positive, which means that the scores calculated using this method are proportional, when they should not be.
...
temp <- data.frame(
  ...
  ci = p3_avg_ci_scores$REQ,
  area = p3_avg_area_scores$REQ,
  ci_dev = p3_avg_ci_scores$DEV,
  area_dev = p3_avg_area_scores$DEV)

p3_avg_lm <- stats::lm(formula = gt_consensus ~ ci + area + ci_dev + area_dev, data = temp)
...
$$
\begin{aligned}
  x_1,x_2,\operatorname{req},\operatorname{dev}\dots&\;\text{lower/upper integration interval and project signals,}
  \\[1ex]
  \bm{\tau}=&\;\Big\{\mathit{L}^{\text{avgconf}}(x_1,x_2,\operatorname{req}),\;\mathit{L}^{\text{areadist}}(x_1,x_2,\operatorname{req}),\;\mathit{L}^{\text{avgconf}}(x_1,x_2,\operatorname{dev}),\;\mathit{L}^{\text{areadist}}(x_1,x_2,\operatorname{dev})\Big\}\;\text{,}
  \\[1ex]
  \operatorname{detect}^{\text{ci+area}}(\bm{\tau})=&\;-1.889 + 5.786\times\bm{\tau}_1 + 1.149\times\bm{\tau}_2 + 1.431\times\bm{\tau}_3 + 0.585\times\bm{\tau}_4\;\text{.}
\end{aligned}
$$
...
With this linear combination, we were able to significantly boost the detection to $\approx0.94$, which implies that combining both methods is of worth for a detection using inhomogeneous confidence intervals only. If we only combine the scores of the variable `REQ` into a model, we still achieve a correlation of $\approx0.88$. This should probably be preferred to keep the degrees of freedom low, countering overfitting.
