...
#### Detailed results derivative models

If we inspect the metrics used in the picked champion model (III/MSE), we can make some very interesting observations (table \ref{tab:ac-it-details-pm-iii}).
...
temp$weight <- round(ac_it_weights_pm_I$par[ac_it_weights_pm_I$par > 0], 3)
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    row.names = FALSE,
    caption = "The metrics used in PM III. All but one metric are metrics from signal theory.",
    label = "ac-it-details-pm-iii"
  )
}
```
...
```{r echo=FALSE}
#temp <- cbind(ac_weight_grid_it, data.frame(weight = ac_it_weights_pm_IV_I$par + ac_it_weights_pm_IV_IIa$par + ac_it_weights_pm_IV_III$par))
