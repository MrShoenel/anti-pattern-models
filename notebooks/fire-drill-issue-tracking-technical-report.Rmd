...
curve(desc_p3, 0, 1, col = "gold", add = TRUE)
legend(0, 1, legend = c("cum. emp. req%", "cum. emp. dev%", "cum. emp. desc%"), col = c("blue", "red", "gold"), lty = 1, lwd = 2)
```
...
curve(desc_p3, 0, 1, col = "gold", add = TRUE)
curve(desc_ci_upper_p3avg, 0, 1, col = "#ffd70033", add = TRUE, lty = 2)
curve(desc_ci_lower_p3avg, 0, 1, col = "#ffd70033", add = TRUE, lty = 3)

legend(0.01, .98, legend = c("Pt. 3 (avg) desc% upper CI", "Pt. III (avg) desc% lower CI", "Pt. III (avg) desc%"),
       col = "gold", lty = c(3, 2, 1), lwd = c(1, 1, 2))
```
...
image(x = x, y = y, z = z_desc, main = "Variable: DESC",
      xlab = "Relative time", ylab = "Relative cumulative time spent",
      col = colorRampPalette(colors = c("#ffffff", "#FFD700"))(100), zlim = c(0, max(z_desc)))
grid()
curve(desc_p3, 0, 1, col = "#eebb00", add = TRUE)
