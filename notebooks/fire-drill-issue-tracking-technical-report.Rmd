...
This method requires three features: A reference-variable, an upper- and a lower confidence interval. However, none of these are required to be of empirical nature. For example, we can even apply this method to the first of our patterns. A reference-variable may be generated as the average between the confidence intervals, etc. All this makes this method versatile. Its robustness comes from the fact any variable it scores, is confined to the boundaries of the confidence intervals.
...
```{r echo=FALSE}
p3_avg_area_scores_p1 <- loadResultsOrCompute(file = "../results/p3_avg_area_scores_p1.rds", computeExpr =  {
  doWithParallelCluster(numCores = length(all_signals), expr = {
    library(foreach)
...
```{r p3-avg-area-scores-p1, echo=FALSE, fig.height=8, fig.cap="All projects plotted against the two variables req\\% and dev\\% of the first pattern.", fig.align="center", fig.pos="ht!"}
par(mfrow=c(2,1))
...
```{r echo=FALSE}
p3_avg_area_scores_p2a <- loadResultsOrCompute(file = "../results/p3_avg_area_scores_p2a.rds", computeExpr =  {
...
```{r p3-avg-area-scores-p2a, echo=FALSE, fig.height=8, fig.cap="All projects plotted against the two variables req\\% and dev\\% of pattern type II (a).", fig.align="center", fig.pos="ht!"}
par(mfrow=c(2,1))

curve(req_p2a, 0, 1, col="red", lwd=2)
...
curve(dev_p2a, 0, 1, col="blue", lwd=2)
tempf <- function(x) rep(0, length(x))
curve(tempf, 0, 1, col="blue", lty=2, lwd=1.5, add = TRUE)
...
```{r echo=FALSE}
if (interactive()) {
  p3_avg_area_scores_p2a
} else {
  knitr::kable(
    x = p3_avg_area_scores_p2a,
    booktabs = TRUE,
    caption = "Correlation of the scores as computed per the average distance to the averaged variable.",
    label = "p3-avg-area-scores-p2a"
  )
}
```
