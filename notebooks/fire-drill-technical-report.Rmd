...
ac_grid_results <- loadResultsOrCompute(file = "../results/ac_grid_results_sc.rds", computeExpr = {
  ...
  doWithParallelCluster(numCores = min(2, parallel::detectCores()), expr = {
    ...
    foreach::foreach(
      grid_idx = sample(rownames(ac_grid), 2),
      .combine = rbind,
      .inorder = FALSE,
      .verbose = TRUE,
...
ac_extract_data <- function(pmName, varName, segIdx, metricName, ac_grid, ac_grid_results) {
  ...
  ac_grid_results[ac_grid_results$SegIdx == segIdx & ac_grid_results$grid_idx %in% rownames(rows), metricName]
}
```
...
```{r}
ac_pmp_score <- function(pmName, projName, results_uniform) {
  temp <- results_uniform[
    results_uniform$PM == pmName &
    results_uniform$Project == projName, as.character(temp.Metric)]
...
ac_pmp_score_weighted <- function(pmName, projName, weightGrid, weightVector, results_uniform) {
  ...
  for (i in 1:length(weightVector)) {
    ...
    score <- results_uniform[
      results_uniform$PM == pmName &
      results_uniform$Project == projName &
      results_uniform$Var == params$Var &
      results_uniform$SegIdx == params$SegIdx, params$Metric]
...
In summary, the conclusions that can be drawn from the finding the optimal weights depend on the assumptions we are certain to make. So for example, if we are confident about our ground truth, it indicates a flaw in, e.g., the model (expectation) or the (choice of) objectives.
