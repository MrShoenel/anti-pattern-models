...
for (vartype in names(weight_vartype)) {
  ...
  p2_signals[[vartype]] <- Signal$new(
    name = paste0("p2_", vartype), support = c(0, 1), isWp = TRUE, func = temp$M)
}
```
...
for (vartype in names(weight_vartype)) {
  f <- Vectorize(p2_signals[[vartype]]$get0Function())
...
The 2nd pattern, as derived from the ground truth, is shown in \ref{fig:p2-signals}.

```{r p2-signals, echo=FALSE, fig.cap="Second pattern as aligned by the ground truth."}
plot_project_data(data = temp, boundaries = p2_best_thetaB[2:4])
```

While this worked I suppose it is fair to say that our initial pattern is hardly recognizable. Since we expected this, we planned for a third kind of pattern in section \ref{sec:pattern2}, that is purely evidence-based. It appears that, in order to match the ground truths we have at our disposal, projects register some kind of initial peak, that is followed by a somewhat uneventful second and third interval. Interestingly, the optimization seemed to have used to mostly straight lines in the Long Stretch phase to model linear declines and increases, before distorting what was the Fire Drill phase into a new kind of Long Stretch. The new Aftermath phase is just as long, with all activities increasing to their maximum, and without the chance of any decline before the last observed commit. We will check how this adapted pattern fares in section \ref{ssec:score-pattern2}.



## Pattern III: Evidence-based\label{sec:pattern2}
...
```{r score-mech}
score_variable_alignment = function(alignment, use = c("area", "corr", "jsd", "arclen", "signals")) {
  use <- match.arg(use)
  ...
  for (vartype in vartypes) {
    # The names of the 2 variables to compare. Which one is WP
    # and which one is WC does not matter for our purpose.
    sigNames <- names(sigs)[grepl(pattern = paste0(vartype, "$"), x = names(sigs))]
    sig1 <- sigs[[sigNames[1]]]
    sig2 <- sigs[[sigNames[2]]]
    stopifnot(all.equal(sig1$getSupport(), sig2$getSupport()))
    stopifnot(sig1$getSupport()[1] == 0 && sig1$getSupport()[2] == 1)
    temp <- NA_real_
    
    if (use == "area") {
      temp <- area_diff_2_functions_score()(
        f1 = sig1$get0Function(), f2 = sig2$get0Function())
    } else if (use == "corr") {
      tempsd <- stat_diff_2_functions(f1 = sig1$get0Function(), f2 = sig2$get0Function())
      ...
      temp <- stat_diff_2_functions_symmetric_JSD_score()(
        f1 = sig1$get0Function(), f2 = sig2$get0Function())
    } else if (use == "arclen") {
      temp <- stat_diff_2_functions_arclen_score()(
        f1 = sig1$get0Function(), f2 = sig2$get0Function())
    } else if (use == "signals") {
      temp <- prod(stat_diff_2_functions_signals_score()(
        f1 = sig1$get0Function(), f2 = sig2$get0Function()))
    }
...
```{r}
compute_all_scores <- function(alignment) {
  useScores <- c("area", "corr", "jsd", "arclen", "signals")
  
  cl <- parallel::makePSOCKcluster(length(alignment))
  `rownames<-`(doWithParallelClusterExplicit(cl = cl, expr = {
    foreach::foreach(
      projectName = sort(names(alignment)),
      .inorder = TRUE,
      .combine = rbind
    ) %dopar% {
      scores <- c()
      for (score in useScores) {
        ...
        scores <- c(scores, `names<-`(
          c(mean(temp), prod(temp)), paste0(score, c("_m", "_p"))))
      }
      scores
    }
...
time_warp_project <- function(pattern, project) {
...
#   temp <- time_warp_project(pattern = p1_signals, project = project_signals$project_1)
...
p1_scores <- loadResultsOrCompute(file = "../results/p1_scores.rds", computeExpr = {
  as.data.frame(compute_all_scores(alignment = p1_align))
})
```
...
Let's show a correlation matrix:
