...
compute_segment_metric <- function(
  ...
  if (use == "area") {
    ...
    diff <- 1 - stat_diff_2_functions_cor_score(requiredSign = 0, numSamples = numSamples)(f1 = f1, f2 = f2)
  } else if (use == "jsd") {
...
get_smoothed_curve_buffered <- function(seed = NA, npoints = 15, include_deriv = FALSE) {
  if (!include_deriv) {
    # We only buffer those with derivative
    return(get_smoothed_curve(seed = seed, npoints = npoints, include_deriv = FALSE))
  }
  
  lock_file <- paste0(tempdir(), "/gsc.lock")
  lock <- filelock::lock(path = lock_file, exclusive = TRUE)
  
  tryCatch(expr = {
    gsc_file <- paste0(tempdir(), "/gsc_", seed, "_", npoints, ".rds")
  
    if (file.exists(gsc_file)) {
      readRDS(file = gsc_file)
    } else {
      sc <- get_smoothed_curve(seed = seed, npoints = npoints, include_deriv = TRUE)
      saveRDS(object = sc, file = gsc_file, compress = FALSE)
      sc
    }
  }, finally = {
...
```{r}
ac_grid <- expand.grid(list(
  PM = c("I", "II", "III(avg)", "III(b,11)")[1],
  Var = names(weight_vartype)[1],
  Seed = seq(from = 1, length.out = 2),#1e4),
  Seg_idx = 1:10))
...
ac_grid_results <- loadResultsOrCompute(file = "../results/ac_grid_results_sc.rds", computeExpr = {
  ...
  doWithParallelCluster(numCores = 4, expr = {
    ...
    foreach::foreach(
      ...
      `rownames<-`(ac_worker(PM = pm_var, P = p_var, seg_idx = params$Seg_idx), grid_idx)
    }
  })
})
```
