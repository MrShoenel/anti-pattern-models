...
The average importance per variable in figure \ref{fig:ac-sc-weights-detailed-plot} favored `A` and `FREQ`, then `SCD` and then with some distance `CP`. In figure \ref{fig:ac-sc-weights-detailed-plot-counts} however, `A`, `CP`, and `FREQ` have almost the same number of scores on them $\approx27$, while `SCD` has almost $10$ more. Requiring more scores on a variable could indicate it having a large variance, for example. In other words, if relatively more scores are required in a segment, then those are probably required to accommodate the larger variances.
