...
```{r}
temp <- matrix(nrow = 15, ncol = 521)
temp[, 521] <- temp.gt

for (i in 1:15) {
  projName <- names(append(project_signals, project_signals_2nd_batch))[i]
  for (j in 1:520) {
...
colnames(temp) <- c(paste0("w", 1:520), "gt")
```
...
Model type III(b, numInt=11) even only requires the usage of 17 weights ($\approx3.27$% of all objectives), but is simultaneously the 2nd-hardest to optimize. Its MSE is the best and comparable to that of model type II(a), which is also expected, since this type of PM was crafted out of no expectation, and then simply aligned to all projects using a weighted average.
Recall that this type of model consists of piece-wise linear function, which means no perturbations. This could explain why it is the best in terms of (R)MSE and number of weights as its nature means it is simple, yet effective.
