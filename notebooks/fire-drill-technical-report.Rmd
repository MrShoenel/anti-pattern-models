...
All complementary data and results can be found at Zenodo. This notebook was written in a way that it can be run without any additional efforts to reproduce the results. This notebook has a canonical URL[^1] and can be read online as a rendered markdown[^2] version. All code can be found in this repository, too.
...
for (numIntervals in c(1:16)) {
  loadResultsOrCompute(file = paste0("../results/p3-compute/i_", numIntervals, ".rds"), computeExpr = {
    p3_vars <- c("A", "CP", "FREQ", "SCD")
...
for (tempPath in gtools::mixedsort(
  Sys.glob(paths = paste0(getwd(), "/../results/p3-compute/*.rds")))
) {
  ...
  p3_params <- rbind(p3_params, data.frame(
    ...
    neglogLik = -log(1 / (.Machine$double.eps + exp(temp$optR$value))),
...
This is the same process as for pattern II, as the parameters need inversion. We will reconstruct the warped signals according to the inversed parameters to produce the third pattern. According to the overview above, the best model is the one with __`r p3_params[which.min(p3_params$AIC), ]$numIntervals`__ intervals. Its parameters are the following:
...
```{r p3-signals, echo=FALSE, fig.cap="Third pattern as aligned by the ground truth only.", fig.align="top", fig.pos="ht!"}
plot_project_data(data = temp, boundaries = p3_best_thetaB[2:(length(p3_best_thetaB) - 1)])
```
...
```{r score-mech}
score_variable_alignment <- function(alignment, patternName, projectName, use = c("area", "corr", "jsd", "kl", "arclen", "sd", "var", "mae", "rmse", "RMS", "Kurtosis", "Peak", "ImpulseFactor")) {
  use <- match.arg(use)
...
compute_all_scores <- function(alignment, patternName) {
  useScores <- c("area", "corr", "jsd", "kl", "arclen", "sd", "var", "mae", "rmse", "RMS", "Kurtosis", "Peak", "ImpulseFactor")
...
```{r echo=FALSE}
temp <- stats::cor(ground_truth$consensus, p1_scores)
...
p3_avg_align <- loadResultsOrCompute(file = "../results/p3_avg_align.rds", computeExpr = {
  ...
  unlist(doWithParallelClusterExplicit(cl = cl, expr = {
    foreach::foreach(
      ...
      cl_nested <- parallel::makePSOCKcluster(4)
      `names<-`(list(doWithParallelClusterExplicit(cl = cl_nested, expr = {
        temp <- time_warp_project(
...
The third and last pattern is based on the ground truth only. Starting with straight lines and equally long intervals, a pattern was generated and selected using a best trade-off between number of parameters and highest _likelihood_.
