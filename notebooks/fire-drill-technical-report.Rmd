...
"Automatic calibration" refers to a new approach of gauging a continuous PM, so that it can compute true scores for any process, also scores that are __comparable__ across PMs, given the same configuration (segments, objectives, regularizers) was used.
This approach is based on the idea of __rectification of scores__[^3]. The goal is to learn for every objective a __uniform__ score, where uniform refers to both, a uniform distribution _and_ a codomain of $[0,1]$. With such scores, a linear scalarizer will produce a score with the __same__ properties, and that means we will be able to compare the continuous process models we formulated so far! Even beyond that, we can interpret this score as a probabilistic measure of how well a process matches the PM, so in our case we can translate this information into the degree to which a Fire Drill is present in a project, according to the process model in question. We will also be able to compute the Brier- and Logarithmic scoring rules, and compare against the results that we got using the binary decision rule.
...
We make a caching version of getting a smoothed random process, as obtaining a precise numeric derivative is expensive, and we do not want to repeat this.
...
ac_grid_results <- loadResultsOrCompute(file = "../results/ac_grid_results_sc.rds", computeExpr = {
  ...
  doWithParallelCluster(numCores = 124, expr = {
    ...
    foreach::foreach(
      ...
      p_var <- get_smoothed_curve_buffered(seed = params$Seed, include_deriv = FALSE)$f0
...
ac_extract_data <- function(pmName, varName, segIdx, metricName, ac_grid, ac_grid_results) {
  ...
  ac_grid_results[ac_grid_results$seg_idx == segIdx & ac_grid_results$grid_idx %in% rownames(rows), metricName]
}
```

So let's plot a few randomly picked examples:

```{r ac-grid-example, echo=FALSE, fig.height=5, fig.cap="Four randomly picked ECDFs as simulated using the random processes of the automatic calibration."}
...
So as we see in figure \ref{fig:ac-grid-example}, the ECDFs can be quite diverse. In almost any case, the ECDF is __non-linear__, which is exactly what we are after, as it will then be used to rectify the corresponding objective later.
  ...
  doWithParallelCluster(numCores = 16, expr = {
    ...
    foreach::foreach(
      ...
      for (i in 1:length(project_signals_all)) {
        res[1, i] <- tempf(ac_grid_projects_results[
          ...
          ac_grid_projects_results$seg_idx == params$SegIdx, params$Metric])
      }
...
The results as computed are in a bit of an unpractical format, we need to do some partial transpositions:

```{r}
temp <- NULL
temp.Metric <- sort(unique(ac_grid_projects_results_uniform$Metric))

for (projName in names(append(project_signals, project_signals_2nd_batch))) {
  
  project.df <- NULL
  
  for (metric in sort(unique(ac_grid_projects_results_uniform$Metric))) {
    temp.df <- ac_grid_projects_results_uniform[
      ac_grid_projects_results_uniform$Metric == metric, c(projName, "PM", "Var", "SegIdx")]
...
ac_grid_projects_results_uniform <- temp
```
...
```{r}
ac_pmp_score <- function(pmName, projName) {
  temp <- ac_grid_projects_results_uniform[
    ac_grid_projects_results_uniform$PM == pmName &
    ac_grid_projects_results_uniform$Project == projName, as.character(temp.Metric)]
  
  sum(temp) / (nrow(temp) * ncol(temp))
}
```
...
for (pmName in c("I", "II", "III(avg)", "III(b,11)")) {
  temp.pred <- data.frame(
    pred = sapply(X = names(temp), function(pName) ac_pmp_score(pmName = pmName, projName = pName)),
    ground_truth = temp.gt)
  
  temp.df <- rbind(temp.df, data.frame(
    PM = pmName,
    Score_for_best = ac_pmp_score(pmName = pmName, projName = paste0("project_", which.max(temp.gt))),
    Score_for_worst = ac_pmp_score(pmName = pmName, projName = paste0("project_", which.min(temp.gt))),
...
Process model type III(avg) appears best for detecting the absence of the anti-pattern. However, we have multiple projects with a ground truth of $0$, and ideally the champion PM would be picked by examining multiple projects where the confidence in the accuracy of the ground truth is high.
Nonetheless, all these scores are close together, so picking a champion needs to be done with care. Also, the score from each PM is the unweighted aggregation across hundreds of objectives, and at this point we have yet to examine which objectives are the most important (next section).


### Weighted

We have a large number of weights, so it is important to be able to "address" them correctly. A weight is specific to the variable (4), segment (10), and metric (13).
...
```{r}
ac_pmp_score_weighted <- function(pmName, projName, weightGrid, weightVector) {
  ...
  for (i in 1:length(weightVector)) {
    ...
    score <- ac_grid_projects_results_uniform[
      ac_grid_projects_results_uniform$PM == pmName &
      ac_grid_projects_results_uniform$Project == projName &
      ac_grid_projects_results_uniform$Var == params$Var &
      ac_grid_projects_results_uniform$SegIdx == params$SegIdx, as.character(params$Metric)]
...
ac_sc_weights_optim <- function(pmName) {
  loadResultsOrCompute(file = paste0("../results/ac_sc_weights_pm_", pmName, ".rds"), computeExpr = {
    ...
    cl <- parallel::makePSOCKcluster(min(16, parallel::detectCores()))
      ...
      optimParallel::optimParallel(
        ...
        fn = function(x) {
          Metrics::mse(actual = temp.gt, predicted = sapply(X = temp.names, FUN = function(projName) {
            ac_pmp_score_weighted(pmName = pmName, projName = projName, weightGrid = ac_weight_grid, weightVector = x)
          }))
        },
        parallel = list(cl = cl, forward = FALSE, loginfo = TRUE)
      )
...
ac_sc_weights_varimp <- loadResultsOrCompute(file = "../results/ac_sc_weights_varimp.rds", computeExpr = {
  doWithParallelCluster(expr = {
    ...
    set.seed(1)
    control <- caret::trainControl(method="repeatedcv", number=10, repeats=3)
    modelFit <- caret::train(gt ~., data=bla, method="rf", trControl=control)
    imp <- caret::varImp(object = modelFit)
...
No. of variables tried at each split: 2
          ...
          Mean of squared residuals: 0.1019207
                    % Var explained: -15.47")
}
```
...
ac_sc_weights_pm_IIa <- ac_sc_weights_optim(pmName = "II")
...
```{r echo=FALSE}
templ <- list(ac_sc_weights_pm_I, ac_sc_weights_pm_IIa, ac_sc_weights_pm_IIIavg, ac_sc_weights_pm_IIIb11)
temp <- data.frame(
  PM = c("I", "II", "III(avg)", "III(b,11)"),
  ...
  Weights_used_perc = unlist(lapply(templ, function(res) paste0(round(100 * sum(res$par > 0) / length(res$par), 2), "%"))),
...
With relatively few iterations of the optimizer, we can obtain optimized weights for all models. All MSEs are quite low and close together (i.e., no significant variance).

Model type II(a) appears to have the lowest MSE, and requires the second fewest weights to be used (only $22$). This PM is an optimized version of type I, so the expectation is that fewer adjustments (fewer weights, less weight in total) are required for this model to perform well, when compared to type I.

Model type III(b, numInt=11) even only requires the usage of 17 weights ($\approx3.27$% of all objectives), but is simultaneously the 2nd-hardest to optimize. Its MSE is comparable to that of model type II(a), which is also expected, since this type of PM was crafted out of no expectation, and then simply aligned to all projects using a weighted average.

Model type III(avg), while performing similar to model types II(a) and III(b, numInt=11), requires adjustment and usage of comparatively more weights. This is expected, since this kind of model is the weighted average of all observed projects, and hence also includes all fine details and perturbations of each project. It is hence more detailed than the other two previously mentioned types and supposedly requires more weight adjustments to account for that.
Re-running the optimization for this model often uses less than $44$ weights ($\approx5$ to $10$ fewer), and achieves a little lower MSE ($\approx0.329$).

While not unusable, the first kind of model was clearly the one being "most off" in describing the manifestation of the Fire Drill. It has the largest MSE, requires the most weight adjustments, and the most steps during optimization. This was expected as well, since this model was designed without inspecting any of the data or ground truth. All the other PM types are either enhanced by data, or solely based on it.
...
The more scores that are used (i.e., weight $>0$), the more insights we can gain. As a compromise, we will first look into the weights of model type III(avg), as it performs similarly to the absolute best ones, and uses relatively many weights ($44$). Also during some runs, it was equally good.
...
```{r echo=FALSE}
temp <- cbind(ac_weight_grid, data.frame(weight = ac_sc_weights_pm_I$par + ac_sc_weights_pm_IIa$par + ac_sc_weights_pm_IIIavg$par + ac_sc_weights_pm_IIIb11$par))
...
barplot(table(temp$SegIdx), main = "No. of scores used / segment (normalized)")
grid()
...
While the average importance per segment suggests that the first segments are somewhat more important than the last ones, we see the most scores were used in the two middle segments and the last segment.
This means that it is more important to sample from a process in these segments, and that fluctuations in these segments are more sensible to the outcome (assessing the goodness of fit). This finding is also interesting w.r.t. the binary decision rule as used for issue-tracking data. It measures at three characteristic points in time of a project, namely at $t_1=0.4$, $t_2=0.85$, and $t_{\mathrm{end}}=1$. If we look at the average importance of each segment, the choice for $t_1$ seems to be a good fit, while the others are slightly off.

The average importance per variable in figure \ref{fig:ac-sc-weights-detailed-plot} favored `A` and `FREQ`, then `SCD` and then with some distance `CP`. In figure \ref{fig:ac-sc-weights-detailed-plot-counts} however, `A`, `CP`, and `FREQ` have almost the same number of scores on them $\approx27$, while `SCD` has almost $10$ more. Requiring more scores on a variable could indicate it having a large variance, for example. In other words, if relatively more scores are required in a segment or for a variable, then those are probably required to accommodate the larger variances.
We also see a significant difference in average importance and number of type of scores, with the ImpulseFactor being the score used most frequently by far. The impulse factor divides the process' peak by its average and is therefore sensitive to perturbations of the entire signal and not just its extremes. A difference between Peak and ImpulseFactor could indicate that in fact differences between process and model exist.
