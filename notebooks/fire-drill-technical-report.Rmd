...
p2_attach_combined_loss <- function(project, vartype, intervals) {
  ...
  lossRss <- srBTAW_Loss2Curves$new(
    srbtaw = p2_inst[[vartype]], wpName = paste0("p1_", vartype),
    wcName = paste(project, vartype, sep = "_"), weight = weight,
    intervals = intervals, use = "rss", continuous = FALSE,
    numSamples = rep(25e2, length(intervals)))
  ...
  # lossArea <- srBTAW_Loss2Curves$new(
  #   srbtaw = p2_inst[[vartype]], wpName = paste0("p1_", vartype),
  #   wcName = paste(project, vartype, sep = "_"), weight = weight,
  #   intervals = intervals, use = "area", continuous = TRUE)
  # lossCorr <- srBTAW_Loss2Curves$new(
  #   srbtaw = p2_inst[[vartype]], wpName = paste0("p1_", vartype),
  #   wcName = paste(project, vartype, sep = "_"), weight = weight,
  #   intervals = intervals, use = "corr", continuous = FALSE,
  #   numSamples = rep(1e3, length(intervals)))
  # lossArclen <- srBTAW_Loss2Curves$new(
  #   srbtaw = p2_inst[[vartype]], wpName = paste0("p1_", vartype),
  #   wcName = paste(project, vartype, sep = "_"), weight = weight,
  #   intervals = intervals, use = "arclen")
  # 
  # p2_lls$setObjective(
  #   name = paste(project, vartype, paste(intervals, collapse = "_"),
  #                "area", sep = "_"), obj = lossArea)
  # p2_lls$setObjective(
  #   name = paste(project, vartype, paste(intervals, collapse = "_"),
  #                "corr", sep = "_"), obj = lossCorr)
  # p2_lls$setObjective(
  #   name = paste(project, vartype, paste(intervals, collapse = "_"),
  #                "arclen", sep = "_"), obj = lossArclen)
  # 
  # p2_inst[[vartype]]$addLoss(loss = lossArea)
  # p2_inst[[vartype]]$addLoss(loss = lossCorr)
  # p2_inst[[vartype]]$addLoss(loss = lossArclen)
}
```
...
Finally, we add the regularizer for extreme intervals and extreme outputs:

```{r}
p2_lls$setObjective(name = "p2_reg_exint", obj = TimeWarpRegularization$new(
  weight = 1, use = "exint",
...
p2_lls$setObjective(name = "p2_reg_output", obj = YTransRegularization$new(
  weight = .25, use = "tikhonov",
  wpName = NULL, wcName = NULL, intervals = seq_len(length.out = length(p2_vars))
)$setSrBtaw(srbtaw = p2_inst$A))

# We add the regularizers to the first srBTAW, because it does not matter.
```
...
# parallel::clusterExport(cl, varlist = list("p2_smv", "p2_lls"))
tempf <- tempfile()
...
res <- doWithParallelClusterExplicit(cl = cl, expr = {
  optimParallel::optimParallel(
    ...
    fn = function(x) {
      ...
      temp$b$compute0()
      # p2_smv$setParams(params = x)
      # p2_lls$compute0()
    },
    # gr = function(x) {
    #   library(foreach)
    #   cl_nested <- parallel::makePSOCKcluster(parallel::detectCores())
    #   doSNOW::registerDoSNOW(cl = cl_nested)
    #   
    #   foreach::foreach(
    #     pIdx = seq_len(length.out = length(x)),
    #     .combine = c,
    #     .inorder = TRUE,
    #     .export = c("p2_smv", "p2_lls"),
    #     .packages = c("pracma")
    #   ) %dopar% {
    #     pracma::grad(f = function(p) {
    #       params <- x
    #       params[pIdx] <- p
    #       p2_smv$setParams(params = params)
    #       p2_lls$compute0()
    #     }, x0 = x[pIdx])
    #   }
    # },
...
saveRDS(object = res, file = "../results/ser-test.rds")
```


```{r}
# for (o in p2_lls$.__enclos_env__$private$objectives) {
#   print(paste(o$.__enclos_env__$private$use, o$getWcName(), o$getWpName(), unname(o$compute0())))
# }

p2_res <- loadResultsOrCompute(file = "../results/fdtr_p2_res.rds", computeExpr = {
  cl <- parallel::makePSOCKcluster(parallel::detectCores())
  doWithParallelClusterExplicit(cl = cl, expr = {
    fr <- FitResult$new(paramNames = c(p2_smv$getParamNames(), "loss"))
    fr$start()
    ...
    optR <- stats::optim(
      ...
      fn = function(x) {
        fr$startStep()
        p2_smv$setParams(params = x)
        loss <- p2_lls$compute0()
        fr$stopStep(resultParams = c(x, loss))
        loss
      # },
      # gr = function(x) {
      #   pracma::grad(f = function(x_) {
      #     p2_smv$setParams(params = x_)
      #     p2_lls$compute0()
      #   }, x0 = x)
      }
    )
...
time_warp_project_p1 <- function(project) {
  ...
  for (vartype in names(p1_signals)) {
    ...
    # loss <- srBTAW_Loss_JSD$new(
    #   intervals = seq_len(length.out = 4), weight = 1,
    #   wpName = p1_signals[[vartype]]$getName(), wcName = project[[vartype]]$getName())
    # 
    # inst$addLoss(loss = loss)
    # obj$setObjective(name = paste("jsd", vartype, sep = "_"), obj = loss)
    
    loss <- srBTAW_Loss_Rss$new(intervals = seq_len(length.out = 4),
      wpName = p1_signals[[vartype]]$getName(), wcName = project[[vartype]]$getName(),
      weight = 1, continuous = FALSE, numSamples = rep(250, length(names(project))))
    inst$addLoss(loss = loss)
    obj$setObjective(name = paste("rss", vartype, sep = "_"), obj = loss)
    
    # loss <- srBTAW_Loss2Curves$new(srbtaw = inst, wpName = p1_signals[[vartype]]$getName(),
    #   wcName = project[[vartype]]$getName(), weight = .25, intervals = seq_len(length.out = 4),
    #   use = "corr", numSamples = rep(250, 4))
    # inst$addLoss(loss = loss)
    # obj$setObjective(name = paste("corr", vartype, sep = "_"), obj = loss)
  }
  
  # for (varName in names(p1_signals)) {
  #   inst$setSignal(signal = p1_signals[[varName]])
  #   inst$setSignal(signal = project[[varName]])
  #   
  #   for (intervals in interval_types) {
  #     loss <- srBTAW_Loss_Rss$new(intervals = intervals,
  #       wpName = p1_signals[[varName]]$getName(), wcName = project[[varName]]$getName(),
  #       weight = weight_vartype[varName] * unique(weight_interval[intervals]),
  #       continuous = TRUE)#, numSamples = rep(1e3, length(names(project))))
  #     
  #     inst$addLoss(loss)
  #     obj$setObjective(name = paste(
  #       "rss", varName, paste(intervals, collapse = "_"), sep = "_"), obj = loss)
  #   }
  # }
  # 
  # Add a time-warp regularizer:
  reg <- TimeWarpRegularization$new(
    weight = .05, use = "exint2", wpName = p1_signals$A$getName(), wcName = project$A$getName(),
    intervals = seq_len(length.out = length(names(project))))
  inst$addLoss(loss = reg)
  obj$setObjective(name = "reg_exint", obj = reg)
...
```{r}
res16 <- doWithParallelCluster(expr = {
