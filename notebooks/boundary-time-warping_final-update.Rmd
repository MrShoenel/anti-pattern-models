...
In some of our other tests it turned out that a slightly modified logarithmic residual sum of squares is the most robust and consistently required the least iterations. The modified version adds $1$ to the strictly positive result of the RSS and returns the logarithm of it. This loss converges to $+0$.

$$
\begin{aligned}
  \mathcal{L}^{\text{data}}=&\;\log{\Big(1+\text{RSS}\Big)}\;\text{,}
  \\[1ex]
  =&\;\log{\bigg(1+\sum_{i=1}^{N}\;\Big(\mathbf{y}_i-m_{q=f(i)}\big(\mathbf{x}_i\big)\Big)^2\bigg)}\;\text{, where}\;f\;\text{selects}\;q\;\text{according to}\;i\text{.}
\end{aligned}
$$
...
## Regularized loss for the data

$$
\begin{aligned}
  \text{RMSE}=&\;\sqrt{\frac{1}{N}\;\sum_{i=1}^{N}\;(\mathbf{y}_i-\mathbf{\hat{y}}_i)^2}\;\text{,}
  \\[1ex]
  =&\;\sqrt{\frac{1}{N}\;\Bigg[\sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;\Bigg(\mathbf{y}_i-f\bigg(\frac{(\mathbf{x}_i - b_{q_s})\times(b_{o_e} - b_{o_s})}{b_{q_e}-b_{q_s}} + b_{o_s}\bigg)\Bigg)^2\Bigg]}\;\text{, and our data-loss will be:}
  \\[1em]
  \mathcal{L}^{\text{data}}=&\;\log{\big(1+\text{RMSE}\big)}\;\text{, with gradient}
  \\[1ex]
  \frac{\partial\,\mathcal{L}^{\text{data}}}{b_{q_s}}=&\;-1\times\frac{
    \sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;
    \Big(\frac{(b_{o_e}-b_{o_s})\times(\mathbf{x}_i-b_{q_s})}{(b_{q_e}-b_{q_s})^2}-\frac{b_{o_e}-b_{o_s}}{b_{q_e}-b_{q_s}}\Big)\times
    \Big(\mathbf{y}_i-f(\psi_i)\Big)\times
    f'(\psi_i)
  }{
    N\times\Bigg(1+\sqrt{
      \frac{1}{N}\times\Big[\sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;(\mathbf{y}_i-f(\psi_i))^2\Big]
    }\,\Bigg)\times
    \sqrt{
      \frac{1}{N}\times\Big[\sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;(\mathbf{y}_i-f(\psi_i))^2\Big]
    }
  ...
  \\[1ex]
  \frac{\partial\,\mathcal{L}^{\text{data}}}{b_{q_e}}=&\;\frac{
    \sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;(b_{o_e}-b_{o_s})\times(\mathbf{x}_i-b_{q_s})\times(\mathbf{y}_i-f(\psi_i))\times f'(\psi_i)
  }{
    N\times(b_{q_e}-b_{q_s})^2\times\Bigg(1+\sqrt{
      \frac{1}{N}\times\Big[\sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;(\mathbf{y}_i-f(\psi_i))^2\Big]
    }\,\Bigg)\times
    \sqrt{
      \frac{1}{N}\times\Big[\sum_{q=i}^{\max{Q}}\,\sum_{i=1}^{N}\;(\mathbf{y}_i-f(\psi_i))^2\Big]
...
While this is not pretty, it is robust for two reasons: First, we are normalizing the error by dividing through the mean, then taking the root. Secondly, since the error is always positive, but not guaranteed to be greater than $1$, we add this before we pass it to the logarithm.

A simple model using only this loss for the data and no regularizers should converge with and without the gradient.
...
optR <- stats::optim(
  ...
  #lower = rep(0, length(query_bounds)),
  #upper = rep(1, length(query_bounds)),
  par = c(query_bounds, 1/3, 1/3, 1/3),
  fn = function(x) {
    loss <- L_updated_log(
      theta_b_org = query_bounds,
      theta_b = x[1:length(query_bounds)],
      theta_w = tail(x, 3),
      r = r,
      f = f,
      weightErr = 2/3,
...
temp3 <- M_final_no_NA(
  theta_b_org = query_bounds,
  theta_b = optR$par[1:11],
  r = signal_ref,
  f = signal_query)

plotBtw(data.frame(x = temp3$X, y = temp3$y_hat), optR$par[1:11])
```
