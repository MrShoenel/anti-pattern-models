...
\begin{aligned}
  ...
  \\[1ex]
  &\;\mathbf{x}_q\;\text{, the vector of all}\;x_q\;\text{in the current interval,}
  \\[1ex]
  \text{}&\;q \in \mathcal{I}\;\text{, the index of the query-interval}\;x_q\;\text{falls into,}
  \\[1ex]
  &\;\langle s_o,e_o\rangle\;\text{, where }\;s,e\;\text{ demarcate the start and end of an interval,}
  \\[1ex]
  &\;\langle s_q,e_q\rangle\;\text{, the query-interval and its start/end,}
  \\[1ex]
  ...
  \\[1ex]
  \tau_q = &\;x_o - x_q\;\text{, or, alternatively,}
  \\[1ex]
  \tau_q = &\;s_o + \bigg(\frac{x_q - s_q}{e_q - s_q}\times (e_o - s_o)\bigg) - x_q\;\text{, vector-valued as}
  \\[1ex]
  \mathbf{\tau} = &\ x_o - \mathbf{x}_q\;\text{, resulting in}
  \\[1ex]
  \hat{y}_q = &\; f(\tau_q+x_q)\;\text{, or,}
  \\[1ex]
  \mathbf{\hat{y}} = &\; f(\mathbf{\tau}+\mathbf{x}_q)\;\text{ (vector-valued version).}
\end{aligned}
$$

In other words, $x_o$ is the $x$ we want to return $y$ for. In more other words, when boundaries are moved, and intervals get translated and scaled, $x_q$ always falls into an interval, and it assumes a relative position in it. So the correct $y$ to return is the one in the original interval, at that same relative position. All this can be reduced to a single offset, $\tau$, added to $x_q$.
...
$$
\begin{aligned}
  \mathbf{\tau} &= x_o - \mathbf{x}_q\;\text{, fully expanded as}
  \\[1ex]
  \tau_q &= s_o + \bigg(\frac{x_q - s_q}{e_q - s_q}\times (e_o - s_o)\bigg) - x_q\;\text{,}
  \\[1ex]
  \nabla\tau_q &= \frac{\partial\;\text{RSS}_m}{\partial\;\tau}\big(\mathbf{x},\mathbf{\tau}\big)\;\text{, the gradient for}\;\tau\;\text{in the current interval}\;q\,\text{,}
  \\[1em]
  s_q\prime &= \frac{x_{\text{first}}\times(s_o - e_o) + e_q\times\Big(\nabla\tau_q + x_{\text{first}} - s_o\Big)}{\nabla\tau_q + x_{\text{first}} - e_o}\;\text{, and}
  \\[1ex]
  e_q\prime &= \frac{e_o\times(s_q-x_{\text{last}}) + e_ox_{\text{last}} - s_q(\nabla\tau_q + x_{\text{last}})}{s_o - x_{\text{last}} - \nabla\tau_q}\;\text{.}
\end{aligned}
$$

Note that all the other parameters, $s_o, e_o, x_q, \tau$, are either given/constant or calculated. So, for each interval, the gradient calculates a value for $\tau$ (or better: how it changes), and we have now the means to translate this back to the current delimiting boundaries, $s_q,e_q$, producing $s_q\prime,e_q\prime$.
